...
class BienSoLoiDialog:
    def __init__(self, parent, ma_the, bien_so_cu, bien_so_moi, anh_xe_ra=None, anh_vao_url=None, anh_mat_vao_url=None, anh_mat_ra=None):
        self.parent = parent
        self.ma_the = ma_the
        ...
        self.anh_mat_vao_url = anh_mat_vao_url  # URL ảnh mặt vào từ database
        self.anh_mat_ra = anh_mat_ra  # Đường dẫn ảnh mặt ra vừa chụp
        self.ket_qua = None
    ...
    def load_anh_tu_url(self, url_path):
        ...
        try:
            ...
            # Convert URL "server/images/file.jpg"
            if url_path.startswith("server/"):
                ...
                if os.path.exists(url_path):
                    ...
                    print(f"File không tồn tại: {url_path}")
                    return None
    ...
    def tao_dialog(self):
        ...
        self.dialog.geometry("900x850")  # Tăng kích thước để chứa thêm ảnh face
        ...
        # **THÊM MỚI: Frame hiển thị ảnh xe so sánh**
        self.hien_thi_anh_xe_so_sanh(main_frame)
        
        # **THÊM MỚI: Frame hiển thị ảnh mặt so sánh**
        self.hien_thi_anh_mat_so_sanh(main_frame)
    ...
    def hien_thi_anh_xe_so_sanh(self, parent):
    ...
    def hien_thi_anh_mat_so_sanh(self, parent):
        """Hiển thị 2 ảnh mặt so sánh: vào và ra"""
        # Chỉ hiển thị nếu có ít nhất 1 ảnh mặt
        if not self.anh_mat_vao_url and not self.anh_mat_ra:
            return
            
        face_frame = tk.LabelFrame(
            parent, 
            text="So Sánh Ảnh Mặt", 
            font=("Helvetica", 12, "bold"), 
            bg="#f0f9ff", 
            fg="#0369a1"
        )
        ...
        # Container cho 2 ảnh mặt
        container = tk.Frame(face_frame, bg="#f0f9ff")
        container.pack(fill=tk.X, padx=10, pady=10)
        ...
        tk.Label(left_frame, text="Ảnh Mặt khi VÀO", 
                font=("Helvetica", 11, "bold"), bg="#10b981", fg="white").pack(fill=tk.X)
        
        # Load và hiển thị ảnh mặt vào
        anh_mat_vao = self.load_anh_tu_url(self.anh_mat_vao_url)
        if anh_mat_vao:
            try:
                resized_mat_vao = anh_mat_vao.resize((200, 150), Image.Resampling.LANCZOS)
                photo_mat_vao = ImageTk.PhotoImage(resized_mat_vao)
                
                label_mat_vao = tk.Label(left_frame, image=photo_mat_vao, bg="#f0f9ff")
                label_mat_vao.image = photo_mat_vao  # Keep reference
                label_mat_vao.pack(pady=5)
            except Exception as e:
                print(f"Lỗi hiển thị ảnh mặt vào: {e}")
                tk.Label(left_frame, text="Không thể tải ảnh mặt vào", 
                        bg="#f0f9ff", fg="#dc2626").pack(pady=20)
        else:
            tk.Label(left_frame, text="Không có ảnh mặt vào", 
                    bg="#f0f9ff", fg="#6b7280").pack(pady=20)
        ...
        tk.Label(right_frame, text="Ảnh Mặt khi RA", 
                font=("Helvetica", 11, "bold"), bg="#dc2626", fg="white").pack(fill=tk.X)
        
        # Hiển thị ảnh mặt ra
        if self.anh_mat_ra:
            try:
                # Load ảnh mặt ra từ đường dẫn
                if os.path.exists(self.anh_mat_ra):
                    anh_mat_ra_img = Image.open(self.anh_mat_ra)
                    resized_mat_ra = anh_mat_ra_img.resize((200, 150), Image.Resampling.LANCZOS)
                    photo_mat_ra = ImageTk.PhotoImage(resized_mat_ra)
                    
                    label_mat_ra = tk.Label(right_frame, image=photo_mat_ra, bg="#f0f9ff")
                    label_mat_ra.image = photo_mat_ra  # Keep reference
                    label_mat_ra.pack(pady=5)
                else:
                    tk.Label(right_frame, text="File ảnh mặt ra không tồn tại", 
                            bg="#f0f9ff", fg="#dc2626").pack(pady=20)
            except Exception as e:
                print(f"Lỗi hiển thị ảnh mặt ra: {e}")
                tk.Label(right_frame, text="Không thể tải ảnh mặt ra", 
                        bg="#f0f9ff", fg="#dc2626").pack(pady=20)
        else:
            tk.Label(right_frame, text="Không có ảnh mặt ra", 
                    bg="#f0f9ff", fg="#6b7280").pack(pady=20)
