...
import requests
import json
from server.url import url_login_api
...
class LoadingScreen(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Progress bar
        self.progress = ttk.Progressbar(self, length=300, mode='determinate', maximum=100)
        ...
        self.protocol("WM_DELETE_WINDOW", self.cancel_loading)
    ...
    def loading(self):
        ...
        if self.current <= 100:
            ...
            self.percent_label['text'] = f"{self.current}%"
            self.current += 1
            self.after(10, self.loading)  # 150ms * 100 = 15s
        else:
            self.destroy()
            self.parent.deiconify()
    ...
    def apply(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('code') and data.get('token'):
                    self.result = True
                    return
...
def show_login_dialog(root):
    while True:
        ...
        if dialog.result:
            if messagebox.askyesno("Xác nhận", "Bắt đầu phiên làm việc mới?"):
                loading = LoadingScreen(root)
                root.wait_window(loading)
                # Nếu loading bị hủy thì quay lại login
                if hasattr(loading, 'cancelled') and loading.cancelled:
                    continue
                
                # Gọi API tạo bảng cho phiên làm việc mới
                try:
                    from server.api import taoBangChoPhienLamViec
                    result = taoBangChoPhienLamViec()
                    if not result.get('success', False):
                        error_msg = result.get('message', '')
                        if "đã tồn tại" in error_msg.lower():
                            if messagebox.askyesno("Thông báo", "Tiếp tục phiên làm việc hôm nay?"):
                                return True
                            continue
                        messagebox.showerror("Lỗi", f"Không thể tạo phiên làm việc mới: {error_msg}")
                        continue
                except Exception as e:
                    messagebox.showerror("Lỗi", f"Không thể tạo phiên làm việc mới: {str(e)}")
                    continue
