...
//   // Simple reset logic when resetKey changes
//   useEffect(() => {
//     if (resetKey > 0) {
//       console.log(
//         `🔄 RTSPPlayer reset triggered for camera ${cameraId} with key ${resetKey}`
//       );
...
//       if (wsRef.current) {
//         wsRef.current.close(1000, "Reset triggered");
//         wsRef.current = null;
//       }
...
//         } catch (err) {
//           console.warn(
//             `Error cleaning up MediaSource during reset for camera ${cameraId}:`,
//             err
//           );
//         }
...
//       // Trigger reinitialize with delay to ensure cleanup completes
//       setTimeout(() => {
//         if (mountedRef.current && rtspUrl) {
//           console.log(
//             `🚀 Reinitializing RTSPPlayer for camera ${cameraId} after reset`
//           );
//           // This will trigger the main useEffect to run again
//           setError("Reinitializing...");
...
//   useEffect(() => {
//     // Only prevent initialization if already initializing and not after a reset
//     if (initializingRef.current && resetKey === 0) {
//       return;
//     }
...
//         const wsUrl = `ws://localhost:9999/?rtsp=${encodeURIComponent(
//           stableRtspUrl.current
//         )}&cameraId=${encodeURIComponent(stableCameraId.current || "unknown")}`;
...
//               10000 * Math.pow(2, reconnectAttemptsRef.current),
//               60000
//             ); // Max 60 seconds
...
//             console.log(
//               `🔌 SourceBuffer ready, connecting WebSocket for camera ${cameraId}`
//             );
//             connectWebSocket();
//           } catch (err) {
//             setError(err.message);
...
//         } catch (err) {
//           console.warn(
//             `Error ending media source for camera ${cameraId}:`,
//             err
//           );
//         }
//       }
...
//       reconnectAttemptsRef.current = 0;
//     };
//   }, [resetKey]); // Include resetKey to reinitialize when reset is triggered
...
//       if (
//         now - lastCaptureTimeRef.current >= SNAP_INTERVAL &&
...
//                 // Reduced minimum size for test
//                 console.log(
//                   "❌ Failed to create blob from frame or blob too small:",
//                   blob?.size || 0
//                 ); // Debug log
//                 return;
//               }
...
//             "image/jpeg",
//             0.9
//           ); // Increase quality for better detection
//         } catch (captureError) {
...
//   return (
//     <div
//       className={`rtsp-player ${className}`}
  ...
  return (
    ...
    >
      <video
        ...
        // Minimize buffering for real-time streaming
        style={{
          width: "100%",
          height: "100%",
          objectFit: "cover",
        ...
        onLoadedMetadata={(e) => {
          // Set ultra-low latency properties
          const video = e.target;
          if (video) {
            // Minimize buffering for real-time
            video.buffered &&
              console.log(`📊 Video buffered: ${video.buffered.length} ranges`);

            // Try to set low latency hint
            if ("requestVideoFrameCallback" in video) {
              video.requestVideoFrameCallback(() => {
                // Frame-level processing for minimal latency
              });
            }
          }
        }}
