...
const ImageCaptureModal = ({
  ...
  useEffect(() => {
    if (
      isOpen &&
      (images.plateImageBlob || images.plateImage) &&
      !licensePlateResult &&
      !isRecognizing
    ) {
  ...
  const setupAutoSave = async () => {
    if ("showDirectoryPicker" in window) {
      ...
      alert(
        "Trình duyệt không hỗ trợ File System Access API. Ảnh sẽ tự động download."
      );
    }
  };
      ...
      <div className="image-capture-modal">
        ...
        <div className="image-capture-modal-body">
          <div className="card-info">
            <h3>
              Mã Thẻ: <span className="card-id">{cardId}</span>
            </h3>
            <p
              style={{
                ...
                <div className="recognition-result">
                  <div
                    className={`recognition-status ${
                      licensePlateResult.confidence >= 0.9
                        ? "success"
                        : "warning"
                    }`}
                  >
                  ...
                  <div className="license-plate-info">
                    ...
                    {licensePlateResult.license_plate &&
                      licensePlateResult.confidence < 0.9 && (
                        <div className="plate-number-low-confidence">
                          <strong>Biển Số (Độ tin cậy thấp): </strong>
                          <span className="plate-text-low-confidence">
                            {licensePlateResult.license_plate}
                          </span>
                          <div className="low-confidence-warning">
                            Không hiển thị do độ tin cậy &lt; 90%
                          </div>
                        </div>
                      )}
                      ...
                      <div className="confidence">
                        ...
                        <span
                          className={`confidence-value ${
                            licensePlateResult.confidence > 0.8
                              ? "high"
                              : licensePlateResult.confidence > 0.6
                              ? "medium"
                              : "low"
                          }`}
                        >
                    ...
                    {licensePlateResult.processing_time && (
                      <div className="processing-time">
                        ...
                        <span>
                          {licensePlateResult.processing_time.toFixed(2)}s
                        </span>
                      </div>
                    )}
                  </div>
