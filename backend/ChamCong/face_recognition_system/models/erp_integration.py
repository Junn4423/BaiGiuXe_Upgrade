...
class ERPAttendanceIntegration:
    ...
    def create_attendance_record(self, employee_id, attendance_time=None):
        """
        ...
        try:
            ...
            values = (
                ...
                attendance_date,
                attendance_time_str,
                'IN',  # lv004 - Loại điểm danh (IN/OUT hoặc giá trị phù hợp)
                self.default_source,  # lv005 set mặc định là 'Camera'
                self.camera_ip  # lv099 set là ip của camera
            )
            
            cursor.execute(query, values)
            conn.commit()
            
            logger.info(f"Đã ghi chấm công ERP cho nhân viên {employee_id} vào {attendance_date} {attendance_time_str}")
    ...
    def check_recent_attendance(self, employee_id, minutes=10):
        ...
        try:
            ...
            query = f"""
                ...
                FROM {self.table}
                WHERE {self.columns['employee_id']} = %s
                  AND CONCAT({self.columns['date']}, ' ', {self.columns['time']}) >= 
                      DATE_SUB(NOW(), INTERVAL %s MINUTE)
            """
            
            cursor.execute(query, (employee_id, minutes))
            result = cursor.fetchone()
