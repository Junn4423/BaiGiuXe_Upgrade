...
<table class="ftable" cellpadding="1" cellspacing="1" style="position:relative;">
	...
	$vStrPost='';
	$vCount=0;
	echo $slq="select * from (select A.lv001,A.lv002,A.lv003,A.lv004,DATEDIFF(B.lv007,'$vNow') State  from hr_lv0020 A inner join hr_lv0027 B on A.lv001=B.lv002  where  B.lv006<='$vNow' and B.lv007>='$vNow' and  A.lv009 not in (2,3,7)) MP where MP.State<=60 order by MP.State Asc";
$vResult=db_query($slq);
while($vrow=db_fetch_array($vResult))
{
	$vCount++;
