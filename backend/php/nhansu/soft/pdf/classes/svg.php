...
class SVG {
	...
	function svgImage($attribs) {
		...
		// preserveAspectRatio
		$ar = preg_explode('/\s+/', strtolower($par));
	...
	function svgGradient($gradient_info, $attribs, $element){
		...
		if ($element=='rect') {
			...
			$pts = preg_explode('/[ ,]+/', trim($attribs['points']));
			$maxr=$maxb=0;
			$minl=$mint=999999;
	...
	function svgOffset ($attribs){
		...
		if(isset($this->svg_attribs['viewBox'])) {
			$vb = preg_explode('/\s+/is', trim($this->svg_attribs['viewBox']));
			if (count($vb)==4) {
...
*/
	...
	if ($critere_style['stroke'] != 'none'){
		...
		if (isset($critere_style['stroke-dasharray'])){
			$off = 0;
			$d = preg_explode('/[ ,]/',$critere_style['stroke-dasharray']);
			if (count($d) == 1 && $d[0]==0) {
				$path_style .= '[] 0 d '; 
			}
			else {
			...
			function xml_svg2pdf_start($parser, $name, $attribs){
				...
				case 'text':
						...
						if (_SVG_CLASSES && isset($attribs['class']) && $attribs['class']) {
							$classes = preg_explode('/\s+/',trim($attribs['class']));
...
function markScriptToLang($html) {
	...
	$n = '';
	$a=preg_explode('/<(.*?)>/ms',$html,-1,PREG_SPLIT_DELIM_CAPTURE);
	foreach($a as $i => $e) {
	   if($i%2==0) {
		$e = strcode2utf($e);
