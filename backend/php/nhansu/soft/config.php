...
define("DB_USER","root");
define("DB_PWD","");
define("DB_DATABASE","nhansu_erp_sofv4_0");
...
{         
             $paging="";
				 ...
				 if($totalRows>=$maxRows)
					...
					if($totalPages>10 && ($totalPages-$curPage)<10 )
					{
						$paging .= "<a href='javascript:GotoPageMulti($frmName,$vobj,".(1).")'  tabindex=$vTabIndex>"."<font class='lvnumpagenext'> Đầu</font></a>&nbsp;";
					}
					 if($curPage>$maxPages)
					...
					if($totalPages-$curPage>10)
				{
					$paging .= "<a href='javascript:GotoPageMulti($frmName,$vobj,".($totalPages).")'  tabindex=$vTabIndex>"."<font class='lvnumpagenext'> Cuối</font></a>&nbsp;";
				}
		return $paging;		
}
...
{
	$vArrLang=Array('VN'=>'VN','EN'=>'EN');
	if($vStrLanguage!='VN' && $vStrLanguage!='EN')
	{
		
		$vArr=null;
		$vArr=array();
		$vTT=0;
		foreach($vArrLang as $vLang)
		{
			$filename =$vDir."languages/".$vLang."/".$vStrFile;
			...
			while($vLine=stream_get_line($handle ,0,"\n"))
			{
				if($vTT>0)
				{
					$vArr[$i]=$vArr[$i]."<br>".substr($vLine,0,strlen($vLine)-1);
				}
				else
