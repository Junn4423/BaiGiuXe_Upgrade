...
{
	$vrStr="";
	$tsql="select A.lv001 from $vTable A where A.lv001 in ($vStr)";
	$tresult=db_query($tsql);
	if($tresult)
	{
...
function LCurrency($strValue,$vLang)
{
	...
	$decimal=strlen($arrNum[1]);
		if($strValue==NULL || trim($strValue)=='' || trim($strValue)=='&nbsp;')  $strValue=0;
...
// {
	...
	// $strReturn = "";
	// $arrNum = explode(".", $strValue);//Tach thanh 2 phan 2 ben dau cham dua vao mang
	// if (isset($arrNum[1]) && ((int)$arrNum[1]) != 0) 
	// {
		// $vChieuDai=strlen($arrNum[1]);
		// if($vChieuDai>2)
		// {
			// for($i=2;$i<=$vChieuDai;$i++)
			// {
				// $vGiaTri=substr($arrNum[1],$vChieuDai-$i,1);
				// if($vGiaTri!='0')
				// {
					// $strValue=$arrNum[0].'.'.substr($arrNum[1],0,$vChieuDai-$i+1);
					// $decimal=$vChieuDai-$i+1;
...
function LCurrencys($strValue, $vLang)
{
    ...
    if (!is_numeric($strValue) || is_nan((float)$strValue) || trim($strValue) === '' || trim($strValue) === '&nbsp;') {
        $strValue = 0;
    }
    ...
    // Bước 2: Tính số chữ số sau dấu thập phân (nếu có)
    $arrNum = explode(".", (string)$strValue);
    if (isset($arrNum[1]) && ((int)$arrNum[1]) != 0) {
        $vChieuDai = strlen($arrNum[1]);
    ...
    // Bước 3: Format theo ngôn ngữ
    if ($vLang === "VN" || $vLang === "vn") {
        $strReturn = number_format((float)$strValue, $decimal, ",", ".");
    } else {
        $strReturn = number_format((float)$strValue, $decimal, ".", ",");
    }
...
/////////////////////////////////////////////////////////////////Read VND////////////////////////////////////////////////
function LNum2Text($NumCurrency, $plang,$vTienTe='')
{
	$vreturn="";
	if($plang == "VN")//Ca tieng Viet va tieng Anh deu su dung ngon ngu tieng Viet
	{
		...
		if($arrNum[1]>0 && strlen($arrNum[1])==1)
		{
			$arrNum[1]=$arrNum[1].'0';
		}
		...
		while( $I <= 5)	
		{
			...
			if($SoDoi != 0)
			{
				...
				$BangChu = trim($BangChu).((strlen($BangChu) == 0) ? "" : " ").(($Tram != 0) ? trim($CharVND[$Tram])." trăm " :(($Tram==0 && $vLanDau)?" không trăm ":""));
	...
	if($plang == "EN")//Ca tieng Viet va tieng Anh deu su dung ngon ngu tieng Viet
	{
		...
		$vSoChan=$NumCurrency-(float)("0.$SoLe");
		$DonViTien = $vTienTe;
		if($DonViTien=='') $DonViTien='dong';
		$vreturnchan=LNum2TextEN($NumCurrency);
		
		$vreturnle='';
		if($SoLe>0)
		{
			$vreturnle=LNum2TextEN($SoLe,'CENTS');
		}
		$vreturn=LV_TraLaiTen($DonViTien,$vreturnchan,$vreturnle,$vSoChan,$SoLe);
		$vreturn=strtoupper($vreturn);
		return $vreturn;
		//$DonViLe = "xu";
		$DonViLe = "";//xu
...
function LV_TraLaiTen($vTienTe,$vChuChan,$vChuLe,$vSoTien,$SoLe)
{
	$vReturn='';
	switch($vTienTe)
	{
		case 'USD':
			...
			if($SoLe==1)
				...
				if($vChuLe!='')
				{
					$vReturn=$vChuChan.' '.$vTienTe.' AND '.$vDVLe.' '.$vChuLe.' ONLY';
				}
				else
				{
					$vReturn=$vChuChan.' '.$vTienTe.' ONLY';
				}
			break;
		default:
			if($SoLe==1)
...
function LNum2TextEN($num = false)
{
...
{
	$vsql="	update lv_lv0007 set lv099='$themes' WHERE lv006='$vUserID';";
	$vresult=db_query($vsql);
}
function GetUserName($strUser, $plang){
	//if(is_numeric(substr($strUser,0,1))){
		$strSQL = "SELECT lv002 Name FROM hr_lv0020 WHERE lv001='$strUser'; ";
		$arrResult = db_query($strSQL);
		$intRows = db_fetch_array($arrResult);
		if($plang=="VN"){
			$strFullName=$intRows['Name'];
		} else {
			$strFullName=$intRows['Name'];
		}
		return $strFullName;
...
function InsertWithCheckFist($vTableName, $vField, $vSYMBOL,$vLengthCast){
	$vReturn = "";
 	$sqlS = "	SELECT 
					(SELECT MAX(CAST(SUBSTRING($vField,1,$vLengthCast) AS SIGNED)) FROM $vTableName where ($vField LIKE '%$vSYMBOL') ) ROWMAX, 
					(SELECT COUNT(*) FROM $vTableName where ($vField LIKE '%$vSYMBOL') ) AS NUMROW;";
	...
	if ($totalRows>0) {
		...
		if ((int)$vNumRow <= (int)$vRowMax) {
			...
			if($vRowMax>1)
			{
				...
				return Fillnum("1",$vLengthCast).$vSYMBOL;
		}
	}
	return $vReturn;
}
...
function InsertWithCheckExt1($vTableName, $vField, $vSYMBOL,$vLengthCast){
	$vReturn = "";
	$sqlS = "	SELECT 
					(SELECT MAX(CAST(SUBSTRING($vField, ".(strlen($vSYMBOL)+1).") AS SIGNED)) FROM $vTableName where $vField like '$vSYMBOL%') ROWMAX, 
					(SELECT COUNT(*) FROM $vTableName where $vField like '$vSYMBOL%') AS NUMROW;";
	$bResultS = db_query($sqlS);
	$totalRows = db_num_rows($bResultS);
	if ($totalRows>0) {
...
{
	$arr_uni=array('À','Ô','Ă','Ó','Ú','É','Õ','Ê','Ũ','Ẹ','Ù' ,'È','Ì','Í','Ò','Đ','Ũ','Ý','Ố','Â','Ĩ','Ị','Á','Ú','Ạ','Ộ','Ể','Ụ','Ủ','Ử','Ệ','Ã','Ò','Ậ','Ề','Ã','Ế','Ả','Ụ','Ọ','Ễ','Ó','Ỏ','Ầ','Ẫ','Ằ','Ẩ','Ọ','Ẳ','Ẽ','Ỏ','Õ','Ấ','Ũ','Ồ','Ỗ','Ỳ','Ơ','Ặ','Ư','Ờ','Ỹ','Ắ','Ở','Ứ','Ő');
	  $UNICODE_UPPER_ACCENTS = array('À'=>'A' ,'Ô'=>'O',
  'Ă'=>'A','Ó'=>'O','Ú'=>'U','É'=>'E','Õ'=>'O','Ê'=>'E','Ű'=>'U','Ẹ'=>'E','Ù'=>'U' ,'È'=>'E','Ì'=>'I','Í'=>'I' ,'Ò'=>'O','Đ'=>'D' ,'Ũ'=>'U','Ý'=>'Y','Ő'=>'O','Â'=>'A','Ĩ'=>'I','Ị'=>'I' ,'Á'=>'A','Ứ'=>'U','Ạ'=>'A','Ộ'=>'O','Ể'=>'E','Ụ'=>'U','Ủ'=>'U','Ử'=>'U','Ệ'=>'E','Ã'=>'A','Ở'=>'O','Ậ'=>'A','Ề'=>'E','Ã'=>'A','Ế'=>'E','Ả'=>'A','Ự'=>'U','Ợ'=>'O','Ễ'=>'E','Ớ'=>'O','Ở'=>'O','Ầ'=>'A','Ẫ'=>'A','Ằ'=>'A','Ổ'=>'O','Ọ'=>'O','Ẳ'=>'A','Ẽ'=>'E','Ỏ'=>'O','Ỡ'=>'O','Ấ'=>'A','Ữ'=>'U','Ồ'=>'O','Ỗ'=>'O','Ỳ'=>'Y','Ơ'=>'O','Ặ'=>'A','Ư'=>'U','Ờ'=>'O','Ỹ'=>'Y','Ắ'=>'A','Ở'=>'O','Ứ'=>'U','Ő'=>'O');
	  // foreach ($arr_uni as $uni)
	  // {
		// $fac_contetn=str_replace($uni,$UNICODE_UPPER_ACCENTS[$uni],$fac_contetn);
	  // }
	  foreach ($arr_uni as $uni) {
		if (isset($UNICODE_UPPER_ACCENTS[$uni])) {
			$fac_contetn = str_replace($uni, $UNICODE_UPPER_ACCENTS[$uni], $fac_contetn);
		}
	}
	return $fac_contetn;
...
function InsertWithCheckExtNext($vTableName, $vField, $vSYMBOL,$vLengthCast){	
	$vReturn = "";
		$sqlS = "	SELECT 
...
function LV_BuilPopup($lvList,$vArrCot,$bResult,$objid)
	{
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				$vTemp=str_replace("@02",$lstArr[$i],$vTemp);
				$strH=$strH.$vTemp;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				$vTemp=str_replace("@02",$vrow[$vArrCot[$lstArr[$i]]],str_replace("@01",$vrow['lv001'],$lvTd));
				$strL=$strL.$vTemp;
			}
	...
	function Get_CodeSOF_OK($vStr)
	{
		$sReturn='';
		$len=strlen($vStr);
		for($i=0;$i<$len;$i++)
		{
			$vkytu=substr($vStr,$i,1);
			$ch=(ord($vkytu));
			if($vkytu=='_' || $vkytu=="-" || $vkytu==".")
			{
				$sReturn=$sReturn.$vkytu;
			}
			else
			{
				if($ch>=65 && $ch<=90)
