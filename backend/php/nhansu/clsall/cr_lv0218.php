<?php
/////////////coding cr_lv0046///////////////
class   cr_lv0218 extends lv_controler
{
...
///////////
	public $DefaultFieldList="lv001,lv016,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014";	
////////////////////GetDate
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"2","lv007"=>"2","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0","lv909"=>"0");
	var $ArrViewEnter=array("lv001"=>"-1","lv002"=>"99","lv003"=>"999","lv004"=>"88","lv005"=>"49","lv006"=>"2","lv007"=>"2","lv009"=>"100","lv010"=>"-1","lv011"=>"-1","lv012"=>"-1","lv013"=>"-1","lv014"=>"-1","lv016"=>"999");
	var $Tables=array('lv002'=>'cr_lv0004','lv003'=>'cr_lv0005','lv004'=>'cr_lv0031',"lv016"=>"hr_lv0020");
	var $TableLink=array("lv002"=>"concat(lv001,@! @!,lv002)","lv003"=>"concat(lv002,@! @!,lv003,@! @!,lv004)","lv004"=>"concat(lv001,@! @!,lv002)","lv016"=>"concat(lv001,@! @!,lv002)",);
	var $TableLinkReturn=array("lv002"=>"lv001","lv003"=>"lv001","lv04"=>"lv001","lv016"=>"lv001");
	...
	{
		$lvsql="select * from  cr_lv0046 Where lv001='$vlv001'";
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update cr_lv0046 set lv009=1,lv007=now(),lv008='$this->LV_UserID'  WHERE cr_lv0046.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0046.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$vUserID=$this->LV_UserID;
		$lvsql = "Update cr_lv0046 set lv009=0,lv007=now(),lv008='$this->LV_UserID'  WHERE cr_lv0046.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0046.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM cr_lv0046  WHERE cr_lv0046.lv001 IN ($lvarr) and lv009=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'cr_lv0046.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	//////////get view///////////////
	...
	{
		$vCount=0;
		$sqlC = "SELECT A.lv014 FROM cr_lv0005  A  inner join cr_lv0004 C on A.lv002=C.lv001    WHERE A.lv011=1 and A.lv027=1  and A.lv003='DUYETCONG' order by A.lv005 asc ";
		$bResultC = db_query($sqlC);
		$this->ThongSoLuong='';
		while($arrRowC = db_fetch_array($bResultC))
		{
			if(trim($arrRowC['lv014'])!='')
			{
				$this->ThongSoLuong=trim($arrRowC['lv014']);
				$vCount++;
			}
		}
		return $vCount;
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,C.lv009 lv909 FROM cr_lv0046 A inner join  cr_lv0005 B on A.lv002=B.lv001 inner join  cr_lv0004 C on B.lv002=C.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."cr_lv0046/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.*,C.lv009 lv909 FROM cr_lv0046 A inner join  cr_lv0005 B on A.lv002=B.lv001  inner join  cr_lv0004 C on B.lv002=C.lv001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vTax)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.* FROM cr_lv0046 A inner join  cr_lv0005 B on A.lv002=B.lv001 inner join  cr_lv0004 C on B.lv002=C.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LV_BuilListReport_Short($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTable="
		<table  align=\"center\" class=\"lvtable\" border=0>
		@#01
		@#02
		</table>
		";
		...
		$lvTdDone="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$lvTdStrong="<td style=\"background:#afead7\"  align=\"@#05\"><strong>@02</strong></td>";
		...
		if($this->DateFrom)
		{
			$vCondition=$vCondition." and A.lv006>='$this->DateFrom 00:00:00'";
		}
		if($this->DateTo)
		{
			$vCondition=$vCondition." and A.lv006<='$this->DateTo 23:59:59'";
		}
		if($this->DepID!='')
		{
			$vCondition=$vCondition." and A.lv013 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv029 in (".$this->LV_GetDep($this->DepID).")").")";
		}
		if($this->ListStaff!='')
		{
			if($this->lv013=='')
				$vCondition=$vCondition." and (  A.lv013 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv029 in (".$this->LV_GetDep($this->ListStaff).")").")";
			else
				$vCondition=$vCondition." and ( A.lv013='$this->lv013' OR A.lv008 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv029 in (".$this->LV_GetDep($this->ListStaff).")")."))";
		}
		else
		{
			if($this->lv013!='')
		...
		if($this->NVID!='')
		{
			$vCondition=$vCondition." and A.lv013='$this->NVID'";
		}
		if($this->PlanID!='')
		{
		...
		$sqlS = "SELECT A.*,AA.lv008 NDPLAN,C.lv004 NTPLAN,C.lv097 NSPLAN,C.lv009 lv909 FROM cr_lv0046  A  left join cr_lv0005  AA on A.lv003=AA.lv001  left join cr_lv0004 C on AA.lv002=C.lv001 left join cr_lv0003 D on AA.lv003=D.lv001  where 1=1  $vCondition order by A.lv015 asc ";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			/*if($vMaNguoiUng!=$vrow['lv008'] && $vMaNguoiUng!='')
			{
				$vSoLanDau++;
				$vMaNguoiUng=$vrow['lv008'];
				for($i=0;$i<count($lstArr);$i++)
				{
					switch($lstArr[$i])
					{	
						default:
							$vTemp=str_replace("@02",'',$this->Align($lvTdStrong,(int)$this->ArrView[$lstArr[$i]]));
							break;
					}
					$strL=$strL.$vTemp;
				}
			...
			*/
			//$vIdDuyet=$this->LV_CheckQuyenQuanLy($vrow['NDPLAN'],$vrow['NTPLAN'],$vrow['NSPLAN']);
			$vIdDuyet=0;
			for($i=0;$i<count($lstArr);$i++)
			{
				switch($lstArr[$i])
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv013':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0020 where lv009 in (0,1)";
				break;
			case 'lv004':
				$vsql="select lv001,concat(IF(lv009=1,'-------',''),lv001,IF(lv001='','',' - '),lv002) lv002,IF(lv001='$vSelectID',1,0) lv003 from  cr_lv0003 order by lv008 asc";
				break;
		}
		return $vsql;
	}
	private function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv003':
				$lvopt=0;
				$vsql="select A.lv001,IF(A.lv004<>'',A.lv004,B.lv002) lv002,IF(A.lv001='$vSelectID',1,0) lv003 from  cr_lv0005 A inner join cr_lv0003 B on A.lv003=B.lv001 where A.lv001='$vSelectID'";
				break;
			case 'lv004':
