<?php
/////////////coding hr_lv0021///////////////
class   hr_lv0021 extends lv_controler
{
	public $lv001=null;
	public $Dir =null;
...
////////////
	...
	{
		$lvsql="select * from  hr_lv0021 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0021 (lv001,lv002) values('$this->lv001','$this->lv002')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0021.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update hr_lv0021 set lv002='$this->lv002' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0021.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		 $lvsql = "DELETE FROM hr_lv0021  WHERE hr_lv0021.lv001 IN ($lvarr) ";//and (select count(*) from hr_lv0021 B where  B.lv002= hr_lv0021.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0021.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0021 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0021 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
    ...
    <li class=\"menusubT1\">
        <img src=\"" . $this->Dir . "../images/lvicon/config.png\" border=\"0\" />" . ($this->ArrFunc[12] ?? '') . "
        <ul id=\"submenu1-nav\">
            @#01
        </ul>
		</li>
		...
		{
			window.open('$this->Dir'+'hr_lv0021/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
	
		
		</script>
";
		$buttonLabel = $this->ArrFunc[13] ?? 'Export';
		$lvFromJs = json_encode($lvFrom);
		$lvScript="<li class=\"menuT\"> @01 </li>";
		$lvexcel = "<input class=\"lvbtdisplay\" type=\"button\" id=\"lvbuttonexcel\" value=\"" . htmlspecialchars($buttonLabel, ENT_QUOTES) . "\" onclick=\"Export($lvFromJs, 'excel')\">";
		...
		$pdfLabel = $this->ArrFunc[15] ?? 'Export PDF';
		$wordLabel = $this->ArrFunc[14] ?? 'Export Word';
		$lvFromJs = json_encode($lvFrom); // đảm bảo biến JS an toàn

		$lvpdf = "<input class=\"lvbtdisplay\" type=\"button\" id=\"lvbuttonpdf\" value=\"" . htmlspecialchars($pdfLabel, ENT_QUOTES) . "\" onclick=\"Export($lvFromJs, 'pdf')\">";
		$lvword = "<input class=\"lvbtdisplay\" type=\"button\" id=\"lvbuttonword\" value=\"" . htmlspecialchars($wordLabel, ENT_QUOTES) . "\" onclick=\"Export($lvFromJs, 'word')\">";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0021 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
