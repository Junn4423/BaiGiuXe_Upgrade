<?php
/////////////coding ts_lv0054///////////////
class   ts_lv0054 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select lv007 from ts_lv0053 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  ts_lv0054 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		if($vReturn){
			...
			{	$vWhId=$this->GetParentWH($this->lv002);
		 	...
		 	$this->InsertLogOperation($this->DateCurrent,'ts_lv0054.insert',sof_escape_string($lvsql));
	...
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv098,lv099 from ts_lv0030 where lv002='$vlv002'";
		$vReturn= db_query($lvsql);
	...
	function LV_UpdateEdit($vOldlv004,$vOldlv006)
	{
		...
		if($vReturn){
			...
			if(	db_query($vsql1))
			{
				$vWhId=$this->GetParentWH($this->lv002);
				$this->AddLotReciept($this->lv014,$this->lv003,$vWhId,$this->lv017,$this->lv018,$this->lv019,$this->lv015,$this->lv011);
				$this->InsertLogOperation($this->DateCurrent,'ts_lv0054.update',sof_escape_string($vsql1));
			}
		 $this->InsertLogOperation($this->DateCurrent,'ts_lv0054.update',sof_escape_string($lvsql));
			if($this->obj_wb_lv0006!=null)
			 	{
	...
	{
		$lvsql="select lv007 from  ts_lv0053 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ts_lv0054  WHERE ts_lv0054.lv001 IN ($lvarr) and (select lv007 from ts_lv0053 B where  B.lv001= ts_lv0054.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'ts_lv0054.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_DeleteDetail($lvarr)
	{
		$sqlD = "SELECT A.lv001 lv001,A.lv002 lv002,A.lv003 lv003,A.lv004 lv004,A.lv006 lv006,B.lv002 lvb002 FROM ts_lv0054 A inner join ts_lv0053 B on A.lv002=B.lv001 WHERE A.lv001 IN ($lvarr) and (select lv007 from ts_lv0053 B where  B.lv001= A.lv002)<=0;";
		...
		while($vrow=db_fetch_array($vresult))
		{
			...
			if(db_query($vsql))
			{
				$tsql="delete from ts_lv0054 where lv001='".$vrow['lv001']."'";
				if(db_query($tsql))
				{
					$strReturn= true;
					if($this->obj_wb_lv0006!=null)
	...
	{
		$sqlD = "SELECT A.*,B.lv002 lvb002 FROM ts_lv0054 A inner join ts_lv0053 B on A.lv002=B.lv001 WHERE B.lv007<=0;";
		$vresult=db_query($sqlD);
		$strReturn=false;
		while($vrow=db_fetch_array($vresult))
		{
	...
	{
		$sqlD = "SELECT lv002 FROM ts_lv0053 WHERE lv001='$vlv001' ";
		$vresult=db_query($sqlD);
		if($vresult){
			$vrow=db_fetch_array($vresult);
			return $vrow['lv002'];
	...
	protected function GetCondition()
	{
		...
		$strCondi=$strCondi." and A.lv002 in (select B.lv001 from ts_lv0053 B where B.lv002 in ($strwh))";		
		return $strCondi;
	}
	////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0054 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,IF(ISNULL(CC.lv016),BB.lv006,CC.lv016) lv099 FROM ts_lv0054 A inner join ts_lv0053 BB on A.lv002=BB.lv001 left join sl_lv0013 CC on BB.lv006=CC.lv001 and BB.lv005 in ('BANHANG','TRAHANG')  WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ts_lv0054/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.*,IF(ISNULL(CC.lv016),BB.lv006,CC.lv016) lv099 FROM ts_lv0054 A inner join ts_lv0053 BB on A.lv002=BB.lv001 left join sl_lv0013 CC on BB.lv006=CC.lv001 and BB.lv005 in ('BANHANG','TRAHANG')  WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";		
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	...
	function Get_String_BuilHeader()
	{
		$this->lvHeader="";
		$strTD="";
		$sqlS1 = "SELECT lv014  FROM ts_lv0054 A WHERE 1=1  ".$this->GetCondition()." ";
	...
	function LV_BuilListReportOtherSoon($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vTax)
	{
		...
		$strF="<tr><td colspan=\"2\">&nbsp;</td>";
		$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
		$sqlS = "select * from (SELECT lv003,lv014,sum(lv004) lv004,lv005 FROM ts_lv0054 A WHERE 1=1  ".$this->GetCondition()." group by lv003,lv014) MP order by MP.lv003";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
