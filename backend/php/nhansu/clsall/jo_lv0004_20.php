...
class   jo_lv0004 extends lv_controler
{
	...
	public $ArrJobs=null;
///////////
	public $DefaultFieldList="lv001,lv015,lv022,lv016,lv017,lv098,lv008,lv013,lv018,lv014,lv019,lv021,lv009,lv010,lv025";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv025"=>"26","lv098"=>"99","lv099"=>"100","lv829"=>"830");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"4","lv006"=>"0","lv007"=>"4","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"4","lv017"=>"4","lv018"=>"4","lv019"=>"4","lv020"=>"0","lv021"=>"0","lv022"=>"0","lv023"=>"0","lv024"=>"2","lv025"=>"22","lv098"=>"0","lv099"=>"0");	
	...
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0004 WHERE lv021=0 and lv022 not in (0,1,2,3,13) ".$this->GetCondition();
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0004 WHERE 1=1 and lv022 not in (0,1,2,3,13) ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
';
		...
		';
		$sqlS = "SELECT *,lv015 lv829,lv015 lv099,IF(SUBSTR(lv003,1,3)='1/2',DATEDIFF(lv017,lv016)+0.5,IF(TIMEDIFF(lv017,lv016)<'24:00:00' AND DATEDIFF(lv017,lv016)<1,1,DATEDIFF(lv017,lv016)+1)) lv098 FROM jo_lv0004  WHERE 1=1  and lv022 not in (0,1,2,3,13) ".$this->GetCondition()."";
		$vorder=$curRow;
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,lv015 lv099,IF(SUBSTR(lv003,1,3)='1/2',DATEDIFF(lv017,lv016)+0.5,IF(TIMEDIFF(lv017,lv016)<'24:00:00' AND DATEDIFF(lv017,lv016)<1,1,DATEDIFF(lv017,lv016)+1)) lv098,ADDTIME(lv025,'00:05:00') CheckNgay FROM jo_lv0004 WHERE 1=1  and lv022 not in (0,1,2,3,13) ".$this->GetCondition()." order by lv016 desc LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				else if ($lstArr[$i]=='lv098')
				{
					...
					$vTemp=str_replace("@02",str_replace("@02",$vSoGio,str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
	...
	function LV_BuilList7Day($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,lv015 lv099,IF(SUBSTR(lv003,1,3)='1/2',DATEDIFF(lv017,lv016)+0.5,IF(TIMEDIFF(lv017,lv016)<'24:00:00' AND DATEDIFF(lv017,lv016)<1,1,DATEDIFF(lv017,lv016)+1)) lv098 FROM jo_lv0004 WHERE 1=1  and lv022 not in (0,1,2,3,13) and lv016>=concat(curdate(),' 00:00:00') order by lv016 asc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT *,lv015 lv829,lv015 lv099,IF(SUBSTR(lv003,1,3)='1/2',DATEDIFF(lv017,lv016)+0.5,IF(TIMEDIFF(lv017,lv016)<'24:00:00' AND DATEDIFF(lv017,lv016)<1,1,DATEDIFF(lv017,lv016)+1)) lv098 FROM jo_lv0004 WHERE 1=1 and lv022 not in (0,1,2,3,13)  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0002 where lv001<>'1/4KP'";
				break;
			case 'lv004':
			...
			case 'lv021':
				...
				break;
			case 'lv022':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  jo_lv0100 where lv001 not in (0,1,2,3,13)  order by lv009";
				break;
		}
		return $vsql;
	}
