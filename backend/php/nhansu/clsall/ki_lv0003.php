<?php
/////////////coding ki_lv0003///////////////
class   ki_lv0003 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"2","lv006"=>"0","lv007"=>"0");	
	...
	{
		$lvsql="select * from  ki_lv0003 Where lv001='$vlv001'";
	...
	{
		$vmonth=(int)$vmonth;
		$lvsql="select * from  ki_lv0003 Where Month(lv004)='$vmonth' and Year(lv004)='$vyear'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  ki_lv0003 Where lv008=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into ki_lv0003 (lv001,lv002,lv003,lv004,lv005) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->InsertLogOperation($this->DateCurrent,'ki_lv0003.insert',sof_escape_string($lvsql));
			$this->LV_InsertChild($this->lv001);
		}
		return $vReturn;
	}	
	function LV_InsertChild($vParentID)
	{
		$lvsql="insert into tc_lv0062 (lv002,lv003,lv004) select '$vParentID',A.lv001,0 lv004 from hr_lv0020 A where A.lv009 not in (2,3)";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->InsertLogOperation($this->DateCurrent,'tc_lv0062.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_CheckUpdateRate($lvarr)
	{
		$lvsql="select * from  ki_lv0003 Where lv008<=1 and lv001 in ($lvarr)";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$this->LV_InsertChildMore($vrow['lv001'],$vrow['lv004']);
		}
	}
	function LV_InsertChildMore($vParentID,$vDateEnd)
	{
		$lvsql="insert into tc_lv0062 (lv002,lv003,lv004) select '$vParentID',A.lv001,0 lv004 from hr_lv0020 A where A.lv030<='$vDateEnd' and A.lv009 not in (2,3) and concat('$vParentID',A.lv001) not in ( select concat(lv002,lv003) from tc_lv0062 where lv002='$vParentID')";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->InsertLogOperation($this->DateCurrent,'tc_lv0062.insert',sof_escape_string($lvsql));
	...
	function LV_Update()
	{
		...
		$lvsql="Update ki_lv0003 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004' where  lv001='$this->lv001' and lv007<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0003.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ki_lv0003  WHERE ki_lv0003.lv001 IN ($lvarr) and ki_lv0003.lv007<=0   ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0003.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update ki_lv0003 set lv006='".getInfor($_SESSION['ERPSOFV2RUserID'],2)."',lv007=IF(lv007<2,lv007+1,2) WHERE ki_lv0003.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0003.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$vUserID=getInfor($_SESSION['ERPSOFV2RUserID'], 2);
		$lvsql = "Update ki_lv0003 set lv006='".getInfor($_SESSION['ERPSOFV2RUserID'],2)."',lv007=IF(lv007>0,lv007-1,0) WHERE ki_lv0003.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0003.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ki_lv0003 WHERE 1=1 ".$this->GetCondition();
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join ki_lv0003 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004 from ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 )) PM ";	
	...
	function LV_GetContractMoneyDate($vContractID,$StartDate,$EndDate)
	{
		...
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join ki_lv0003 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 $condition1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004 from ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 $condition2)) PM ";	
		$vresult1=db_query($lvsql);
		$vrow1 = db_fetch_array ($vresult1);
		return $vrow['convertmoney']+$vrow['money']+$vrow1['convertmoney']+$vrow1['money'];
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTable="
		<div id=\"func_id\" style='position:position;background:#f2f2f2'><div style=\"float:left\">".$this->TabFunction($lvFrom,$lvList,$maxRows)."</div><div style=\"float:right\">".$this->ListFieldSave($lvFrom,$lvList,$maxRows,$lvOrderList,$lvSortNum)."</div><div style='float:right'>&nbsp;&nbsp;&nbsp;</div><div style='float:right'>".$this->ListFieldExport($lvFrom,$lvList,$maxRows)."</div></div><div style='height:35px'></div><table  align=\"center\" class=\"lvtable\"><!--<tr ><td colspan=\"".(2+count($lstArr))."\" class=\"lvTTable\">".$this->ArrPush[0]."</td></tr>-->
		@#01
		...
		$sqlS = "SELECT * FROM ki_lv0003 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			{
				case 0:
					$strL=str_replace("@#04","",$strL);
					break;
				case 1:
					$strL=str_replace("@#04","lvlineapproval",$strL);
					break;
				case 2:
					$strL=str_replace("@#04","lvlineapproval_red",$strL);
					break;
			}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('ki_lv0003/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM ki_lv0003 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportStartEndDate($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvEmployeeID,$lvStartDate,$lvEndDate)
	{
		...
		$sqlS = "SELECT * FROM ki_lv0003 WHERE 1=1 $condition ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
