...
/////////////coding hr_lv0214///////////////
class   hr_lv0214 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0");
	var $ArrViewEnter=array("lv003"=>"89","lv005"=>"33","lv006"=>"-1","lv007"=>"-1");
	...
	{
		$vsql="select * from  hr_lv0214 where lv001='$vCateID' and (lv003='' or lv001=lv001)";
	...
	{
		$vsql="select * from  hr_lv0214 where lv003='$vCateID'";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
				$str_return=$str_return.",'".$vrow['lv001']."'".$this->LV_CheckChildCate($vrow['lv001']);
	...
	{
		$lvsql="select * from  hr_lv0214 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0214 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0214.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$lvsql="Update hr_lv0214 set  lv001='$this->lv001_',lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->LV_UserID',lv007=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0214.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0214  WHERE hr_lv0214.lv001 IN ($lvarr)";// and (select count(*) from hr_lv0214 B where  B.lv002= hr_lv0214.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0214.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0214 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0214 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{	
					case 'lv005':
						if($this->GetEdit()==1)
						{	
							$lvTdTextBox="<td align=center><input class='txtenterquick' type=\"checkbox\" value=\"1\" ".(($vrow['lv005']==1)?'checked="true"':'')." @03 onclick=\"UpdateTextCheck(this,'".$vrow['lv001']."',5)\" style=\"width:35px;text-align:center;\" tabindex=\"2\" maxlength=\"32\"   onKeyPress=\"return CheckKey(event,7)\"/></td>";
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."hr_lv0214/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0214 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function getvaluelink($vFile,$vSelectID)
	{
		...
		if (isset($this->ArrGetValueLink[$vFile][$vSelectID][0]) && $this->ArrGetValueLink[$vFile][$vSelectID][0]) {
