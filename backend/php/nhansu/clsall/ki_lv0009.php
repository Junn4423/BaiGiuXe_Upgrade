<?php
/////////////coding ki_lv0005///////////////
class   ki_lv0009 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv099,lv003,lv004,lv005,lv006,lv007,lv100";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	public $ArrPoint=null;
	protected $objhelp='ki_lv0005';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv099"=>"100","lv100"=>"101");	
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv099"=>"0","lv100"=>"10");	
	...
	{
		$lvsql="select * from  ki_lv0005 Where lv001='$vlv001'";
	...
	function LV_Insert_No()
	{
		if($this->isApr==0) return false;
		$lvsql="insert into ki_lv0005 (lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0005.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Insert()
	{
		...
		$this->lv040 = ($this->lv040!="")?recoverdate(($this->lv040), $this->lang):$this->DateDefault;
		$this->lv083 = ($this->lv083!="")?recoverdate(($this->lv083), $this->lang):$this->DateDefault;
		$this->lv301 = ($this->lv301!="")?recoverdate(($this->lv301), $this->lang):$this->DateDefault;
		$this->lv302 = ($this->lv302!="")?recoverdate(($this->lv302), $this->lang):$this->DateDefault;
		$this->lv321 = ($this->lv321!="")?recoverdate(($this->lv321), $this->lang):$this->DateDefault;
		$this->lv322 = ($this->lv322!="")?recoverdate(($this->lv322), $this->lang):$this->DateDefault;
		$this->lv341 = ($this->lv342!="")?recoverdate(($this->lv341), $this->lang):$this->DateDefault;
		$this->lv342 = ($this->lv343!="")?recoverdate(($this->lv342), $this->lang):$this->DateDefault;
		$lvsql="insert into ki_lv0005 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0005.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$lvsql="Update ki_lv0005 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ki_lv0005  WHERE ki_lv0005.lv001 IN ($lvarr) and  ki_lv0005.lv007<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0005 set lv006='".$this->LV_UserID."',lv007=IF(lv007=1,1,lv007+1)  WHERE ki_lv0005.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0005 set lv006='".$this->LV_UserID."',lv007=IF(lv007=0,0,lv007-1) WHERE ki_lv0005.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_CalculateAuto($vObj)
	{
		if($this->isApr==0) return false;
		$vCalID=$vObj->lv001;
		$vDateEnd=$vObj->lv005;
		$lvsql="insert into ki_lv0005 (lv002,lv003,lv004) 
		select MP.StaffID,MP.CalID,IF(TRIM(MP.KPIStaff)='' or ISNULL(MP.KPIStaff),MP.KPIDept,MP.KPIStaff) KPI from (
		select A.lv001 StaffID,'$vCalID' CalID,B.lv099 KPIDept,A.lv299 KPIStaff from hr_lv0020 A inner join hr_lv0002 B on A.lv029=B.lv001 where A.lv030<='$vDateEnd' and A.lv009 not in (2,3) and concat(A.lv001,'$vCalID') not in ( select concat(lv002,lv003) from ki_lv0005 where lv003='$vCalID') 
		) MP Where MP.KPIStaff<>'' or MP.KPIDept<>'' ";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->InsertLogOperation($this->DateCurrent,'ki_lv0005.insert',sof_escape_string($lvsql));
			$this->LV_AutoChildKPI($vCalID);
		}
		return $vReturn;
	}
	function LV_GetKPIDep($vStaffID)
	{
		$vStrReturn='';
		$lvsql="select lv001 from  hr_lv0002 Where lv100='$vStaffID'";
		$vresult=db_query($lvsql);
		...
		if($vStrReturn=='') $vStrReturn="''";
		return $vStrReturn;
	}
	function LV_AutoChildKPI($vCalID)
	{
		if($this->isApr==0) return false;
		$lvsql="select * from  ki_lv0005 Where lv003='$vCalID' and lv007=0";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vParentID=$vrow['lv001'];
			$vKPIID=$vrow['lv004'];
			$lvsql="insert into ki_lv0006(lv002,lv003,lv004,lv015) select '$vParentID' ParentID,lv003 TargetKPI,lv005 MacDinh,lv004 ParaRate from ki_lv0004 where lv002='$vKPIID' and lv003 not in (select B.lv003 from ki_lv0006 B where B.lv002='$vParentID')";
			$vReturn1= db_query($lvsql);
			if($vReturn1)
			{	
				$this->InsertLogOperation($this->DateCurrent,'ki_lv0006.insert',sof_escape_string($lvsql));
			}
		}
	}
	...
	protected function GetCondition()
	{
		...
		{	
			...
			foreach($vArrName as $vName)
			{
				...
				{
				if($strCondi1=="")	
					$strCondi1= " AND ( concat(lv004,' ',lv003,' ',lv002)  like '%$vName%'";
				else
					$strCondi1=$strCondi1." OR concat(lv004,' ',lv003,' ',lv002)  like '%$vName%'";		
				}
			}
			$strCondi1=$strCondi1.")";
			$strCondi=$strCondi." and (lv002 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where 1=1 $strCondi1")."))";
			
		}
		if($this->lv029!="")  $strCondi=$strCondi." and (lv002 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv029 in (".$this->LV_GetDep($this->lv029).")")."))";
		if($this->lv029_!="")  $strCondi=$strCondi." and (lv002 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv029 in (".$this->LV_GetDep($this->lv029_).")")."))";
		if($this->lv010!="")  $strCondi=$strCondi." and (lv002 in (".$this->GetMultiValue("select DD.lv001 from hr_lv0020 DD where DD.lv010 = '".$this->lv010."'")."))";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ki_lv0005 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$this->LV_LoadArrPointKPI($this->lv003);
		...
		$lvImg="<td  ><img name=\"imgView\" border=\"0\" style=\"border-color:#CCCCCC\" title=\"\" alt=\"Image\" width=\"96px\" height=\"128px\" src=\"$this->Dir../images/interview/@01/@02\" /></td>";
		$sqlS = "SELECT MP.*,MP.lv002 lv099,0 lv100 FROM (SELECT *,(select count(*) from ki_lv0006 B where B.lv002=ki_lv0005.lv001) numR FROM ki_lv0005 WHERE 1=1 ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows) MP";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv100')
				{
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_CalPointKPI($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));

				}
				elseif($lstArr[$i]=='lv045')
				{
	...
	//////////////////////Buil list////////////////////
	function LV_BuilListReportView($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,lv002 lv099,0 lv100 FROM ki_lv0005 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv100')
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_CalPointKPI($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				elseif($lstArr[$i]=='lv045')
				{
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$vrow['lv001']),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ki_lv0009/?lang=".$this->lang."&childfunc='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
		</script>
";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTd="<td align=@#05>@02</td>";
		$lvImg="<td  ><img name=\"imgView\" border=\"0\" style=\"border-color:#CCCCCC\" title=\"\" alt=\"Image\" width=\"96px\" height=\"128px\" src=\"$this->Dir../../images/interview/@01/@02\" /></td>";
		$sqlS = "SELECT * FROM ki_lv0005 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0020 where lv009 not in (2,3)";
				break;
			case 'lv003':
