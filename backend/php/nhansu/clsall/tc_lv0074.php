<?php
/////////////coding tc_lv0074///////////////
class   tc_lv0074 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function __construct($vCheckAdmin,$vUserID,$vright)
	{
		...
		$this->ActiveRating=Array(1=>0,2=>1,3=>0,4=>0);
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0074.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_UpdateCode($vlv001,$vRate,$vitri,$RateID)
	{
		...
		if($this->isApr==1)
		{
			...
			{
				case 1:
					$lvsql="Update tc_lv0062 set lv005='$vRate' where  lv001='$vlv001';";
					break;
				case 2:
					$vPercen=(float)$this->ArrRate[$vRate];
					$lvsql="Update tc_lv0062 set lv006='$vRate',lv004=IF(ISNULL(lv005),'$vPercen',lv004) where  lv001='$vlv001';";
					break;
				case 3:
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0074.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0074.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0074.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0074/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."&dayfrom=".$this->dayfrom."&dayto=".$this->dayto."&isTime=".$this->isTime."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0074/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	...
	function Get_String_DateFromTo()
	{
		...
		for($i=1;$i<=4;$i++)
		{
			...
			{
				if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf" || $this->ActiveRating[$i]==0)
				{
					if($_GET['day']=='')
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			foreach($this->ArDays as $vDays)
			{
				...
				$vDayCss=(int)getday($vDays);
					...
					{
						case 0:
							if($this->isEdit==1 && $this->ActiveRating[$vvtL]==1)
							{
							...
							{
								$lvstrgt=(trim($vLoai)=="")?"&nbsp;":$vLoai;
								$lvstrgt1=(trim($vNote)=="")?"&nbsp;":$vNote;
