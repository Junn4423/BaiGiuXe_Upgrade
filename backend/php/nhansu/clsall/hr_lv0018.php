...
////////////
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0018 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->LV_UserID',now())";
	...
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update hr_lv0018 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->LV_UserID',lv007=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				{			
					...
					case 0:		
						...
						$viewType = (int)($this->ArrView[$lstArr[$i]] ?? 0);
						$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, $viewType));
						//$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, (int)($this->ArrView[$lstArr[$i]] ?? 0)));
						break;
					default:
						$vTempEnter="<td>&nbsp;</td>";
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{	
					case 'lv003':
							...
							$vlv003=$vrow['lv003'];
							$lvTdTextBox="<input class='txtenterquick' type=\"textbox\" value=\"".$vlv003."\" @03 onfocus=\"if(this.value=='') this.value='".$vlv093."'\" onblur=\"UpdateText(this,'".$vrow['lv001']."',3)\" style=\"min-width:60px;width:100%;text-align:center\" tabindex=\"2\"  maxlength=\"100\" />";
							...
							$key = $lstArr[$i];
							$viewType = (int)($this->ArrView[$key] ?? 0); // dùng 0 nếu không có cấu hình
							$vTemp = str_replace("@02", $lvTdTextBox, $this->Align($lvTd, $viewType));
						break;
					default:
						...
						$key = $lstArr[$i];
						$viewType = (int)($this->ArrView[$key] ?? 0);
						$value = $vrow[$key] ?? '';

						$vTemp = str_replace(
							"@02",
							$this->getvaluelink($key, $this->FormatView($value, $viewType)),
