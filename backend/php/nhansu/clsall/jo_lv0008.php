...
////////////
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			if($vIsTrue==true && (($vrow['lv003']<>'CT' && $vrow['lv003']<>'QCC' && ($vNghiLienKe+$vNgayNghi)>3) || (($vrow['lv003']=='KL' || $vrow['lv003']=='1/2KL') && ($vNgayNghi+$vSoPhepKLDaDung)>7 )))
			{
				if(($vrow['lv003']=='KL' || $vrow['lv003']=='1/2KL') && ($vNgayNghi+$vSoPhepKLDaDung)>7 )
					$lvsql = "Update jo_lv0004 set lv004='1',lv010='Phép KL đã dùng:".($vNgayNghi+$vSoPhepKLDaDung)." ngày>=7 ngày.',lv013='$this->LV_UserID',lv018=now()  WHERE jo_lv0004.lv001 ='".$vrow['lv001']."' ";
				else
					$lvsql = "Update jo_lv0004 set lv004='1',lv010='Phép liền kề lớn hơn 3 ngày',lv013='$this->LV_UserID',lv018=now()  WHERE jo_lv0004.lv001 ='".$vrow['lv001']."' ";
				$vReturn= db_query($lvsql);
				if($vReturn){
			...
			{
				$lvsql = "Update jo_lv0004 set lv004='1',lv021='1',lv013='$this->LV_UserID',lv018=now()  WHERE jo_lv0004.lv001 ='".$vrow['lv001']."' ";
				$vReturn= db_query($lvsql);
				if($vReturn){
					$this->InsertLogOperation($this->DateCurrent,'jo_lv0004.approval',sof_escape_string($lvsql));
				}
	...
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update jo_lv0004 set lv004='2',lv021='2',lv013='$this->LV_UserID',lv018=now()  WHERE jo_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
	...
	protected function GetCondition()
	{
		...
		$strCondi=$strCondi." and lv004=0 and lv021=0";
		return $strCondi;
	}
		////////////////Count///////////////////////////
	function GetCount()
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,(DATEDIFF(lv017,lv016)+IF(LEFT(lv003,3)='1/2',0.5,1)) lv098,TIMEDIFF(lv017,lv016) lv098_1 FROM jo_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT *,lv015 lv829 FROM jo_lv0004 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
