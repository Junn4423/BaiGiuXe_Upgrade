<?php
/////////////coding tc_lv0072///////////////
class   tc_lv0072 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  tc_lv0062 Where lv001='$vlv001'";
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0072.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_UpdateCode($vlv001,$vRate,$vitri,$RateID)
	{
		//$this->LV_LoadID($vlv001);
		if(!$this->LV_CheckLocked($RateID)) return false;
		if($this->isApr==1)
		{
			switch($vitri)
			{
				case 1:
					$vPercen=(float)$this->ArrRate[$vRate];
					$lvsql="Update tc_lv0062 set lv005='$vRate',lv004='$vPercen' where  lv001='$vlv001';";
					break;
				case 2:
					$lvsql="Update tc_lv0062 set lv006='$vRate' where  lv001='$vlv001';";
					break;
				case 3:
					$lvsql="Update tc_lv0062 set lv007='$vRate' where  lv001='$vlv001';";
					break;
				case 4:
					$lvsql="Update tc_lv0062 set lv008='$vRate' where  lv001='$vlv001';";
					break;
				
			}
			echo $lvsql;
			db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'tc_lv0062.update',sof_escape_string($lvsql1.";".$lvsql));
			
		}
		
		return ;
	}
	function LV_UpdateCodeTimeCard($vlv001,$vRate,$vitri,$RateID)
	{
		if(!$this->LV_CheckLocked($RateID)) return false;
		if($this->isApr==1)
		{
			...
			{
				case 1:
					$lvsql="Update tc_lv0062 set lv009='$vRate' where  lv001='$vlv001';";
					break;
				case 2:
					$lvsql="Update tc_lv0062 set lv010='$vRate' where  lv001='$vlv001';";
					break;
				case 3:
					$lvsql="Update tc_lv0062 set lv011='$vRate' where  lv001='$vlv001';";
					break;
				case 4:
					$lvsql="Update tc_lv0062 set lv012='$vRate' where  lv001='$vlv001';";
					break;
			}
			db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'tc_lv0062.update',sof_escape_string($lvsql1.";".$lvsql));
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0072.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0072.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0072.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_CheckLocked($RateID)
	{
		 echo $lvsql="select lv008 from  tc_lv0061 Where lv001='$RateID'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0072/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."&dayfrom=".$this->dayfrom."&dayto=".$this->dayto."&isTime=".$this->isTime."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0072/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	...
	function Get_String_DateFromTo()
	{
		...
		$vArrTitle=Array('1'=>'Giám đốc','2'=>'Nhân sự','3'=>'Quản lý','4'=>'Tự thân');
		for($i=1;$i<=4;$i++)
		{
			
			$this->ArDays[$i]=$datecur;
			...
			if($this->isTime==1)
			{
				...
				{
					if($_GET['day']=='')
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="3"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></td>';
					else
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center"  colspan="3"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></td>';						
				}
				else
					$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="3"><div style="float:left;text-align:left"><select style="width:59px;" id="hdcheck_'.((int)getday($datecur)).'" onchange="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)">'.$this->CreateSelectArr($vArr,'').'<select></div><div style="float:left"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></div></td>';
			...
			else
			{
				...
				{
					if($_GET['day']=='')
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="2"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></td>';
					else
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center"  colspan="2"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></td>';						
				}
				else
					$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="2"><div style="float:left;text-align:left"><select style="width:59px" id="hdcheck_'.((int)getday($datecur)).'" onchange="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)">'.$this->CreateSelectArr($vArr,'').'<select></div><div style="float:left"><b><font color="'.$color.'">'.$vArrTitle[(int)getday($datecur)].'</font></b></div></td>';
	...
	function LV_GetTimeCard1()
	{
		$vArr=Array();
		$vsql="select lv003 lv001,lv003 lv002 from  tc_lv0025 where  lv002='$this->CalID' order by lv001";
		$bResult=db_query($vsql);
		$i=1;
		$vArr[0]['lv001']='';
		$vArr[0]['lv002']='Chọn';
	...
	{		
		...
		";
		$vcondition=" AND  B.lv001='$this->RateID'";
		if($this->lv028<>"")
		{
			$lsguser=$this->LV_GetDep($this->lv028);
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,A.lv003 NVID,C.lv029 lv001,A.lv001 TimeCardId  FROM tc_lv0062 A inner join tc_lv0061 B on A.lv002=B.lv001 inner join hr_lv0020 C on C.lv001=A.lv003 WHERE 1=1 $vcondition order by C.lv029 ASC,A.lv004 ASC";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$this->ArrEmp[$lvvt][8]=$vrow['TimeCardId'];
			$strL="";
			$vorder++;
			$lvstrgt="";
			$vvtL=0;
			
			foreach($this->ArDays as $vDays)
			{
				$vvtL++;
				switch($vvtL)
				{
				...
				$vDayCss=(int)getday($vDays);
					switch ($lvgt)
					{
						case 0:
							if($this->isEdit==1)
							{
								$lvstrgt='<select title="'.$this->ArrEmp[$lvvt][1]." ".$this->FormatView($vrow['lv004'],2).'" lang="runchangetime('.$vvtL.',\''.$vrow['TimeCardId'].'\',\'@!99\',\''.$this->RateID.'\')" class="lineday_'.$vDayCss.'" style="width:49px;font-size:10px;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="timecard_'.$vrow['TimeCardId'].'" onblur="runchangetime('.$vvtL.',\''.$vrow['TimeCardId'].'\',this.value,\''.$this->RateID.'\')">'.$this->CreateSelectArr($vArr,$vLoai).'</select>';	
								$lvstrgt1='<input type="textbox" title="'.$this->ArrEmp[$lvvt][1]." ".$this->FormatView($vrow['lv004'],2).'" lang="runchangetime2('.$vvtL.',\''.$vrow['TimeCardId'].'\',\'@!99\',\''.$this->RateID.'\')" class="lineday2_'.$vDayCss.'" style="width:120px;font-size:10px;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="timecard2_'.$vrow['TimeCardId'].'" onblur="runchangetime2('.$vvtL.',\''.$vrow['TimeCardId'].'\',this.value,\''.$this->RateID.'\')" value="'.$vNote.'"/>';	
							}
							else
							{
						...
						case 1:
							if($this->isEdit==1)
							{
								$lvstrgt='<select lang="runchangetime('.$vvtL.',\''.$vrow['TimeCardId'].'\',\'@!99\')" class="lineday_'.$vDayCss.'"  style="width:49px;font-size:10px;background-color:#@@99" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime($vvtL,\''.$vrow['TimeCardId'].'\',this.value,\''.$this->RateID.'\')">'.$this->CreateSelectArr($vArr,$vLoai).'<select>';
								$lvstrgt1='<input type="textbox" lang="runchangetime('.$vvtL.',\''.$vrow['TimeCardId'].'\',\'@!99\',\''.$this->ArrEmp[$lvvt][1].'\')" class="lineday2_'.$vDayCss.'"  style="width:49px;font-size:10px;background-color:#@@99" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime($vvtL,\''.$vrow['TimeCardId'].'\',this.value,\''.$this->RateID.'\')" value="'.$vNote.'"/>';
							}
							else
							{
				...
				$this->ArrEmp[$lvvt][6]=str_replace("<!--".$vDays."-->",$lvstrgt,$this->ArrEmp[$lvvt][6]);
				$this->ArrEmp[$lvvt][6]=str_replace("<!--:".$vDays."-->",$lvstrgt1,$this->ArrEmp[$lvvt][6]);
			}
...
/*									
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv007':
				$vsql="select lv003 lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0025 where  lv002='$this->CalID' order by lv001";
				break;
			case 'lv009':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv007':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0025 where lv001='$vSelectID' and lv002='$this->CalID'";
				$lvopt=4;
				break;
			case 'lv009':
