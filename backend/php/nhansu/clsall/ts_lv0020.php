<?php
/////////////coding ts_lv0020///////////////
class   ts_lv0020 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  ts_lv0020 Where concat(lv001,'-',lv002,'-',lv003)='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into ts_lv0020 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
	...
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		//$this->lv007 = ($this->lv007!="")?recoverdate(($this->lv007), $this->lang):$this->DateDefault;
		 $lvsql="Update ts_lv0020 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv008='$this->lv008' where  concat(lv001,'-',lv002,'-',lv003)='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0020.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ts_lv0020  WHERE concat(lv001,'-',lv002,'-',lv003) IN ($lvarr)";// and (select count(*) from ts_lv0020 B where  B.lv002= ts_lv0020.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0020.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function LV_Get_slt_nolot_all($vItemId)
	{
		...
		$vsql="select sum(lv004) sumslnhap from ts_lv0009 A where  A.lv003='$vItemId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(lv004) sumslxuat from ts_lv0011 A where A.lv003='$vItemId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	...
	function LV_Get_slt_lot_once($vLotId,$vItemId,$vWhID)
	{
		...
		$vsql="select sum(A.lv004) sumslnhap from ts_lv0009 A inner join ts_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId' and A.lv014='$vLotId'  and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId' and A.lv014='$vLotId'  and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_slt_more_kholinhkien($vItemId,$vWhID)
	{
		$vsql="select sum(A.lv004) sumslnhap from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId'   and  B.lv002 in ($vWhID)";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId'   and  B.lv002 in ($vWhID)";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_slt_more($vItemId,$vWhID)
	{
		$vsql="select sum(A.lv004) sumslnhap from ts_lv0009 A inner join ts_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId'   and  B.lv002 in ($vWhID)";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId'   and  B.lv002 in ($vWhID)";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_nhap_more_pbhall($vItemId,$vWhID,$vPBHID)
	{
		$vsql="select sum(A.lv004) sumslnhap from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId'   and  B.lv002 in ($vWhID) and B.lv115='$vPBHID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
	...
	function LV_Get_nhap_more_pbh($vItemId,$vWhID,$vPBHID,$vRef='LANDAU')
	{
		$vsql="select sum(A.lv004) sumslnhap from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId'   and  B.lv002 in ($vWhID) and B.lv115='$vPBHID' and B.lv116='$vRef'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		if($slnhap<0) $slnhap=0;
	...
	function LV_Get_xuat_more_pbh($vItemId,$vWhID,$vPBHID,$vRef='LANDAU')
	{
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId'   and  B.lv002 in ($vWhID)  and B.lv115='$vPBHID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		if($slxuat<0) $slxuat=0;
		return $slxuat;
	}
	function LV_Get_xuat_more_pbh_noibo($vItemId,$vWhID,$vPBHID,$vRef='')
	{
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId'   and  B.lv002 in ($vWhID)  and B.lv115='$vPBHID' and B.lv010='NOIBO' and B.lv099='$vRef'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		if($slxuat<0) $slxuat=0;
	...
	function LV_Get_xuat_more_khong_noibo($vItemId,$vWhID)
	{
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId'   and  B.lv002 in ($vWhID)  and B.lv010!='NOIBO'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		if($slxuat<0) $slxuat=0;
	...
	function LV_Get_slt_nolot_once($vItemId,$vWhID)
	{
		...
		$vsql="select sum(A.lv004) sumslnhap from ts_lv0009 A inner join ts_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId' and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId' and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	...
	{
		$vsql="select $vField  from ts_lv0020  where lv002='$vItemId' and lv001='$vLotId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow[$vField];
		return $slnhap;
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0020 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0020 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'ts_lv0020/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ts_lv0020 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
