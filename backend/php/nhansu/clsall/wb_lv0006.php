...
////////////
	...
	function LV_get_slt_lot($vItemId,$vWhId)
	{
		...
		if($sophieu=="") $sophieu="''";
		$vsql="select sum(lv004) sumslnhap from ts_lv0009 A where A.lv002 in ($sophieu) and A.lv003='$vItemId' ";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$sophieu=$this->LV_get_sophieu($vWhId,1,'lv001');
		$vsql="select sum(lv004) sumslxuat from ts_lv0011 A where A.lv002 in ($sophieu) and A.lv003='$vItemId' ";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
