<?php
/////////////coding tc_lv0071///////////////
class   tc_lv0071 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0071.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_UpdateCode($vlv001,$vlv021)
	{
		...
		if($this->isApr==1)
			...
			$lvsql="Update tc_lv0011 set lv021='$vlv021' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";
		$vReturn= db_query($lvsql);
		if($vReturn) {			
	...
	function LV_UpdateCodeTimeCard($vlv001,$vlv007)
	{
		$this->LV_LoadID($vlv001);
		if(!$this->LV_CheckLocked($this->lv004)) return false;
		if($this->isApr==1)
			$lvsql1="Update tc_lv0011 set lv007='$vlv007' where  lv001='$vlv001';";
		else
			$lvsql1="Update tc_lv0011 set lv007='$vlv007' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";
		...
		if($vReturn) {			
			switch($vlv007)
			{
				case 'CL':
					if($this->isApr==1)
						$lvsql="Update tc_lv0011 set lv016='08:00' where  lv001='$vlv001';";
					else
						$lvsql="Update tc_lv0011 set lv016='08:00' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";

				break;
				case '0.5CL':
					if($this->isApr==1)
						$lvsql="Update tc_lv0011 set lv016='04:00' where  lv001='$vlv001';";
					else
						$lvsql="Update tc_lv0011 set lv016='04:00' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";

				break;
				case 'P':
					if($this->isApr==1)
						$lvsql="Update tc_lv0011 set lv005='08:00' where  lv001='$vlv001';";
					else
						$lvsql="Update tc_lv0011 set lv005='08:00' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";
				break;
				case '0.5P':
					if($this->isApr==1)
						$lvsql="Update tc_lv0011 set lv005='04:00' where  lv001='$vlv001';";
					else
						$lvsql="Update tc_lv0011 set lv005='04:00' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";

				break;
				default:
					if($this->isApr==1)
						$lvsql="Update tc_lv0011 set lv005='00:00',lv016='00:00' where  lv001='$vlv001';";
					else
						$lvsql="Update tc_lv0011 set lv005='00:00',lv016='00:00' where  lv001='$vlv001' and lv004>=concat(CurDate(),' 00:00:00');";

					break;
			}
			db_query($lvsql);
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0044.update',sof_escape_string($lvsql1.";".$lvsql));
		}
		return $vReturn;
	}
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0071.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0071.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0071.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0071/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."&dayfrom=".$this->dayfrom."&dayto=".$this->dayto."&isTime=".$this->isTime."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0071/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	...
	function Get_String_DateFromTo()
	{
		...
		for($i=1;$i<=count($vArrDay);$i++)
		{
			...
			if($this->isTime==1)
			{
				...
				{
					if($_GET['day']=='')
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="3"><b><font color="'.$color.'">'.getday($datecur).'</font></b></td>';
					else
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center"  colspan="3"><b><font color="'.$color.'">'.$this->FormatView($datecur,2).'</font></b></td>';						
				}
				else
					$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="3"><b><font color="'.$color.'">'.getday($datecur).'</font></b><br/><input type="checkbox" id="hdcheck_'.((int)getday($datecur)).'" onclick="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)"/></td>';
				$strTD=$strTD.'<td align="center">'."<!--".str_replace("/","-",$datecur)."-->".'</td><td align="center">'."<!--:".str_replace("/","-",$datecur)."-->".'</td><td align="center">'."<!--&".str_replace("/","-",$datecur)."-->".'</td>';
			}
			else
			{
				if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
				{
					...
					$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center" colspan="2"><b><font color="'.$color.'">'.getday($datecur).'</font></b><br/><input type="checkbox" id="hdcheck_'.((int)getday($datecur)).'" onclick="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)"/></td>';
				$strTD=$strTD.'<td align="center">'."<!--".str_replace("/","-",$datecur)."-->".'</td><td align="center">'."<!--:".str_replace("/","-",$datecur)."-->".'</td>';
			}
	...
	function Get_Arr_Employees()
	{
		...
		if($this->lv030!="")  $strCondi=$strCondi." AND DD.lv001 in ('".$this->lv030."')";
		if($this->lvSort==0)
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv009 Petrol,EE.lv001 MonthID,DD.lv044 from hr_lv0020 DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv029";
		else 
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv009 Petrol,EE.lv001 MonthID,DD.lv044 from hr_lv0020 DD DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv001";
		$vresult=db_query($lvsql);	
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
	...
	{
		$vArr=Array();
		$vsql="select * from sl_lv0013 where 1=1  and lv011='1' order by lv001";
		$bResult=db_query($vsql);
		$i=0;
		while ($vrow = db_fetch_array ($bResult)){
	...
	{		
		...
		";
		$vcondition=" AND  C.lv009 not in (2,3,7)";
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.lv002,A.lv021,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,B.lv002 NVID,C.lv029 lv001,A.lv001 TimeCardId,A.lv015  FROM tc_lv0011 A inner join tc_lv0010 B on A.lv002=B.lv001 inner join hr_lv0020 C on C.lv001=B.lv002 WHERE A.lv100<>1 $vcondition order by C.lv029 ASC,A.lv004 ASC";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		switch ($lvgt)
				{
					case 0:
						...
						$vArrALCL[1]['lv001']='P';
						$vArrALCL[1]['lv002']='P';
						$vArrALCL[2]['lv001']='0.5P';
						$vArrALCL[2]['lv002']='0.5P';
						$vArrALCL[3]['lv001']='CL';
						$vArrALCL[3]['lv002']='CL';
						$vArrALCL[4]['lv001']='0.5CL';
						$vArrALCL[4]['lv002']='0.5CL';
						break;
					case 1:
						$vArr[1]['lv001']='';
						$vArr[1]['lv002']='';
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$vDayCss=(int)getday($vrow['lv004']);
				switch ($lvgt)
				{
					case 0:
						if($this->isEdit==1)
						{
							$lvstrgt='<select title="'.$this->ArrEmp[$lvvt][1]." ".$this->FormatView($vrow['lv004'],2).'" lang="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="lineday_'.$vDayCss.'" style="width:49px;font-size:10px;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="timecard_'.$vrow['TimeCardId'].'" onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)">'.$this->CreateSelectArr($vArr,$vrow['lv021']).'</select>';	
							$lvstrgt1='<select title="'.$this->ArrEmp[$lvvt][1]." ".$this->FormatView($vrow['lv004'],2).'" lang="runchangetime2(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="lineday2_'.$vDayCss.'" style="width:49px;font-size:10px;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="timecard2_'.$vrow['TimeCardId'].'" onblur="runchangetime2(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)">'.$this->CreateSelectArr($vArrALCL,$vrow['lv007']).'</select>';	
						}
						else
						{
							$lvstrgt=(trim($vrow['lv015'])=="")?"&nbsp;":$vrow['lv015'];
							$lvstrgt1=(trim($vrow['lv015'])=="")?"&nbsp;":$vrow['lv017'];
						}
							
						$this->ArrTC[$vrow['lv015']]=(int)$this->ArrTC[$vrow['lv015']]+1;
						$this->ArrTCEmp[$vrow['NVID']][$vrow['lv015']]=(int)$this->ArrTCEmp[$vrow['NVID']][$vrow['lv015']]+1;
						break;
					case 1:
						if($this->isEdit==1)
						{
							$lvstrgt='<select lang="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="lineday_'.$vDayCss.'"  style="width:49px;font-size:10px;background-color:#@@99" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)">'.$this->CreateSelectArr($vArr,$vrow['lv021']).'<select>';
							$lvstrgt1='<select lang="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="lineday2_'.$vDayCss.'"  style="width:49px;font-size:10px;background-color:#@@99" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)">'.$this->CreateSelectArr($vArrALCL,$vrow['lv007']).'<select>';
						}
						else
						{
							$lvstrgt=(trim($vrow['lv010'])=="")?"&nbsp;":$vrow['lv010'];
							$lvstrgt1=(trim($vrow['lv007'])=="")?"&nbsp;":$vrow['lv007'];
						}
			...
			$this->ArrEmp[$lvvt][6]=str_replace("<!--:".$vrow['lv004']."-->",$lvstrgt1,$this->ArrEmp[$lvvt][6]);
			$this->ArrEmp[$lvvt][6]=str_replace("<!--&".$vrow['lv004']."-->",$this->GetTimeList($vrow['NVID'],$vrow['lv004'],0),$this->ArrEmp[$lvvt][6]);
	...
	if($childfunc=="rpt" || $childfunc=="excel" || $childfunc=="word"  || $childfunc=="pdf" )
	{
		...
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
			<tr height=\"30px\">
				<td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[1]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[3]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[2]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[5]."</b></td>".$this->lvHeader."<td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[30]."</b></td>		
			</tr>
			@01
		</table>
		";
	...
	if($this->lvSort==1)		
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
			<tr height=\"30px\" class='lvhtable'>
				<td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[1]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[2]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[3]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td><td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[5]."</b></td><td><input name=\"chkAll\" type=\"checkbox\" id=\"chkAll\" onclick=\"ChkAllShift(this)\" value=\"400\" tabindex=\"2\"></td>".$this->lvHeader."<td class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td>	
			</tr>
			@01
		...
		for($i=0;$i<count($this->ArrEmp);$i++)
		{
			...
			$vp_col1_1="<td nowrap='nowrap' align=\"right\">".$this->FormatView($this->ArrEmp[$i][3],10)."</td>";
			...
			if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
				...
				$vp_col2_2="<td nowrap='nowrap'>".$this->ArrEmp[$i][7]."</td>";
			$vp_col3_1="";
			$vp_col3_3="";
			...
			$vp_col4_1='<td nowrap="nowrap"><input id="nvcheck_'.($i+1).'" type="checkbox" class="shiftnv"></td>';
			if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
			...
			
			{
				...
				if(getyear($this->ArrEmp[$i][44])>2014) $color="#05e960";
				if($this->is_tc09_apr==1)
				{
					if($this->ArrEmp[$i][7]==1)
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2$vp_col3_2".$vp_col4_1.$vp_col4.$vp_col5;
					else
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2_1$vp_col3_2".$vp_col4_1.$vp_col4.$vp_col5;	
				}
				else
					$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2_2$vp_col3_2".$vp_col4_1.$vp_col4.$vp_col5;
/*									
				if($this->ArrEmp[$i][7]==1)
				{
					if($this->is_tc09_unapr==1)
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2$vp_col3_3".$vp_col4_1.$vp_col4.$vp_col5;	
					else
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col3_3".$vp_col4_1.$vp_col4.$vp_col5;	
				}
				else
					if($this->is_tc09_add==1)
					{
						...
						{
							if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
								$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col3".$vp_col4_1.$vp_col4.$vp_col5;
							else
								$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1<td nowrap='nowrap'>".$this->ArrEmp[$i][7]."</td>$vp_col3".$vp_col4_1.$vp_col4.$vp_col5;
					...
					{
						if($this->is_tc09_apr==1)
							$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2_1$vp_col1_1".$vp_col4_1.$vp_col4.$vp_col5."</tr>";
						else
						{
							if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
								$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2_2".$vp_col4_1.$vp_col4.$vp_col5."</tr>";
							else
								$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2_2".$vp_col4_1.$vp_col4.$vp_col5."</tr>";
						}
					}
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv007':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0013 order by lv001";
				break;
			case 'lv009':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv007':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0013 where lv001='$vSelectID'";
				$lvopt=4;
				break;
			case 'lv009':
