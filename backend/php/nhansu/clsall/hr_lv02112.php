...
/////////////coding hr_lv0211///////////////
//
class   hr_lv0211 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv002,lv003,lv004,lv005,lv006,lv011,lv007,lv008,lv009,lv010";	
////////////////////GetDate
...
////////////
	...
	var $ArrViewEnter=array("lv003"=>"89","lv005"=>"11","lv006"=>"89","lv007"=>"33","lv008"=>"99","lv009"=>"-1","lv010"=>"-1");
	public $LE_CODE="NjlIUS02VFdULTZIS1QtNlFIQQ==";
	var $Tables=array('lv008'=>'hr_lv0002');
	...
	function LV_LoadCodeHelpID($vlv003)
	{
		$lvsql="select * from  hr_lv0211 Where lv003='$vlv003'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadUserID($vlv002)
	{
		...
		$lvsql="select lv001,lv005,lv003 from  hr_lv0211 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
		if($this->lang=='VN') $btxoa="Xóa";
		else $btxoa="Delete";
		while($vrow = db_fetch_array ($vresult))
		{
			$this->lv001=$vrow['lv001'];
			...
			if($strReturn=="") $strReturn="<tr bordercolor=#999999><td><a href=\"javascript:delattach('".$this->lv001."')\">".$btxoa."</a></td><td align=left>".$this->lv005."</td><td style=\"text-align:right!important\">".$this->LV_GetByte($this->lv003)."</td></tr>";
			else
		...
		if($strReturn!="") $strReturn=$strReturn."<tr bordercolor=#999999><td align=left colspan=2>&nbsp;"."</td><td style=\"text-align:right!important\">".$this->LV_GetByte($strTotal)."</td></tr>";
	...
	function LV_LoadUserArray($vlv002)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select * from  hr_lv0211 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
		return $vresult;
	}
	...
	function LV_LoadStep($vlv007='sovanban',$vlv008='tokhai',$vlv009='bbbg',$vCodeID)
	{
		...
		$lvsql="select * from  hr_lv0211 Where lv007='$vlv007' and lv008='$vlv008' and lv009='$vlv009'";
		$vresult=db_query($lvsql);
		if($this->lang=='VN') $btxoa="Xóa";
		else $btxoa="Delete";
		while($vrow = db_fetch_array ($vresult))
		{
			...
			$strTotal=$strTotal+$vrow['lv003'];
			if($strReturn=="") 
				$strReturn="<tr bordercolor=#999999><td><a href=\"javascript:delattach('".$this->lv001."','".$vlv008."','".$vCodeID."','".$vlv009."')\">".$btxoa."</a></td><td align=left><a target='_blank' href=''.$this->Dir.'".($this->lv004.'/'.$this->lv005)."'>".$this->lv005."</a></td></tr>";
			else
	...
	function LV_LoadStepCheckTemp($vlv007)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select lv001 from  nhansu_sof_documents_v4_0.hr_lv0211_temp Where lv002='$vlv007' ";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		if($vrow)
		{
	...
	function LV_LoadStepCheck($vlv007)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select lv001 from  nhansu_sof_documents_v4_0.hr_lv0211 Where lv002='$vlv007' ";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		if($vrow)
		{
	...
	function LV_UpdateAutoTemp($vKetQua,$vViTri,$vHinh)
	{
		...
		$vField='lv008';
		$lvsql="Update nhansu_sof_documents_v4_0.hr_lv0211_temp set $vField='".sof_escape_string($vHinh)."' where lv001='$vKetQua'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211_temp.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_InsertAuto($vViTri,$vHinh)
	{
		if($this->isAdd==0) return false;
		$vField='lv008';
		$lvsql="insert into nhansu_sof_documents_v4_0.hr_lv0211 (lv002,lv003,lv004,lv005,lv006,lv007,$vField) values('".sof_escape_string($this->lv002)."','".sof_escape_string($this->lv003)."','".sof_escape_string($this->lv004)."','".sof_escape_string($this->lv005)."','".sof_escape_string($this->lv006)."','".sof_escape_string($this->lv007)."','".sof_escape_string($vHinh)."')";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->lv001=sof_insert_id();
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0211.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_UpdateAuto($vKetQua,$vViTri,$vHinh)
	{
		if($this->isEdit==0) return false;
		$vField='lv008';
		$lvsql="Update nhansu_sof_documents_v4_0.hr_lv0211 set $vField='".sof_escape_string($vHinh)."' where lv001='$vKetQua'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Insert()
	{
		if($this->isAdd==0) return false;
		$lvsql="insert into hr_lv0211 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			...
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0211.insert',sof_escape_string($lvsql));
			//Attached files
			$lvsql="insert into nhansu_sof_documents_v4_0.hr_lv0211(lv002,lv003,lv004,lv005,lv006,lv007,lv008) select '$vInsertID',lv003,lv004,lv005,lv006,lv007,lv008 from nhansu_sof_documents_v4_0.hr_lv0211_temp where lv007='".$this->LV_UserID."'";
			$vReturn= db_query($lvsql);
			if($vReturn) 
			{
				$this->InsertLogOperation($this->DateCurrent,'hr_lv0211.insert',sof_escape_string($lvsql));
				$lvsql1="delete from nhansu_sof_documents_v4_0.hr_lv0211_temp where lv007='".$this->LV_UserID."'";
	...
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update hr_lv0211 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->LV_UserID',lv010=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM hr_lv0211  WHERE lv001 IN ($lvarr)";// and (select count(*) from hr_lv0211 B where  B.lv002= hr_lv0211.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0211.delete',sof_escape_string($lvsql));
			$lvsql = "DELETE FROM nhansu_sof_documents_v4_0.hr_lv0211  WHERE lv007 IN ($lvarr)";// and (select count(*) from hr_lv0221 B where  B.lv002= hr_lv0221.lv001)<=0  ";
			$vReturn= db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'nhansu_sof_documents_v4_0.hr_lv0211.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update hr_lv0211 set lv006=1  WHERE hr_lv0211.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isUnApr==0) return false;
		$lvsql = "Update hr_lv0211 set lv006=0  WHERE hr_lv0211.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_DeleteUser($vUserID)
	{
	if($this->isDel==0) return false;
		$lvsql = "DELETE FROM nhansu_sof_documents_v4_0.hr_lv0211  WHERE hr_lv0211.lv002='$vUserID'";// and (select count(*) from hr_lv0211 B where  B.lv002= hr_lv0211.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0211.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	////////GetFile Attachment/////////
	function LV_GetForward($AttachFile,$AttachFileHidden)
	{
		...
		{
			$strTemp=$strTr;
			$this->lv001=InsertWithCheckExt('hr_lv0211', 'lv001', '',0);
			$this->lv002=$_SESSION['ERPSOFV2RUserID'];
			$arrTemp1=explode(">",$arrFile[$i],2);
			$arrTemp2=explode("<",$arrTemp1[1],2);
			$path = $this->Dir."'.$this->Dir.'images/human/File/MailTemp/";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0211 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		<tr ><td align=center>Total:</td><td align=left><font color='red'>".$this->GetCount()."</font></td><td colspan=\"".(count($lstArr))."\">$paging</td></tr>
		@#01
		</table>
		";
		...
		$sqlS = "SELECT * FROM hr_lv0211 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				switch($this->ArrViewEnter[$vField])
				{
					//browfile
					case 11:
						$lvImg="<a target='_blank' href='".$this->Dir."hr_lv0211/readfiletemp.php?FileID=".$vrow['lv001']."&type=8&size=0'><div style='border-radius:5px;background:#aaa;padding:4px;color:#000;width:40px;text-align:center;'>View</div></a>";
						$vstr='
						<table valign=\"top\" style=\"width:100%\">
							<tr>
								...
								<div id="framuploadtemp" style="width:116px">
									<iframe  height=24 width="100%" marginheight=0 marginwidth=0 frameborder=0 src="'.$this->Dir.'hr_lv0211/?childfunc=uploadtemp&ViTriUp=8&FileID='.$vrow['lv001'].'&lang='.$this->lang.'"></iframe>
								</td>
								<td align=\"center\">
					...
					case 99:
						...
						$vstr='<ul style="width:100%" id="pop-nav" lang="pop-nav1" onMouseOver="ChangeName(this,1)" onKeyUp="ChangeName(this,1)"> <li class="menupopT">
									<input autocomplete="off" class="txtenterquick" type="text" autocomplete="off" style="width:100%;min-width:30px" name="qxt'.$vField.'" id="qxt'.$vField.'" onKeyUp="LoadSelfNextParent(this,\'qxt'.$vField.'\',\''.$this->Tables[$vField].'\',\'lv001\',\'concat(lv002,@! @!,lv001)\')"  onKeyPress="return CheckKey(event,7)" tabindex="2" value="'.$this->Values[$vField].'">
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv005')
				{
					$lvImg="<a target='_blank' href='".$this->Dir."hr_lv0211/readfile.php?FileID=".$vrow['lv001']."&type=8&size=0'><div style=\"border-radius:5px;background:#aaa;padding:4px;color:#000;\">Download</div></a>";
					$vTempEnter='
					<table valign=\"top\" style=\"width:100%\">
						...
						<iframe  height=24 width="100%" marginheight=0 marginwidth=0 frameborder=0 src="'.$this->Dir.'hr_lv0211/?childfunc=upload&ViTriUp=8&FileID='.$vrow['lv001'].'&lang='.$this->lang.'"></iframe>
						</div>
						</td>
				...
				elseif($lstArr[$i]=='lv011')
				{
					if($this->GetEdit()==1)
					{		
						$key = $lstArr[$i];				
						$lvTdTextBox="<td align=center><input class='txtenterquick' type=\"checkbox\" value=\"1\" ".(($vrow['lv011']==1)?'checked="true"':'')." @03 onclick=\"UpdateTextCheck(this,'".$vrow['lv001']."',11)\" style=\"width:35px;text-align:center;\" tabindex=\"2\" maxlength=\"32\"   onKeyPress=\"return CheckKey(event,7)\"/></td>";
						//$vTemp=str_replace("@02",$this->FormatView($vrow[$lstArr[$i]],0),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]]));	
						$vTemp = str_replace("@02",$this->FormatView($vrow[$key] ?? '', 0),$this->Align(str_replace("@01", $vrow['lv001'] ?? '', $lvTdTextBox),(int)($this->ArrView[$key] ?? 0)));
					}
					else
						$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0211/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0211 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{	
			case 'lv003':
				$vsql="select lv001, lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0214";
				break;
			case 'lv006':
	...
	{
		if($this->ArrGetValueLink[$vFile][$vSelectID][0] ?? null) return $this->ArrGetValueLink[$vFile][$vSelectID][1];
		if($vSelectID=="")
		{
			return $vSelectID;
		}
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0214 where lv001='$vSelectID'";
				break;
			case 'lv006':
