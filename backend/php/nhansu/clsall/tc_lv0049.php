...
/////////////coding tc_lv0049///////////////
class   tc_lv0049 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0049.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0049.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0049.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0049.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
...
//////////////////////Buil list////////////////////
	...
	function LV_BuilListInput($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,DAYOFWEEK(lv004) DOW FROM tc_lv0011 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0049/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0049/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$lvHref="@02";
		$sqlS = "SELECT lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,DAYOFWEEK(lv004) DOW FROM tc_lv0011 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function Get_String_DateFromTo()
	{
		...
		for($i=1;$i<=count($vArrDay);$i++)
		{
			...
			if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
				{
					if($_GET['day']=='')
					{
						$this->lvHeader=$this->lvHeader.'<td COLSPAN="2" class="lvhtable" align="center"><b><font color="'.$color.'">'.getday($datecur).'</font></b></td>';
						$this->lvHeader1=$this->lvHeader1.'<td  align="center">Chỉnh giờ vào ra</td><td  align="center">Giờ từ máy</td>';
					}
					else
					{
						$this->lvHeader=$this->lvHeader.'<td COLSPAN="2" class="lvhtable" align="center"><b><font color="'.$color.'">'.$this->FormatView($datecur,2).'</font></b></td>';						
						$this->lvHeader1=$this->lvHeader1.'<td  align="center">Chỉnh giờ vào ra</td><td  align="center">Giờ từ máy</td>';
					}
				}
			else
			{
				if($this->lang=="EN")
				{
					$this->lvHeader=$this->lvHeader.'<td colspan=2 class="lvhtable" align="center"><b><font color="'.$color.'">'.getday($datecur).'</font></b><br/><input type="checkbox" id="hdcheck_'.((int)getday($datecur)).'" onclick="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)"/></td>';
					$this->lvHeader1=$this->lvHeader1.'<td align="center">Edit In/Out</td><td  align="center">Machine In/Out</td>';
				}
				else
				{
					$this->lvHeader=$this->lvHeader.'<td colspan=2 class="lvhtable" align="center"><b><font color="'.$color.'">'.getday($datecur).'</font></b><br/><input type="checkbox" id="hdcheck_'.((int)getday($datecur)).'" onclick="checkRunCong(\''.trim(((int)getday($datecur))).'\',this)"/></td>';
			...
			$strTD=$strTD.'<td align="center">'."<!--6_".str_replace("/","-",$datecur)."-->".'</td><td align="center"><!--7_'.str_replace("/","-",$datecur).'--></td>';
	...
	function GetTimeListOutOff($vlv001,$vlv002,$vid,$vopt=0,$vGetNoID=0)
	{
		...
		if($vresult){
			...
			while($vrow=db_fetch_array($vresult))
			{
				...
				{
					if(trim($vrow['lv005'])=='' || $vrow['lv005']==NULL)
						$strReturn=$strReturn.'<a href="javascript:'.(($vrow['lv006']=='1')?'undeltime':(($vrow['lv006']=='0' && $vrow['lv004']=='OTO')?'deltime':'overtime')).'(\''.$vrow['lv001'].'\',\''.$vrow['lv002'].'\',\''.$vrow['lv003'].'\',\''.$vid.'\')"><font style="'.(($vrow['lv006']=='1')?'text-decoration:line-through;':(($vrow['lv006']=='0' && $vrow['lv004']=='OTO')?'color:red':'color:blue;')).'">'.substr($vrow['lv003'],0,5)."</font></a>";
					else
					{
			...
			if($vGetNoID==1)
				return '<span onclick="showtimeadd('.$vid.')" style="cursor:pointer"><img src="../images/icon/add_bn.png"/></span><div style="display:none;position:absolute;" id="timeadd_'.$vid.'" ><input class="lineday_'.((int)getday($vlv002)).'" id="txttimeadd_'.$vid.'" type="textbox" value="00:00" onchange="addtime(\''.$vlv001.'\',\''.$vlv002.'\','.$vid.',this.value)" lang="addtime(\''.$vlv001.'\',\''.$vlv002.'\','.$vid.',\'@!99\')" style="width:60px;right:0px;top:0px;"/><img src="../images/icon/close.png"  onclick="closetimeadd('.$vid.')"/></div> | '.$strReturn.'';
			else
				return '<div id="'.$vid.'" style="padding:3px;white-space: nowrap;"><span onclick="showtimeadd('.$vid.')" style="cursor:pointer"><img src="../images/icon/add_bn.png"/></span><div style="display:none;position:absolute;" id="timeadd_'.$vid.'" ><input class="lineday_'.((int)getday($vlv002)).'" id="txttimeadd_'.$vid.'" type="textbox" value="00:00" onchange="addtime(\''.$vlv001.'\',\''.$vlv002.'\','.$vid.',this.value)" lang="addtime(\''.$vlv001.'\',\''.$vlv002.'\','.$vid.',\'@!99\')" style="width:60px;right:0px;top:0px;"/><img src="../images/icon/close.png"  onclick="closetimeadd('.$vid.')"/></div> | '.$strReturn.'</div>';
	}
	...
	{
		$vArr=Array();
		$vsql="select * from tc_lv0004 order by lv001";
		$bResult=db_query($vsql);
		$i=0;
		while ($vrow = db_fetch_array ($bResult)){
	...
	{		
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,B.lv002 NVID,C.lv029 lv001,A.lv001 TimeCardId,A.lv015  FROM tc_lv0011 A inner join tc_lv0010 B on A.lv002=B.lv001 inner join hr_lv0020 C on C.lv001=B.lv002 WHERE 1=1 $vcondition order by C.lv029 ASC,A.lv004 ASC";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$vDayCss=(int)getday($vrow['lv004']);
				switch ($lvgt)
				{
					case 0:
						if($this->isEdit==1)
							$lvstrgt='<select lang="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="lineday_'.$vDayCss.'" style="width:49px;font-size:10px;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="timecard_'.$vrow['TimeCardId'].'" onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)">'.$this->CreateSelectArr($vArr,$vrow['lv015']).'</select>';
						else
			...
			if($this->isEdit==1)
				$this->ArrEmp[$lvvt][6]=str_replace("<!--6_".$vrow['lv004']."-->",$this->GetTimeListOutOff($vrow['NVID'],$vrow['lv004'],$vrow['TimeCardId'],$vViewTimeAll),$this->ArrEmp[$lvvt][6]);
			else
				$this->ArrEmp[$lvvt][6]=str_replace("<!--6_".$vrow['lv004']."-->",$this->GetTimeList($vrow['NVID'],$vrow['lv004'],1),$this->ArrEmp[$lvvt][6]);
			$this->ArrEmp[$lvvt][6]=str_replace("<!--7_".$vrow['lv004']."-->",$this->GetTimeList($vrow['NVID'],$vrow['lv004']),$this->ArrEmp[$lvvt][6]);
			}
	...
	if($childfunc=="rpt" || $childfunc=="excel" || $childfunc=="word"  || $childfunc=="pdf" )
	{
		...
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
			<tr height=\"30px\">
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[1]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[2]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td>".$this->lvHeader."
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[30]."</b></td>		
			</tr>
			@01
		</table>
		";
	...
	if($this->lvSort==1)		
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
			<tr height=\"30px\" class='lvhtable'>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[1]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[2]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[3]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td>
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[5]."</b></td>
				<td rowspan='2'><input name=\"chkAll\" type=\"checkbox\" id=\"chkAll\" onclick=\"ChkAllShift(this)\" value=\"400\" tabindex=\"2\"></td>".$this->lvHeader."
				<td rowspan='2' class=\"lvhtable\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td>	
			</tr>
			<tr>
				".$this->lvHeader1."		
