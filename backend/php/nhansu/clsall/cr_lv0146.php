<?php
/////////////coding cr_lv0146///////////////
class   cr_lv0146 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $month=null;
	public $year=null;
	public $isConnectSQLSERVER=null;
	public $lv011=null;
	
	
 
///////////
	//public $DefaultFieldList="lv004,lv012,lv005,lv007";	
	public $DefaultFieldList="lv004,lv999,lv012,lv099";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='cr_lv0146';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv099"=>"100","lv100"=>"101","lv999"=>"1000");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0","lv017"=>"0","lv018"=>"0","lv019"=>"0","lv020"=>"0","lv099"=>"0","lv100"=>"0","lv999"=>"0");	
	...
	function LV_Update()
	{
		...
		$lvsql="Update tc_lv0011 set lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016',lv017='$this->lv017' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'cr_lv0146.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0146.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0146.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0146.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	protected function GetCondition()
	{
		...
		if($this->lvNVID!="")
		{
			$vListCalendar=$this->LV_GetCalendarEmp($this->lvNVID);
			$strCondi=$strCondi." and lv002 IN ($vListCalendar)";	
		}			
		return $strCondi;
	}
...
//////////////////////Buil list////////////////////
	...
	function LV_LoadContractLaborParent($vParentID)
	{
		$this->isLoop=0;
		...
		while($vrow=db_fetch_array($vresult))
		{
			switch($vrow['lv003'])
			{
				...
				case 2:
					return 'Hợp đồng 2 năm gia hạn 1 năm';
				case 3:
					return 'Hợp đồng vô thời hạn';
					break;
				default:
					$vHDVisible='PLHĐ';
					$vHDVisible1=$this->LV_LoadContractLaborChild($vrow['lv099'],$vHDVisible);
					return $vHDVisible1;
	...
	function LV_LoadContractLaborChild($vParentID,$vHDVisible)
	{
		$lvsql="select A.lv001,A.lv002,A.lv003,A.lv004,A.lv006,A.lv099,A.lv097 from hr_lv0038 A where A.lv001='$vParentID'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			switch($vrow['lv003'])
			{
				...
				case 3:
					...
					break;			
				default:
					$vHDVisible=$this->LV_LoadContractLaborChild($vrow['lv099']);
				return $vHDVisible;
			}
		}
		return $vHDVisible;
	...
	{
		$vArrThu=Array(1=>'CN',2=>'T2',3=>'T3',4=>'T4',5=>'T5',6=>'T6',7=>'T7');
		$this->LV_CheckLock();
		if($lvList=="") $lvList=$this->DefaultFieldList;
		if($this->isView==0) return false;
		$lstArr=explode(",",$lvList);
		...
		*/
		if ($this->LV_CheckLocked($this->lv004 . "-01")) {
    // Đảm bảo $vLangArr là mảng
			if (!isset($vLangArr) || !is_array($vLangArr)) {
				$vLangArr = [1 => 'Save', 2 => 'Save']; // Giá trị mặc định nếu không có
			}

			$title = isset($vLangArr[1]) ? htmlspecialchars($vLangArr[1]) : 'Save';
			$label = isset($vLangArr[2]) ? htmlspecialchars($vLangArr[2]) : 'Save';

			$vSave = "<a class=\"lvtoolbar\" href=\"javascript:Save();\" tabindex=\"47\">
				<img src=\"../images/controlright/save_f2.png\" alt=\"Save\" title=\"$title\" name=\"save\" border=\"0\" align=\"middle\" id=\"save\" />
				$label</a>";
		...
		$lvTable="
		<table  class=\"lvtable\" style=\"width:400px!important\">
		@#01
		</table>
		";
		$lvTrH="<tr class=\"lvhtable\">
		...
		$sqlS = "SELECT lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,DAYOFWEEK(lv004) DOW,lv014,lv015,lv016,lv017,lv018,lv099,lv100 FROM tc_lv0011 WHERE lv100<>1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
    ...
    if (!isset($vArrSave[$vrow['lv099']][0]) || $vArrSave[$vrow['lv099']][0] != true) {
        $vTenHopDong = $this->LV_LoadContractLaborParent($vrow['lv099']);
        $vArrSave[$vrow['lv099']][0] = true;
        $vArrSave[$vrow['lv099']][1] = $vTenHopDong;
    }
						else
						{
							$vTenHopDong=$vArrSave[$vrow['lv099']][1];
						}
						
						$vTemp=str_replace("@02",str_replace("@02",$vTenHopDong,str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
					case 'lv999':
						$vTemp=str_replace("@02",str_replace("@02",$vArrThu[$vrow['DOW']],str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
					case 'lv012':
						$vTemp=str_replace("@02",str_replace("@02",$this->GetTimeList($this->lvNVID,$vrow['lv004']),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
					default:
						$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."cr_lv0146/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	public function GetTimeCodeMore($vEmployeeID,$vyear,$vmonth)
	{
		...
		$this->ArrTimeCordPush[5]="Ti?n com dã an";
		for($i=0;$i<count($lstArr);$i++)
			{
				$vTemp=str_replace("@01","",$lvTdH);
...
//////////////////////Buil list////////////////////
	...
	function Get_String_DateFromTo()
	{
		...
		for($i=1;$i<=$lvNumDate+1;$i++)
		{
			...
			if($vdayofw==1) 
				...
				$color='';
			$this->lvHeader=$this->lvHeader.'<td class="tdhprint"><b><font color="'.$color.'">'.$this->FormatView($datecur,2).'</font></b></td>';
	...
	function Get_Arr_Employees()
	{
		...
		if($this->lv030!="")  $strCondi=$strCondi." AND DD.lv001 in ('".$this->lv030."')";
		if($this->lvSort==0)
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv001 MonthID from hr_lv0020 DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv029";
		else 
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv001 MonthID from hr_lv0020 DD DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv001";
		$vresult=db_query($lvsql);	
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
	...
	{
		$vArr=Array();
		$vsql="select lv001, concat(lv003,'(',lv004,'%)') lv002 from  tc_lv0025 where lv002 in (select A.lv001 from tc_lv0013 A where A.lv011=1) ";
		$bResult=db_query($vsql);
		$i=1;
		$vArr[0]['lv001']='';
		$vArr[0]['lv002']='';
	...
	{		
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=@#04>@02</td>";
		$sqlS = "SELECT A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,B.lv002 NVID,C.lv029 lv001,A.lv001 TimeCardId  FROM tc_lv0011 A inner join tc_lv0010 B on A.lv002=B.lv001 inner join hr_lv0020 C on C.lv001=B.lv002 WHERE lv100<>1 $vcondition order by C.lv029 ASC,A.lv004 ASC";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$lvstrgt="";
				...
				{
					case 0:
							$lvstrgt='<select '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" onblur="runchangetime(\''.$vrow['TimeCardId'].'\',this.value)">'.$this->CreateSelectArr($vArr,$vrow['lv007']).'</select>';
						break;
					case 1:
							$lvstrgt='<select '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime(\''.$vrow['TimeCardId'].'\',this.value)">'.$this->CreateSelectArr($vArr,$vrow['lv010']).'<select>';
						break;
				}
	...
	function Get_BuildList_Array()
	{
		...
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\">
				<td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[0]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[1]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[2]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[3]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[4]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[5]."</b></td><td class=\"tdhprint\" width=\"10%\" align=\"center\"><b>".$this->ArrPush[6]."</b></td>".$this->lvHeader."	
			</tr>
			@01
		</table>
		";
		...
		for($i=0;$i<count($this->ArrEmp);$i++)
		{
			...
			if($this->lvSort==1)
			{
				if($this->ArrEmp[$i][7]==1)
					$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'><td nowrap='nowrap'>".$i."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td><td nowrap='nowrap'><input id='btmonth_".$this->ArrEmp[$i][8]."' value='Mở Khóa' style='width:39px' type='button' onclick=\"UnLock('".$this->ArrEmp[$i][8]."')\"/></td><td nowrap='nowrap'>".$this->FormatView($this->ArrEmp[$i][3],10)."</td><td nowrap='nowrap'><select>'".$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td>".str_replace("<!--","&nbsp;<!--",$this->ArrEmp[$i][6])."</tr>";	
				else
					$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'><td nowrap='nowrap'>".$i."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td><td nowrap='nowrap'><input id='btmonth_".$this->ArrEmp[$i][8]."' value='Khóa' style='width:39px' type='button' onclick=\"Locked('".$this->ArrEmp[$i][8]."'\")/></td><td nowrap='nowrap'><input style='width:39px' type='text' value='".$this->ArrEmp[$i][3]."' onblur=\"UpdateMonthly('".$this->ArrEmp[$i][8]."',this.value,1)\"/></td><td nowrap='nowrap'><select ".(($vFocus==1)?' tabindex="1"':'').'" onblur="UpdateMonthly(\''.$this->ArrEmp[$i][8].'\',this.value,2)">'.$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td>".str_replace("<!--","&nbsp;<!--",$this->ArrEmp[$i][6])."</tr>";	
			}
			else
			{
				if($this->ArrEmp[$i][7]==1)
					$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'><td nowrap='nowrap'>".$i."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td><td nowrap='nowrap'><input id='btmonth_".$this->ArrEmp[$i][8]."' value='Mở Khóa' style='width:39px' type='button' onclick=\"UnLock('".$this->ArrEmp[$i][8]."')\"/></td><td nowrap='nowrap'>".$this->FormatView($this->ArrEmp[$i][3],10)."</td><td nowrap='nowrap'><select>'".$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td>".str_replace("<!--","&nbsp;<!--",$this->ArrEmp[$i][6])."</tr>";	
				else
					$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'><td nowrap='nowrap'>".$i."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td><td nowrap='nowrap'><input id='btmonth_".$this->ArrEmp[$i][8]."' value='Khóa' style='width:39px' type='button' onclick=\"UpdateMonthly('".$this->ArrEmp[$i][8]."','".$this->ArrEmp[$i][7]."',3)\"/></td><td nowrap='nowrap'><input style='width:39px' type='text' value='".$this->ArrEmp[$i][3]."' onblur=\"UpdateMonthly('".$this->ArrEmp[$i][8]."',this.value,1)\"/></td><td nowrap='nowrap'><select ".(($vFocus==1)?' tabindex="1"':'').'" onblur="UpdateMonthly(\''.$this->ArrEmp[$i][8].'\',this.value,2)">'.$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td>".str_replace("<!--","&nbsp;<!--",$this->ArrEmp[$i][6])."</tr>";
			}
		}
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv011':
				$vsql="
				select MP.* from(
				select '' lv001,'...' lv002, 0 lv003
				union
				select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0002 
				) MP
				order by MP.lv001";
				break;
			case 'lv015':
				$vsql="
				select '' lv001,'...' lv002,IF(''='$vSelectID',1,0) lv003 
				union
				select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0004";				
				break;
			case 'lv099':
