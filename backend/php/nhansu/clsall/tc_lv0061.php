<?php
/////////////coding tc_lv0061///////////////
class   tc_lv0061 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"2","lv004"=>"2","lv005"=>"0","lv006"=>"2","lv007"=>"0","lv008"=>"0","lv009"=>"10");	
	...
	{
		$lvsql="select * from  tc_lv0061 Where lv001='$vlv001'";
	...
	{
		$vmonth=(int)$vmonth;
		$lvsql="select * from  tc_lv0061 Where Month(lv004)='$vmonth' and Year(lv004)='$vyear'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  tc_lv0061 Where lv008=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into tc_lv0061 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005',CurDate(),'$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->InsertLogOperation($this->DateCurrent,'tc_lv0061.insert',sof_escape_string($lvsql));
	...
	{
		$lvsql="select * from  tc_lv0061 Where lv008<=1 and lv001 in ($lvarr)";
		$vresult=db_query($lvsql);
	...
	function LV_Update()
	{
		...
		$this->lv003 = ($this->lv003!="")?recoverdate(($this->lv003), $this->lang):$this->DateDefault;
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		$lvsql="Update tc_lv0061 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004' where  lv001='$this->lv001' and lv008<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0061.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM tc_lv0061  WHERE tc_lv0061.lv001 IN ($lvarr) and tc_lv0061.lv008<=0   ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0061.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update tc_lv0061 set lv007='".getInfor($_SESSION['ERPSOFV2RUserID'],2)."',lv008=IF(lv008<2,lv008+1,2) WHERE tc_lv0061.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0061.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$vUserID=getInfor($_SESSION['ERPSOFV2RUserID'], 2);
		$lvsql = "Update tc_lv0061 set lv007='".getInfor($_SESSION['ERPSOFV2RUserID'],2)."',lv008=IF(lv008>0,lv008-1,0) WHERE tc_lv0061.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0061.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM tc_lv0061 WHERE 1=1 ".$this->GetCondition();
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join tc_lv0061 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
	...
	function LV_GetContractMoneyDate($vContractID,$StartDate,$EndDate)
	{
		...
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join tc_lv0061 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 $condition1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM tc_lv0061 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('tc_lv0061/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM tc_lv0061 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportStartEndDate($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvEmployeeID,$lvStartDate,$lvEndDate)
	{
		...
		$sqlS = "SELECT * FROM tc_lv0061 WHERE 1=1 $condition ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
