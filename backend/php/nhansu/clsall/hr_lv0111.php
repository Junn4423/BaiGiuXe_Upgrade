<?php
/////////////coding hr_lv0111///////////////
class   hr_lv0111 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='hr_lv0111';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"10","lv006"=>"10","lv007"=>"0","lv008"=>"0","lv009"=>"22");	
	var $ArrViewEnter=array("lv003"=>"33","lv008"=>"-1","lv009"=>"-1");
	...
	function LV_CheckAutoActive($vYear,$vMonth,$vDay,$vDate)
	{
		if($vDay==1 || $vDay==16)
		{
			$vDay=(int)$vDay;
			$lvsql="select lv001,lv099 from  hr_lv0111 Where lv006='$vMonth' and lv007='$vYear' and lv004<='$vDate' and lv005>='$vDate'";
			$vresult=db_query($lvsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
			{
				//Kích hoạt kỳ trước lên 2
				//Kích hoạt kỳ hiện tại.
				$vsql="update hr_lv0111 set lv011=1 where lv001='".$vrow['lv001']."' and lv011<1";
				db_query($vsql);
				$vsql="update hr_lv0111 set lv011=2 where lv001='".$vrow['lv099']."' and lv011<=1";
				db_query($vsql);
			}
			
		}
	}
	function LV_GetKy2($vYear,$vMonth)
	{
		$vMonth=(int)$vMonth;
		$vID='';
		$lvsql="select lv001 from  hr_lv0111 Where lv006='$vMonth' and lv007='$vYear' and lv003='2'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
		...
		if($vID=='' || $vID==null)
		{
			$vDate=$vYear."-".Fillnum($vMonth,2)."-16";
			$this->LV_AutoRun($vDate);
			$lvsql="select lv001 from  hr_lv0111 Where lv006='$vMonth' and lv007='$vYear' and lv003='2'";
			$vresult=db_query($lvsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_LoadPreMonthCalAuto($vyearpre,$vmonthpre)
	{
		$lvsql="select * from  hr_lv0111 Where lv007='$vyearpre' and lv006='$vmonthpre' order by lv003 desc";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadPreCalAuto($vCalID)
	{
		$lvsql="select * from  hr_lv0111 Where lv001='$vCalID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadPreCal($vCalID)
	{
		$lvsql="select lv001 from  hr_lv0111 Where lv099='$vCalID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_ConfirmLoad($vYear,$vMonth,$vDay)
	{
		$vDay=(int)$vDay;
		$lvsql="select * from  hr_lv0111 Where lv006='$vMonth' and lv007='$vYear' and lv003=IF($vDay<16,1,2)";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadMonthYear($vMonth,$vYear)
	{
		$lvsql="select * from  hr_lv0111 Where lv006='$vMonth' and lv007='$vYear'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  hr_lv0111 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadRunID($vlv001)
	{
		$lvsql="select * from  hr_lv0111 Where lv001='$vlv001' and lv011<=2";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  hr_lv0111 Where lv011=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_InsertAuto()
	{
		...
		$lvsql="insert into hr_lv0111(lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv098,lv099) 
		values('".sof_escape_string($this->lv001)."','".sof_escape_string($this->lv002)."','".sof_escape_string($this->lv003)."','".sof_escape_string($this->lv004)."','".sof_escape_string($this->lv005)."','".sof_escape_string($this->lv006)."','".sof_escape_string($this->lv007)."','$this->LV_UserID',now(),'".sof_escape_string($this->lv098)."','".sof_escape_string($this->lv099)."')";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0111.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_LoadMaOld($lv099)
	{
		$lvsql="select lv001 from  hr_lv0111 Where lv099='$lv099'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_GetDataAuto()
	{
      ...
      ,[BUDGET_ACTIVE] lv003
      ,[BUDGET_REMARK] lv004
      ,[BudgetOfthisAC1] lv005
      ,[ApprovedRFA] lv006
      ,[RemaindBudget] lv007
      ,[BudgetOfthisAC] lv098
  ...
  where 1=1 $vCondition";
	...
	{
		if($this->isAdd==0) return false;
		$lvsql="insert into hr_lv0111(lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0111.insert',sof_escape_string($lvsql));
		}
	...
	function LV_Update()
	{
		...
		$lvsql="Update hr_lv0111 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->LV_UserID',lv009=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0111.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0111 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0111.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0111 set lv003=1  WHERE hr_lv0111.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0111.approval',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0111 set lv003=0  WHERE hr_lv0111.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0111.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0111 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lstArr=$this->gestort($lstArr,$lstOrdArr);
		$strSort="";
		switch($lvSortNum)
		{
			case 0:
		...
		<tr class=\"cssbold_tab\"><td colspan=\"".(count($lstArr))."\">".$this->TabFunction($lvFrom,$lvList,$maxRows)."</td><td align=right>".$this->ListFieldExport($lvFrom,$lvList,$maxRows)."</td><td align=right>".$this->ListFieldSave($lvFrom,$lvList,$maxRows,$lvOrderList,$lvSortNum)."</td></tr>
		@#01
		...
		$sqlS = "SELECT * FROM hr_lv0111 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		...
		/*	if($vrow['lv011']==1)		$strTr=str_replace("@#04","lvlineapproval",$strTr);
			elseif($vrow['lv011']==2)		$strTr=str_replace("@#04","lvlineapproval_level2",$strTr);
			elseif($vrow['lv011']==3)		$strTr=str_replace("@#04","lvlineapproval_level3",$strTr);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."hr_lv0111/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv001)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
function LV_BuilPopup($lvList,$vArrCot,$bResult,$objid,$objvalue)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{
					case 'lv001':
						$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$vArrCot[$lstArr[$i]]],(int)$this->ArrView[$lstArr[$i]])),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdNo),(int)$this->ArrView[$lstArr[$i]]));
						break;
					default:		
	...
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM hr_lv0111 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function gestort($vArr1,$vArr2)
	{
		$vReturn=array();
		$vGetArr=array();
		...
		for($i=0;$i<count($vArr1);$i++)
		{
			$pos=0;
			$vTem=$vGetArr[$i];
			for($j=0;$j<count($vGetArr);$j++)
			{
				if($vTem>$vGetArr[$j])$pos++;
			}
				while(true)
				{
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0100 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
