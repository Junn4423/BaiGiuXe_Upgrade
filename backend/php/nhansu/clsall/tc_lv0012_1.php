...
////////////////////GetDate
	public $DateDefault="11800-01-01";
	public $DateCurrent="11800-01-01";
	public $Count=null;
	public $paging=null;
	public $lang=null;
...
////////////
	...
	function LV_UpdateState($vEmployeeID,$vDate,$vTime,$vState,$vUserID='')
	{
		...
		$lvsql="Update tc_lv0012 set lv004='I',lv006='$vState',lv007='$vUserID' where  lv001='$vEmployeeID' and lv002='$vDate' and lv003='$vTime';";
		$vReturn= db_query($lvsql);
		if($vReturn) {
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0012.update',sof_escape_string($lvsql));
		}
	...
	{
		if($this->LV_CheckLocked($vDate)==FALSE) return false;
		$lvsql="Update tc_lv0012 set lv004='OTO',lv007='$vUserID' where  lv001='$vEmployeeID' and lv002='$vDate' and lv003='$vTime';";
		$vReturn= db_query($lvsql);
		if($vReturn) {
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0012.update',sof_escape_string($lvsql));
		}
	...
	{		
		if($this->LV_CheckLocked($vDate)==FALSE) return false;
		$lvsql="insert into tc_lv0012 (lv001,lv002,lv003,lv004,lv006,lv005) values('$vEmployeeID','$vDate','$vTime','I','$vState','$vUserID')";
		$vReturn= db_query($lvsql);
		if($vReturn){
	...
	function LV_InsertXML($vEmployeeID,$vDate,$vTime,$vState,$vUserID='')
	{		
		...
		{
			if(trim($Time)!="" && $Time!="00:00" && $Time!=NULL)
			{
			$lvsql="insert into tc_lv0012 (lv001,lv002,lv003,lv004,lv006,lv005) values('$vEmployeeID','$vDate','$Time','I','$vState','$vUserID')";
			$vReturn= db_query($lvsql);
			$lvsqllog=$lvsqllog.$lvsql.";";
			}
		}
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'tc_lv0009.insert',sof_escape_string($lvsqllog));
		 }
		return $vReturn;
	}
	function LV_Insert()
	...
	{
		if(getday($vDateWork)=='01')
		{
			if(getmonth($vDateWork)=='01')
				return $vPreDay=(getyear($vDateWork)-1)."-12-31";	
		}
		$vPreDay=getyear($vDateWork)."-".getmonth($vDateWork)."-".(Fillnum((int)getday($vDateWork)-1,2));
		return $vPreDay;
	}
	...
	//H�m ki?m tra xem t?t c? gi? v�o ra di?u 
	function  CheckInShiftFirst($vArrTime,$vArrShift,$vListShiftReceive)
	{
		//X�c d?nh gi? d?u, gi? k?t th�c.
		$vCurArrTime=Array();
		...
		if($vCount==2)
		{
			...
			if($vArrReturn['CaIn']!=$vArrReturn['CaOut']) return 1;
		...
		elseif($vCount==1)
		{
			...
			$vShift=$this->ConfirmShift($vArrShift,$vStartTime,$vEndTime,$vListShiftReceive);
			$vArrReturn['CaIn']=$vShift['In'];
			if($vArrReturn['CaIn']=='CaNS1' || $vArrReturn['CaIn']=='CaNS2') return 1;
	...
	function ConfirmShift($vArrShift,$vStartTime,$vEndTime,$vListShiftReceive)
	{
		//tr? v? 2 tr?ng th�i
		//Ca l�m vi?c
		//T? gi? starttime x�c d?nh v? tr� ca.
		$vShift=Array();
		...
		///Ca cu?i x�c d?nh c�ng v� c� tang ca kh�ng					
		$vShift['In']=$vShiftIn;
		$vShift['Out']=$vShiftOut;
		return $vShift;
		//C�ng c?a c�ng vi?c 0 kh�ng c� tang ca, 1 c� tang ca.
	}
	function LV_GetListTime_Arr($vDateWork,$vArrShift,$vListShiftReceive,$lineprocess='',$vCheckNT)
	{
		...
		if($vCheckNT==1)
		{
			if(getmonth($vDateWork)==12)
			{
				$vDateNext=(getyear($vDateWork)+1)."-01-01";
			}
			else
			{
				$vDateNext=(getyear($vDateWork))."-".Fillnum(getmonth($vDateWork)+1,2)."-01";
			}
			$vDatePre=getyear($vDateWork)."-".getmonth($vDateWork)."-01";
	...
	function LV_GetListTimeA($vEmployeeID,$vDateWork,$vArrShift,$vListShiftReceive,$vShiftID,$vopt=0)
	{
		...
		if($this->ArrGioVaoRa[$vEmployeeID][$vDateWorkPre]!=NULL)
		{
			...
			$vTruoc=true;
				...
				if($this->LV_GetSecond($vArrReturn[2][$i-1][0],$vrow['lv003'])>1800 || $i==0)
				{
		...
		if($this->ArrGioVaoRa[$vEmployeeID][$vDateWorkNext]!=NULL)
		{
			...
			{
				if($this->LV_GetSecond($vArrReturn[1][$i-1][0],$vrow['lv003'])>1800 || $i==0)
				{
		...
		{
			if($this->LV_GetSecond($vArrReturn[0][$j-1][0],$vrow['lv003'])>1800 || $i==0)
			{
					if($vopt!=1)
					{
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."tc_lv0012/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
