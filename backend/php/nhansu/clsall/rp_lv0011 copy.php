...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		$vsql="select lv002,lv006 from hr_lv0036 where (lv006>='$vStartDate 00:00:00' and lv006<='$vEndDate 23:59:59') and lv004='QCC'";
	...
	function LV_CheckQCCDon($vStartDate,$vEndDate)
	{
		...
		$vsql="select lv015,lv016,lv017,DATEDIFF(lv017,lv016) SoNgay,IF('$vEndDate 23:59:59'>lv017,DATEDIFF(lv017,lv016),DATEDIFF('$vEndDate',lv016)) SoNgayTinh from jo_lv0004 where (lv016>='$vStartDate 00:00:00' and lv016<='$vEndDate 23:59:59') and lv003='QCC' and lv021=1";
		$bResult=db_query($vsql);
...
//////////////////////Buil list////////////////////
	...
	{	
		...
		//	if($vrow['lv004']=='2020-02-03') echo $vTimeLate;
			...
			{
				if($vrow['lv008']=='1/2P' || $vrow['lv008']=='1/2KH' || $vrow['lv008']=='1/2KL' || $vrow['lv008']=='1/2KP'  || $vrow['lv008']=='1/2PB')
				{
					...
					if($vrow['lv008']=='CT')
					{
						...
						{
							if($vrow['lv007']!='1/2KL' && $vrow['lv007']!='1/2KP') $this->ArrTCEmp[$vrow['NVID']]['TI:X']=$this->ArrTCEmp[$vrow['NVID']]['TI:X']+1-$this->GetTime($vrow['lv005'])/$vTimeWorks;
						}
					}
					//$this->ArrTCEmpShowThem[$vrow['NVID']][$vrow['lv004']]=$vrow['lv008'];
				}
			...
			foreach($lvparatimecard as $lvgt)
			{
				...
				switch ($lvgt)
				{
					...
					case 4:
						...
						$vlv002=$vrow['lv004'];
						$vTime='';
						if($_GET['childfunc']!='excel' && $vrow['lv007']!='X' && $vrow['lv007']!='')
						{
							$vTenNguoiSua='';
	...
	function Get_BuildList_ArrayTDL_Duyet($sExport,$lvparatimecard)
	{
		...
		for($i=1;$i<=count($this->ArrEmp);$i++)
		{
			...
			$tNCVP=(float)$this->ArrTCEmp[$this->ArrEmp[$i][0]]['TI:X']+$vQCC_1;
			...
			//SỐ lần đi trễ
			$vSoLanDiTre=$this->ArrTCEmp[$this->ArrEmp[$i][0]]['LateSoon']- $this->ArrTCEmp[$this->ArrEmp[$i][0]]['isLateSoonPhep'];
			//Số tiền đi trễ
			...
			{
				$vKPBiTru=$this->ArrTCEmp[$this->ArrEmp[$i][0]]['KP']+$this->ArrTCEmp[$this->ArrEmp[$i][0]]['1/2KP']*0.5;
			}
			foreach($lvparatimecard as $lvgt)
			{
				switch($lvgt)
				{
					case 4:
						...
						<td align=center $vcolor>".($this->ArrTCEmp[$this->ArrEmp[$i][0]]['KP']+$this->ArrTCEmp[$this->ArrEmp[$i][0]]['1/2KP']*0.5)."</td>
						...
						<td align=center>".round($vSoDonXinDiTre,2)."</td>
	...
	function Get_BuildList_ArrayTDLLimited($sExport,$lvparatimecard)
	{
		...
		for($i=1;$i<=count($this->ArrEmp);$i++)
		{
			$tNCVP=(float)$this->ArrTCEmp[$this->ArrEmp[$i][0]]['TI:X'];
			$tNCCT=(float)$this->ArrTCEmp[$this->ArrEmp[$i][0]]['TI:CT'];
	...
	function Get_BuildList_ArrayTDL($sExport,$lvparatimecard)
	{
		...
		for($i=1;$i<=count($this->ArrEmp);$i++)
		{
			...
			if($this->ArrTCEmp[$this->ArrEmp[$i][0]]['Num_FN_F']>12)
			{
	...
	function GetTimeListArr($vlv001,$vlv002,$shiftDay,$shiftYear,&$passday,$vListShiftReceive,$vPreShift,&$arrTime,$lvopt='0')
	{
		...
		if($lvopt==0)
		{
			...
			if($vresult){
				...
				while($vrow=db_fetch_array($vresult))
				{
					if($this->LV_GetSecond($vTimeSave,$vrow['lv003'])>900 || $i==0)
					{
