<?php
///////////coding ki_lv0006///////////////
class   ki_lv0006 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"1","lv005"=>"1","lv006"=>"1","lv007"=>"1","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0");	
	...
	{
		$lvsql="select lv007 from ki_lv0005 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select lv001 from  ki_lv0006 Where lv002='$vParentID' and lv003='$vCustomerID'";
	...
	function LV_CheckExistStaff($vParentID,$vCustomerID)
	{
		$lvsql="select lv001 from  ki_lv0006 Where lv002='$vParentID' and lv003='$vCustomerID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
	...
	{
		$lvsql="select * from  ki_lv0006 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
	    $lvsql="insert into ki_lv0006 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv015','$this->lv016')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrnt,'ki_lv0006.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function LV_Update()
	{
		...
		$lvsql="Update ki_lv0006 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015'  where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ki_lv0006  WHERE ki_lv0006.lv001 IN ($lvarr) and (select lv007 from ki_lv0005 B where  B.lv001= ki_lv0006.lv002)<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0006 set lv016=1  WHERE ki_lv0006.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_AprovalOne($timeid)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0006 set lv016=1  WHERE ki_lv0006.lv001 ='$timeid'  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0006.approval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0006 set lv016=0,lv021=0  WHERE ki_lv0006.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ki_lv0006 WHERE 1=1 ".$this->GetCondition();
	...
	{
		if($opt==0)
			$lvsql="select sum(lv005) convertmoney from ki_lv0006 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv003='".$vEmployeeID."')";
		else	
			$lvsql="select sum(lv006) convertmoney from ki_lv0006 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv004='".$vEmployeeID."')";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ki_lv0006 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$slv005=$slv005+$vrow['lv005'];
			$slv006=$slv006+$vrow['lv006'];
			$slv007=$slv007+$vrow['lv007'];
			for($i=0;$i<count($lstArr);$i++)
			{
		...
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv004-->",$this->FormatView($slv004,10),$strF);
		$strF=str_replace("<!--lv005-->",$this->FormatView($slv005,10),$strF);
		$strF=str_replace("<!--lv006-->",$this->FormatView($slv006,10),$strF);
		$strF=str_replace("<!--lv007-->",$this->FormatView($slv007,10),$strF);
...
//////////////////////Buil list////////////////////
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ki_lv0006/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM ki_lv0006 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ki_lv0002";
				break;
			default:
				$vsql="";
				break;
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ki_lv0002 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
