<?php
//4//////////coding ki_lv0004///////////////
class   ki_lv0004 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='ki_lv0004';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"1","lv005"=>"0","lv006"=>"0");	
	...
	{
		$lvsql="select lv007 from ki_lv0003 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select lv001 from  ki_lv0004 Where lv002='$vParentID' and lv003='$vCustomerID'";
	...
	function LV_CheckExistStaff($vParentID,$vTargetKPI)
	{
		$lvsql="select lv001 from  ki_lv0004 Where lv002='$vParentID' and lv003='$vTargetKPI'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  ki_lv0004 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
	    $lvsql="insert into ki_lv0004 (lv002,lv003,lv004,lv005,lv006) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrnt,'ki_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertTemp()
	{
		...
		$lvsql="insert into ki_lv0004 (lv001,lv002,lv003,lv004,lv005,lv006) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_Update()
	{
		...
		$lvsql="Update ki_lv0004 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ki_lv0004  WHERE ki_lv0004.lv001 IN ($lvarr) and (select lv007 from ki_lv0003 B where  B.lv001= ki_lv0004.lv002)<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0004 set lv011=1  WHERE ki_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_AprovalOne($timeid)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0004 set lv011=1  WHERE ki_lv0004.lv001 ='$timeid'  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.approval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0004 set lv011=0  WHERE ki_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ki_lv0004.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ki_lv0004 WHERE 1=1 ".$this->GetCondition();
	...
	{
		if($opt==0)
			$lvsql="select sum(lv005) convertmoney from ki_lv0004 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv003='".$vEmployeeID."')";
		else	
			$lvsql="select sum(lv006) convertmoney from ki_lv0004 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv004='".$vEmployeeID."')";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
...
function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
{
	...
	$sqlS = "SELECT * FROM ki_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
	$vorder=$curRow;
	...
	while ($vrow = db_fetch_array ($bResult)){
		$strL="";
		$vorder++;
		$vsumtytrong = (float)($vsumtytrong ?? 0) + (float)($vrow['lv005'] ?? 0);
		for($i=0;$i<count($lstArr);$i++)
		{
	...
	$strF=$strF."</tr>";
	$strF=str_replace("<!--lv005-->",$this->FormatView($vsumtytrong,10),$strF);
...
//////////////////////Buil list////////////////////
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ki_lv0004/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vSortNum)
{
	...
	$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
	$strF="<tr><td colspan=\"2\">&nbsp;</td>";
	$sqlS = "SELECT * FROM ki_lv0004 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
	$vorder=$curRow;
	$bResult = db_query($sqlS);
	$this->Count=db_num_rows($bResult);
	$strTrH="";
	...
	while ($vrow = db_fetch_array ($bResult)){
		$strL="";
		$vorder++;
		$vsumtytrong=$vsumtytrong+$vrow['lv005'];
		for($i=0;$i<count($lstArr);$i++)
		{
			$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
			$strL=$strL.$vTemp;
