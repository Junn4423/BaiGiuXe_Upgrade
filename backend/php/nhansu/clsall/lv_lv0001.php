<?php
/////////////coding lv_lv0001///////////////
class   lv_lv0001 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  lv_lv0001 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into lv_lv0001 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0001.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update lv_lv0001 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0001.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM lv_lv0001  WHERE lv_lv0001.lv001 IN ($lvarr)";// and (select count(*) from lv_lv0001 B where  B.lv002= lv_lv0001.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0001.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
...
////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM lv_lv0001 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM lv_lv0001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		*/
		...
		while ($vrow = db_fetch_array($bResult)) {
				...
				for ($i = 0; $i < count($lstArr); $i++) {
					...
					// Kiểm tra kỹ để không gây lỗi nhưng vẫn giữ dữ liệu
					$value  = isset($vrow[$key]) ? $vrow[$key] : '';
					$format = isset($this->ArrView[$key]) ? (int)$this->ArrView[$key] : 0;

					$formatted = $this->FormatView($value, $format);
					$linkVal   = $this->getvaluelink($key, $formatted);
					$aligned   = $this->Align($lvTd, $format);

					$vTemp = str_replace("@02", $linkVal, $aligned);
					$strL .= $vTemp;
				}

				// Tránh lỗi nếu thiếu 'lv001'
				$lv001 = isset($vrow['lv001']) ? $vrow['lv001'] : '';

				$strTr .= str_replace(
					"@#01", $strL,
					str_replace(
						"@02", $lv001,
						str_replace("@03", $vorder,
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."lv_lv0001/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM lv_lv0001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,CONCAT(lv002,'[',lv003,']') lv002,IF(lv001='$vSelectID',1,0) lv003 from  lv_lv0007	";
				break;
		}
		return $vsql;
	}
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		if (
		isset($this->ArrGetValueLink[$vFile]) &&
		isset($this->ArrGetValueLink[$vFile][$vSelectID]) &&
		isset($this->ArrGetValueLink[$vFile][$vSelectID][0]) &&
		isset($this->ArrGetValueLink[$vFile][$vSelectID][1])
		) {
		...
		switch($vFile)
		{
			...
			case 'lv102':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from lv_lv0007 	 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
