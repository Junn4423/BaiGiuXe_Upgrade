...
class   sl_lv0059 extends lv_controler
{
	...
	public $lv009=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv007,lv008,lv009,lv010,lv011,lv012,lv099,lv005,lv006";	
////////////////////GetDate	
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"4","lv004"=>"4","lv005"=>"0","lv006"=>"22","lv007"=>"0","lv008"=>"0","lv009"=>"10","lv010"=>"10","lv011"=>"10","lv012"=>"0","lv099"=>"0");	
	...
	{
		$lvsql="select A.*,count(A.lv001) NumLine from  sl_lv0059 A  where A.lv008=1 and (date(A.lv003)<=CURRENT_DATE() and date(A.lv004)>=CURRENT_DATE()) and (time(A.lv003)<=CURRENT_TIME() and time(A.lv004)>=CURRENT_TIME())";
	...
	{
		$lvsql="select * from  sl_lv0059 Where concat(',',lv099,',') like '%,$vlv022,%' and lv008=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_GetProcessBOM($vPlanID,$vBOMID)
	{
		$vArrHoaHong=Array(0=>12,1=>10,2=>25);
		//Chọn ưu tiên 1:
		$lvsql="select A.lv004,A.lv005,A.lv006 from sl_lv0060 A inner join sl_lv0059 B on A.lv002=B.lv001 Where  A.lv003='$vBOMID' and B.lv008=1 and B.lv012='2'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vArrHoaHong[0]=$vrow['lv004'];
			$vArrHoaHong[1]=$vrow['lv005'];
			$vArrHoaHong[2]=$vrow['lv006'];
			return $vArrHoaHong;
		}
		//Chọn ưu tiên 2:
		//Mặc định 
		$lvsql="select B.lv009,B.lv010,B.lv011 from sl_lv0059 B Where  concat(',',lv099,',') like '%,$vPlanID,%' and lv008=1  and lv012='1'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vArrHoaHong[0]=$vrow['lv009'];
			$vArrHoaHong[1]=$vrow['lv010'];
			$vArrHoaHong[2]=$vrow['lv011'];
			return $vArrHoaHong;
		}
		//Chọn ưu tiên 3:
		$lvsql="select B.lv009,B.lv010,B.lv011 from sl_lv0059 B Where  lv008=1  and lv012='0'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vArrHoaHong[0]=$vrow['lv009'];
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into sl_lv0059 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv009,lv010,lv011,lv012,lv099) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv099')";
	...
	function LV_Update()
	{
		...
		$this->lv003 = ($this->lv003!="")?recoverdate(($this->lv003), $this->lang)." ".gettime($this->lv003):$this->DateDefault;
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang)." ".gettime($this->lv004):$this->DateDefault;
		$lvsql="Update sl_lv0059 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv006='$this->lv006',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv099='$this->lv099' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv007':
				...
				break;	
			case 'lv012':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0062";
				break;
			case 'lv099':
				$vsql="select lv001,lv002 lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0111";
				break;	
		}
		return $vsql;
	}
