...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0");	
	...
	{
		$lvsql="select * from  hr_lv0210 Where lv001='$vlv001'";
	...
	function GetBuilCheckShift($vListID,$vID,$vTabIndex,$vTbl,$vFieldView='lv002')
	{
		...
		";
		$vsql="select * from  ".$vTbl." where lv001!='' order by lv015";
		$strGetList="";
		$strGetScript="";
		$i=0;
		$vresult=db_query($vsql);
	...
	function LV_CheckSelft($vUserID,$vContractID)
	{
		$lvsql="select count(*) num from  sl_lv0013 Where lv001='$vContractID' and lv010='$vUserID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadUserID($vlv002)
	{
		...
		$lvsql="select lv001,lv005,lv003 from  hr_lv0210 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
	...
	function LV_LoadUserArray($vlv002)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select * from  hr_lv0210 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
		return $vresult;
	}
	...
	function LV_LoadStep($vlv007='sovanban',$vlv008='tokhai',$vlv009='bbbg',$vCodeID)
	{
		...
		$lvsql="select * from  hr_lv0210 Where lv007='$vlv007' and lv008='$vlv008' and lv009='$vlv009'";
		$vresult=db_query($lvsql);
		if($this->lang=='VN') $btxoa="Xóa";
		else $btxoa="Delete";
		while($vrow = db_fetch_array ($vresult))
	...
	function LV_LoadStepCheck($vlv007)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select lv001 from  nhansu_sof_documents_v4_0.hr_lv0210 Where lv007='$vlv007' ";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		if($vrow)
		{
	...
	function LV_InsertAuto($vViTri,$vHinh)
	{
		...
		$vField='lv008';
		$lvsql="insert into nhansu_sof_documents_v4_0.hr_lv0210 (lv002,lv003,lv004,lv005,lv006,lv007,$vField) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','".sof_escape_string($vHinh)."')";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->lv001=sof_insert_id();
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0210.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_UpdateAuto($vKetQua,$vViTri,$vHinh)
	{
		...
		$vField='lv008';
		$lvsql="Update nhansu_sof_documents_v4_0.hr_lv0210 set $vField='".sof_escape_string($vHinh)."' where lv001='$vKetQua'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Insert()
	{
		if($this->isAdd==0) return false;
		$this->lv008=$this->LV_UserID;
		$lvsql="insert into hr_lv0210 (lv001,lv002,lv003,lv004,lv005,lv007,lv008,lv009,lv010) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv007','$this->lv008','$this->lv009',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		$this->lv008=$this->LV_UserID;
		 $lvsql="Update hr_lv0210 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM hr_lv0210  WHERE lv001 IN ($lvarr)";// and (select count(*) from hr_lv0210 B where  B.lv002= hr_lv0210.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0210.delete',sof_escape_string($lvsql));
			$lvsql = "DELETE FROM nhansu_sof_documents_v4_0.hr_lv0210  WHERE lv007 IN ($lvarr)";// and (select count(*) from hr_lv0221 B where  B.lv002= hr_lv0221.lv001)<=0  ";
			$vReturn= db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'nhansu_sof_documents_v4_0.hr_lv0210.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update hr_lv0210 set lv006=1  WHERE hr_lv0210.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isUnApr==0) return false;
		$lvsql = "Update hr_lv0210 set lv006=0  WHERE hr_lv0210.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_DeleteUser($vUserID)
	{
	if($this->isDel==0) return false;
		$lvsql = "DELETE FROM nhansu_sof_documents_v4_0.hr_lv0210  WHERE hr_lv0210.lv002='$vUserID'";// and (select count(*) from hr_lv0210 B where  B.lv002= hr_lv0210.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0210.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	////////GetFile Attachment/////////
	function LV_GetForward($AttachFile,$AttachFileHidden)
	{
		...
		{
			$strTemp=$strTr;
			$this->lv001=InsertWithCheckExt('hr_lv0210', 'lv001', '',0);
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0210 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0210 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{
					$lvTdTextBox="
					<td align=@#05 rowspan=\"@#99\">
						<input type=\"checkbox\" style=\"width:35px;text-align:center;\" tabindex=\"2\" class=\"norequired\" name=\"txtlv004_".$vrow['lv001']."\" id=\"txtlv004_".$vrow['lv001']."\" onclick=\"UpdateText(this,'".$vrow['lv001']."',4)\" ".(($vrow['lv004']==1)?'checked="true"':'')."/>
					</td>";
					$vTemp=str_replace("@02",$this->FormatView((($vrow[$lstArr[$i]]=='')?'':$vrow[$lstArr[$i]]),(int)$this->ArrView[$lstArr[$i]]),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]]));	

				}
				elseif($lstArr[$i]=='lv005')
				{
					$lvImg="<a target='_blank' href='hr_lv0211/readfile.php?ContractID=".$vrow['lv001']."&type=8&size=0'>Download File<br/></a>";
					$vTempEnter='
					<table valign=\"top\" style=\"width:100%\">
						...
						<iframe  height=28 width="100%" marginheight=0 marginwidth=0 frameborder=0 src="hr_lv0211/?childfunc=upload&ViTriUp=8&ContractID='.$vrow['lv001'].'&lang='.$this->lang.'"></iframe>
						</div>
						</td>
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0211/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0210 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
