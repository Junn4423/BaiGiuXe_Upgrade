<?php
/////////////coding tc_lv0023///////////////
class   tc_lv0023 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='tc_lv0023';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv020"=>"21","lv021"=>"22");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"10","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"4","lv012"=>"0","lv013"=>"2");	
	...
	{
		$lvsql="select * from  tc_lv0023 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$this->lv011 = ($this->lv011!="")?recoverdate(($this->lv011), $this->lang):$this->DateCurrent;
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang):$this->DateCurrent;
		echo $lvsql="insert into tc_lv0023 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv011,lv012,lv013) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv011','$this->lv012','$this->lv013')";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'tc_lv0023.insert',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_InsertMulti($lvarr,$lv_lv003,$lv_lv004,$lv_lv005,$lv_lv006,$lv_lv007,$lv_lv008,$lv_lv009)
	{
		if($this->isAdd==0) return false;
		$lv_array=explode(",",$lvarr);
		foreach ($lv_array as $lv)
		{
			if(trim($lv)!="")
			{
		 $lvsql="insert into tc_lv0023 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009) values('$this->lv001','$lv','$lv_lv003','$lv_lv004','$lv_lv005','$lv_lv006','$lv_lv007','$lv_lv008','$lv_lv009')";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'tc_lv0023.insert',sof_escape_string($lvsql));
		 }
	...
	function LV_Update()
	{
		...
		$this->lv013 = ($this->lv013.''!="")?recoverdate(($this->lv013), $this->lang):$this->DateCurrent;
		$lvsql="Update tc_lv0023 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv009='$this->lv009',lv012='$this->lv012',lv013='$this->lv013' where  lv001='$this->lv001' AND lv008<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) {
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0023.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM tc_lv0023  WHERE tc_lv0023.lv008<=0 AND tc_lv0023.lv001 IN ($lvarr)";// and (select count(*) from ts_lv0009 B where  B.lv002= tc_lv0023.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0023.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update tc_lv0023 set lv008=1  WHERE tc_lv0023.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0023.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update tc_lv0023 set lv008=0  WHERE tc_lv0023.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0023.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	{
		$strCondi="";
		if($this->LISTID!="") $strCondi=$strCondi." and (A.lv001  in ($this->LISTID))";
		...
		if($this->lv010!="") $strCondi=$strCondi." and B.lv029  = '$this->lv010'";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM tc_lv0023 A WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,B.lv029 lv010,'______________________' lv014 FROM tc_lv0023 A inner join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vSumTongTien=$vSumTongTien+$vrow['lv005'];
			for($i=0;$i<count($lstArr);$i++)
			{
		...
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv005-->",$this->FormatView($vSumTongTien,10),$strF);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."tc_lv0023/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vSortNum)
	{
		...
		$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
		$strF="<tr><td colspan=\"2\">&nbsp;</td>";
		$sqlS = "SELECT A.*,B.lv029 lv010,'______________________' lv011 FROM tc_lv0023 A inner join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{
					case 'lv010':
							if(strpos($strDepart,$vrow['lv010'])===false)
							{
								...
								$strDepart=$strDepart.$vrow['lv010']."@";
							}
							else
							{
	...
	//////////////////////Buil list////////////////////
	function LV_BuilListReport_Option($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM tc_lv0023 A WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportDepart($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{

		$this->DefaultFieldList=$lvList;
		if($lvList=="") $lvList=$this->DefaultFieldList;
		if($this->isView==0) return false;
		$lstArr=explode(",",$lvList);
		$lstOrdArr=explode(",",$lvOrderList);
		...
		$lvTable="<!--<div align=\"center\"><img  src=\"".$this->GetLogo()."\" style=\"max-width:1024px\" /></div>-->
		<div align=\"center\"><h1>".($this->ArrPush[0])."</h2></div>
		<table  align=\"center\" class=\"lvtable\" border=1 style=\"width:700px!important;\">
		@#01
		@#02
		</table>
		";
		...
		";
		$strF="<tr><td ><strong>Tổng:<strong></td>";
		$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT B.lv029 lv001,B.lv005 lv020,'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' lv021,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv008 FROM tc_lv0023 A left join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1  ".$this->GetCondition()." Order by B.lv029 ASC LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM tc_lv0023 A WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv003':
				...
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0018";
				break;
			case 'lv006':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv003':
				...
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0018 where lv001='$vSelectID'";
				break;
			case 'lv006':
