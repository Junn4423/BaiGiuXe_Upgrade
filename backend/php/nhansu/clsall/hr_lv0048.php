<?php
/////////////coding hr_lv0048///////////////
class   hr_lv0048 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv019=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv010,lv011,lv012,lv013,lv014,lv016,lv017,lv018,lv019,lv009";	
////////////////////GetDate
	public $DateCurrent="1900-01-01";
	public $Count=null;
	public $paging=null;
	public $lang=null;
	protected $objhelp='hr_lv0048';
	public $Dir="";
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"10","lv008"=>"0","lv009"=>"0","lv010"=>"10","lv011"=>"10","lv012"=>"10","lv013"=>"10","lv014"=>"0","lv015"=>"1","lv016"=>"1","lv017"=>"1","lv018"=>"2","lv019"=>"10");	
	...
	{
		$lvsql="select lv006 from hr_lv0044 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  hr_lv0048 Where lv001='$vlv001'";
	...
	function LV_LoadTemplate($vlv001)
	{
		...
		$strReturn="";
		$lvsql="select A.*,B.lv007 lv014 from  hr_lv0048 A left join hr_lv0044 B on A.lv002=B.lv001 Where A.lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Update()
	{
		...
		$this->lv018 = ($this->lv018!="")?recoverdate(($this->lv018), $this->lang):$this->DateDefault;
		 $lvsql="Update hr_lv0048 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016',lv017='$this->lv017',lv018='$this->lv018',lv019='$this->lv019' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0048.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0048  WHERE hr_lv0048.lv001 IN ($lvarr) and (select lv006 from hr_lv0044 B where  B.lv001= hr_lv0048.lv002)<=0";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0048.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UpdateOther()
	{
	    ...
	    $lvsql="Update hr_lv0048 set lv005='$this->lv005',lv009='$this->lv009' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_CheckLocked($vlv002)
	{
		$lvsql="select lv006 from  hr_lv0044 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0048 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0048 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
//////////////////////Buil list////////////////////	
	function LV_BuilListInput($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td >@02";
		$lvTdInput="<td ><input id=\"@03\" name=\"@03\" value=\"@02\" tabindex=2 ></td>";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{
					case 'lv009':
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),str_replace("@03","txt".$lstArr[$i].$vorder,$lvTdInput));
					break;
					case 'lv001':
					case 'lv002':
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0048/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM hr_lv0048 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv003':
				...
				break;
			case 'lv005':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0019";
				break;
			case 'lv014':
	...
	public  function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv003':
				...
				break;
			case 'lv005':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0019 where lv001='$vSelectID'";	
				break;
			case 'lv014':
