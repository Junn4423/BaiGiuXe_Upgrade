	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				$key = $lstArr[$i];
				$value = $vrow[$key] ?? null;
				$viewType = (int)($this->ArrView[$key] ?? 0); // dùng 0 nếu không có kiểu view

				$formattedValue = $this->FormatView($value, $viewType);
				$aligned = $this->Align($lvTd, $viewType);
				$vTemp = str_replace("@02", $this->getvaluelink($key, $formattedValue), $aligned);
				$strL=$strL.$vTemp;
			}
