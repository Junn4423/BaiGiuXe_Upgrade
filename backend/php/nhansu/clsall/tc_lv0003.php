<?php
/////////////coding tc_lv0003///////////////
class   tc_lv0003 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"2","lv003"=>"0","lv004"=>"0");
	...
	function LV_LoadDayIDArr($vlv002)
	{
		$lvsql="select * from  tc_lv0003 Where year(lv002)='".getyear($vlv002)."' and month(lv002)='".getmonth($vlv002)."'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$this->ArrDays[$vrow['lv002']][0]=true;
			$this->ArrDays[$vrow['lv002']]['lv001']=$vrow['lv001'];
			$this->ArrDays[$vrow['lv002']]['lv002']=$vrow['lv002'];
			$this->ArrDays[$vrow['lv002']]['lv003']=$vrow['lv003'];
			$this->ArrDays[$vrow['lv002']]['lv004']=$vrow['lv004'];
		}
		
	}
	function LV_LoadDayIDArrFromTo($vFrom,$vTo)
	{
		$lvsql="select * from  tc_lv0003 Where (lv002)>='$vFrom' and lv002<='$vTo'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$this->ArrDays[$vrow['lv002']][0]=true;
	...
	function LV_LoadDayID($vlv002)
	{
		if($this->ArrDays[$vlv002][0]==true)
		{
			$this->lv001=$this->ArrDays[$vlv002]['lv001'];
			$this->lv002=$this->ArrDays[$vlv002]['lv002'];
			$this->lv003=$this->ArrDays[$vlv002]['lv003'];
			$this->lv004=$this->ArrDays[$vlv002]['lv004'];
		}
		else
		{
			$this->lv001=NULL;
	...
	{
		$lvsql="select * from  tc_lv0003 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$this->lv002 = ($this->lv002!="")?recoverdate(($this->lv002), $this->lang):$this->DateDefault;
		$lvsql="insert into tc_lv0003 (lv001,lv002,lv003,lv004) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0003.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$this->lv002 = ($this->lv002!="")?recoverdate(($this->lv002), $this->lang):$this->DateDefault;
		 $lvsql="Update tc_lv0003 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0003.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM tc_lv0003  WHERE tc_lv0003.lv001 IN ($lvarr)";// and (select count(*) from tc_lv0003 B where  B.lv002= tc_lv0003.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0003.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM tc_lv0003 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM tc_lv0003 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."tc_lv0003/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM tc_lv0003 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv004':
				$vsql="select lv001,lv002 lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0002 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
