<?php
/////////////coding ts_lv0001///////////////
class   ts_lv0001 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  ts_lv0001 Where lv001='$vlv001'";
	...
	function LV_ApprovalOne($vlv003)
	{
		...
		select A.*,
		(select sum(A1.lv004) from ts_lv0009 A1 inner join ts_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009<'$vDateStart' and A11.lv002='$vlv003') ReReceiptQty,
		(select sum(A1.lv004*A1.lv008) from ts_lv0009 A1 inner join ts_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009<'$vDateStart' and A11.lv002='$vlv003') PriceReReceiptQty,
		(select sum(A1.lv004) from ts_lv0009 A1 inner join ts_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd 23:00:00' and A11.lv002='$vlv003') InReceiptQty,
		(select sum(A1.lv004*A1.lv008) from ts_lv0009 A1 inner join ts_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd 23:00:00' and A11.lv002='$vlv003') PriceInReceiptQty,
		(select sum(A2.lv004) from ts_lv0011 A2 inner join ts_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009<'$vDateStart' and A21.lv002='$vlv003') ReOutlv004 ,
		(select sum(A2.lv004) from ts_lv0011 A2 inner join ts_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009>='$vDateStart' and A21.lv009<='$vDateEnd 23:00:00' and A21.lv002='$vlv003') InOutlv004   
		from ts_lv0012 A inner join sl_lv0007 B on A.lv002=B.lv001 where A.lv003='$vlv003'  and B.lv015=0 ) MP";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$sql="update ts_lv0012 set lv004='".$vrow['sl1']."',lv006='".$vrow['sl2']."',lv008='".$vrow['Price']."' where lv002='".$vrow['lv002']."' and lv003='".$vlv003."'";
			db_query($sql);
		}
		$vsql="delete from ts_lv0012 where (lv002 not in (select A.lv003 from ts_lv0009 A)) and (lv002 not in (select A.lv003 from ts_lv0011 A)) ";
		$vresult=db_query($vsql);
	//	return $vReturn;
	}
	function LV_Insert()
	{		
		...
		$lvsql="insert into ts_lv0001 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0001.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update ts_lv0001 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0001.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ts_lv0001  WHERE ts_lv0001.lv001 IN ($lvarr)";// and (select count(*) from ts_lv0001 B where  B.lv002= ts_lv0001.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0001.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0001 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'ts_lv0001/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ts_lv0001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
