<?php
/////////////coding jo_lv0005///////////////
class   jo_lv0005 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select lv021 from jo_lv0004 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  jo_lv0005 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into jo_lv0005 (lv002,lv003,lv004,lv005) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn){	
		 	$this->InsertLogOperation($this->DateCurrent,'jo_lv0005.insert',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		  $lvsql="Update jo_lv0005 set lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'jo_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_CheckLocked($vlv002)
	{
		$lvsql="select lv021 from  jo_lv0004 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
			{
				if($vrow['lv021']<=1) return true;
				else 
					return false;
			}
			else
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM jo_lv0005  WHERE jo_lv0005.lv001 IN ($lvarr) and (select lv021 from jo_lv0004 B where  B.lv001= jo_lv0005.lv002)<=1  ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'jo_lv0005.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	//////////get view///////////////
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0005 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.* FROM jo_lv0005 A inner join  jo_lv0004 B on A.lv002=B.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."jo_lv0005/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.* FROM jo_lv0005 A inner join  jo_lv0004 B on A.lv002=B.lv001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
