<?php
///////////coding ki_lv0006///////////////
class   ki_lv0013 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function LV_Update()
	{
		...
		$lvsql="Update ki_lv0006 set lv004='$this->lv004',lv008='$this->lv008',lv009='$this->lv009',lv012='$this->LV_UserID',lv017=NOW() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ki_lv0006 set lv016=0  WHERE ki_lv0006.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
	...
	function LV_GetSumMarkForSale($vEmployeeID,$opt=0,$vParent)
	{
		...
		return $vrow['convertmoney'];
			...
			//////////////////////Buil list////////////////////
//////////////////////Buil list////////////////////
	function LV_GetKPITarget()
	{
		$vArrKPITarget=Array();
		$vsql="select * from  ki_lv0002";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vArrKPITarget[$vrow['lv001']][4]=$vrow['lv004'];
			$vArrKPITarget[$vrow['lv001']][5]=$vrow['lv005'];
		}
		return $vArrKPITarget;
	}
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$vArrKPITarget=$this->LV_GetKPITarget();
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{	
					case 'lv006':
						if($this->GetEdit()==1 )
						{
							$vCotIndex=(int)str_replace("lv","",$lstArr[$i]);
							$lvTdTextBox='<td align=@#05>'.$this->LV_GetKPI_InputText($vArrKPITarget[$vrow['lv003']][4],$vrow[$lstArr[$i]],$vrow['lv001'],$vCotIndex).'</td>';
						...
						break;
					case 'lv010':
						if($this->GetEdit()==1 && $vrow['lv096']==0)
						{
							$lvTdTextBox="<td align=@#05><input class='txtenterquick' type=\"textbox\" value=\"@02\" @03 onblur=\"UpdateText(this,'".$vrow['lv001']."',10)\" style=\"width:80px\" tabindex=\"2\"  maxlength=\"255\"   onKeyPress=\"return CheckKey(event,7)\"/></td>";
							$vTemp=str_replace("@02",$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]]),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]]));	
						}
						else
							$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
	...
	function LV_GetInputText($vOpt,$vValue)
	{
		$vRetrun="";
		switch($vOpt)
		{
...
//////////////////////Buil list////////////////////
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ki_lv0013/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
