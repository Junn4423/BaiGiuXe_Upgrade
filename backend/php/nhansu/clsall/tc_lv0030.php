			...
			*/
			...
			while ($vrow = db_fetch_array ($bResult)){
				...
				if($vrow['ProjectID']!="")
				{
					...
					$vGioCTE='';
					$isNDX=false;
					$vTimeNghiPhepNDX='00:00:00';
					$vTimeNghiPhepCT='00:00:00';
					$vSoGioDiTreVeSom='00:00:00';
					$this->DonXinPhep->TimeCard='';
					$vPhepNuaNgay='';
					$vIsSpecial=false;
					$vSoPhepOK=false;
						...
						if($this->DonXinPhep->TimeCard!='QCC')
						{
							switch($this->DonXinPhep->TimeCard)
							{
								case 'CT':
									...
									$vTimeNghi=TIMEADD($vTimeNghi,$this->DonXinPhep->TimeNghi);
									$vTimeNghiPhepCT=TIMEADD($vTimeNghiPhepCT,$this->DonXinPhep->TimeNghi);
									break;
								case '1/2KL':
								case '1/2P':
									$vXinNuaNgay=true;
									if($vPhepNuaNgay==$this->DonXinPhep->TimeCard)
									{
										$vXinNuaNgay=false;
										$vTimeNghiPhep='08:00:00';
										$vTimeCardID=str_replace("1/2","",$this->DonXinPhep->TimeCard);
										$vIsSpecial=true;
									}
									else
									{
									...
									$vPhepNuaNgay=$this->DonXinPhep->TimeCard;
									break;
								case 'P':
									$vXinNuaNgay=true;
								...
								case 'NĐX':
									...
									$vGioCTE=substr($this->DonXinPhep->lv017,11,8);
									/*if($vGioCTE>='17:00:00')
										$vTimeNghiPhepNDX='00:00:01';
									else
										*/$vTimeNghiPhepNDX=$this->DonXinPhep->TimeNghi;
									break;
								default:
									if($this->DonXinPhep->TimeCard!='' && $this->DonXinPhep->TimeCard!=null) $vXinNuaNgay=true;
									$vTimeNghi=$this->DonXinPhep->TimeNghi;
					...
					//echo "$vTimeNghi=$vTimeNghiPhep;";
					if(($vTimeNghi=='' || $vTimeNghi=='00:00:00') && $vTimeNghiPhep>'00:00:00')
					{
				...
				$vDonXinTreSom=false;
				$vCountDonXinTreSom=0;
				$vCountDonXinTreSom=$this->DonXinPhep->LV_CheckPhepStateMulti($vrow['NVID'],$vrow['lv004'],13);
				if($this->DonXinPhep->lv022==13)
				{
					$vDonXinTreSom=true;
					$vGioCTS=substr($this->DonXinPhep->lv016,11,8);
					$vGioCTE=substr($this->DonXinPhep->lv017,11,8);
					$vSoGioDiTreVeSom=TIMEDIFF($vGioCTE,$vGioCTS);
				}
					...
					if($vrow['ShiftCal']==1 || $vrow['FixDay']==1)
					{
						if($vrow['ProjectID']!='' && $vCaPro!='')
						{
							if($vrow['NhomCC']=='KHO')
							{
								...
								$vSoLanQuet=count($vArrTime[0]);
								if(count($vArrTime[0])<4 && $vXinNuaNgay==false)
								{
									if(count($vArrTime[0])>=2)
									{
										...
										if($vReturnTime['GioLam']>='04:00:00' || TIMEDIFF($vReturnTime['GioRa'],$vReturnTime['GioVao'])>='04:00:00') 
										{
					...
					else
					{
						...
						else
						{
							...
							else
							{
								$vArrTime=$this->motc_lv0012->LV_GetListTimeA($vrow['NVID'],$vDateCal,$vArrShift,$vArrDep[$vrow['lv001']][0],$vrow['lv015']);
								$vReturnTime=$this->ProcessTime($vrow['NVID'],$vArrTime,$vArrShift,0,$vrow['lv015'],$vArrDep[$vrow['lv001']][0]);
								$vSoLanQuet = (is_countable($vArrTime[0])) ? count($vArrTime[0]) : 0;
								//echo $vDateCal.':'.$vReturnTime['GioLam'].'<br/>';
								if($vDonXinTreSom)
								{
									if($vReturnTime['GioLam']<'05:00:00' && TIMEDIFF($vReturnTime['GioRa'],$vReturnTime['GioVao'])>='03:50:00') 
									{
									...
									{
										if($vReturnTime['GioLam']>='05:00:00')
										{
								...
								{
									if($vReturnTime['GioLam']<='06:00:00' && TIMEDIFF($vReturnTime['GioRa'],$vReturnTime['GioVao'])>='03:50:00') 
									{
										if($vrow['DOWS']!=1 && $vrow['DOWS']!=7)
										{
						...
						else
						{
							...
							if($vrow['NhomCC']=='KHO')
							{
								...
								/*if($vrow['DOWS']==1)
								{
									...
									$vReturnTime['GioRa']='';
									$vReturnTime['GioVao']='';
									$vReturnTime['FreeTimeStart']='';
									$vReturnTime['CaIn']='';
									$vReturnTime['FreeTime']='';
									$vReturnTime['GioLamThuc']='00:00:00';
									$vArrTime[0][0]=null;
									$vTimeCardID='';
								}*/
							}
							...
							{
								$vArrTime=$this->motc_lv0012->LV_GetListTimeA($vrow['NVID'],$vDateCal,$vArrShift,$vArrDep[$vrow['lv001']][0],$vrow['lv015'],1);
								$vSoLanQuet = (isset($vArrTime[0]) && is_array($vArrTime[0])) ? count($vArrTime[0]) : 0;
								$vReturnTime=$this->ProcessTime($vrow['NVID'],$vArrTime,$vArrShift,$vShiftCal,$vrow['lv015'],$vArrDep[$vrow['lv001']][0]);
								/*if($vrow['DOWS']==1)
								{
									$vReturnTime['GioLam']='00:00:00';
				...
				{
					$vLong_DateO=(float)str_replace("-","",$vrow['DateOff']);
					if($vLong_DateW<=$vLongDateCal && $vLong_DateO>=$vLongDateCal)
					{
				...
				else
				{
					if($vTimeCardID=='')
					{
						...
						if($vAHoliday[$vrow['lv004']][0]==1) 
						{
							if((($vrow['NhomCC']!='KHO' && count($vArrTime)<2) || ($vrow['NhomCC']=='KHO' && count($vArrTime)<2)) && $vReturnTime['GioLam']=='00:00:00')		
							{
								if($this->DonXinPhep->TimeCard!='VR')
								{
									$vLong_DateW=(float)str_replace("-","",$vrow['DateWork']);
				...
				if($vTimeCardID=="" || $vTimeCardID==NULL || $vTimeCardID=='QCC')
				{
					$this->DonXinPhep->LV_CheckPhepStateMulti($vrow['NVID'],$vrow['lv004'],'0');
					if($this->DonXinPhep->TimeCard=='VR')
					{
					...
					if($vTimeCardID=="" || $vTimeCardID==NULL || $vTimeCardID=='QCC')
					{
						...
						{
							$vLong_DateO=(float)str_replace("-","",$vrow['DateOff']);
							//echo "$vLong_DateO>$vLongDateCal <br/>";
							if($vLong_DateO>$vLongDateCal && $vrow['DOWS']!=1) 
							{
								//echo $vLongDateCal.'|'.count($vArrTime).'<br/>';
								if($vrow['NhomCC']!='KHO' )
								{
									if($vSoLanQuet==2 )
									{
										...
										if($vrow['DOWS']!=1 && $vrow['DOWS']!=7)
										{
											if($vReturnTime['GioLam']=='04:00:00')
											{
												$isNoTreSom=false;
												$vTimeCardID='1/2KP';
											}
									...
									else if( $vSoLanQuet<2  && $vReturnTime['GioLam']=='00:00:00')		
									{
										$isNoTreSom=false;
										$vTimeCardID='KP';
									}
						...
						{
							$vLong_DateW=(float)str_replace("-","",$vrow['DateWork']);
							if($vLong_DateW<=$vLongDateCal) 
							{
								if($vrow['DOWS']!=1)
								{
									//echo $vLongDateCal.'|'.count($vArrTime).'=='.$vReturnTime['GioLam'].'<br/>';
									if($vrow['NhomCC']!='KHO' )
									{
										if($vSoLanQuet==2 )
										{
				...
				switch($vStatePrj)
				{
					...
					case 2:
						...
						{
							//echo $vrow['lv004'].'==>'.$vSoPhepOK.'<br/>';
							switch($mohr_lv0038->lv011)
							{
								case 13:
								case 0:	
									if($vSoPhepOK==false)// && ($vTimeCardID=="" || $vTimeCardID==NULL))
									{
										if($vrow['DOWS']==1)
										{
											$vTimeCardID='';
				...
				switch($vTimeCardID)
				{
					...
					case 'CT':
						if($vTimeCardID=='CT')
						{
							if($vrow['NhomCC']=='KHO')
							{
								if(count($vArrTime)==0 || count($vArrTime)==2) $isNoTreSom=false;
							}
							elseif(count($vArrTime)==0) 
							{
								$isNoTreSom=false;
							}
							else
							{
								if(count($vArrTime)>=2 && $vTimeNghiPhepCT>'00:00:00')
								{
									//echo TIMEADD($vTimeNghiPhepCT,$vReturnTime['GioLam']);
									if(TIMEADD($vTimeNghiPhepCT,$vReturnTime['GioLam'])>='08:00:00')
									{
										$isNoTreSom=false;
									}
								} 
					...
					case 'KP':
						...
						break;
					case 'NĐX':
						//$vTimeWorkCur=TIMEDIFF($vTimeWorkCur,$vTimeNghi);
					default:
						$isKhongCongFree=false;
						
						if(!(strpos($vReturnTime['GioLamThuc'],"-")===false)) 
						{
						...
						else
						{
							...
							$vTimeWorkCur=$this->LV_LamTronGioLam($vTimeWorkCur,$mohr_lv0038->lv007); // lam tron h 
						}
						...
						{
							//if($vTimeWorkCur>'00:00:00' && $isNDX==false)
							if($vTimeWorkCur>'00:00:00')
								$vsql=$this->motc_lv0011->LV_UpdateTimeWorkEmp($vrow['CodeID'],'08:00:00',1);
							else
								$vsql=$this->motc_lv0011->LV_UpdateTimeWorkEmp($vrow['CodeID'],$vTimeWorkCur,1);
				...
				if($vTimeNghiPhepNDX>'00:00:00')
				{
					//echo $vArrcheck['VESOM']."|".$vTimeNghiPhepNDX.'<br/>';
					$vArrcheck['VESOM']=TIMEADD($vArrcheck['VESOM'],$vTimeNghiPhepNDX);
				}
				if($vDonXinTreSom)
				{
					if($vArrcheck['DITRE']=='00:00:00' && $vArrcheck['VESOM']=='00:00:00')
					{
						if($vCountDonXinTreSom<=1)
						{
						$vArrcheck['VESOM']=$vSoGioDiTreVeSom;
						}
					}
				}
				//echo $vDateCal.'==>'.$vArrcheck['DITRE'].','.$vArrcheck['VESOM'].'<br/>';
