<?php
/////////////coding hr_lv0098///////////////
class   hr_lv0098 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"2","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv099"=>"0");	
	...
	{
		$lvsql="select * from  hr_lv0098 Where lv001='$vlv001'";
	...
	{
		$lvsql="select lv001 from  hr_lv0098 Where lv002='$vlv002' Order by lv004 DESC limit 0,1";
		$vresult=db_query($lvsql);
		if($vresult)
		{
			$vrow=db_fetch_array($vresult);
	...
	{
		$lvsql="select * from  hr_lv0098 Where lv002='$lv002' and lv009='1'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadTemplate($vlv001)
	{
		...
		$strReturn="";
		$lvsql="select A.*,B.lv003 lv013 from  hr_lv0098 A left join hr_lv0043 B on A.lv008=B.lv001 Where A.lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0098 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv099) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv099')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0098.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		 $lvsql="Update hr_lv0098 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv099='$this->lv099' where  lv001='$this->lv001' AND lv009<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0098.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0098  WHERE hr_lv0098.lv001 IN ($lvarr) AND hr_lv0098.lv009<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0098.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update hr_lv0098 set lv009=1  WHERE hr_lv0098.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0098.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_AcceptDept()
	{
		$lvsql="select lv001,lv002,lv006,lv003,lv010,lv011,lv012,lv013 from  hr_lv0098 Where lv009='1' and lv004<=CurDate() and lv003 in ('DOIPHONGBAN','DOICHUCVU','DOICVPB')";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			...
			$vTitleVN=$vrow['lv010'];	
			$vTitleEN=$vrow['lv011'];	
			$vTitleLawVN=$vrow['lv012'];	
			$vTitleLawEN=$vrow['lv013'];	
			switch($vrow['lv003'])
			{
				case 'DOIPHONGBAN':
					$lvsql = "Update hr_lv0020 set lv029='$vDeptID' where lv001='$vEmpID'";
					break;
				case 'DOICHUCVU':
					$lvsql = "Update hr_lv0020 set lv005='$vTitleVN',lv060='$vTitleEN',lv061='$vTitleLawVN',lv067='$vTitleLawEN' where lv001='$vEmpID'";
					break;
				default:
			...
			$vReturn= db_query($lvsql);
			$lvsql = "Update hr_lv0098 set lv009=2 where lv001='$vID'";
			$vReturn= db_query($lvsql);
		}
	}
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0098 set lv009=lv009-1  WHERE hr_lv0098.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0098.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0098 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0098 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0098/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0100 ";
				break;
			case 'lv006':
				$vsql="select lv001,CONCAT(lv003,'[',lv002,']') lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0002";
				break;
			case 'lv099':
				$vsql="select lv001,concat(lv001,'(',lv004,'->',lv005,')') lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0038 where lv002='$this->lv002' order by lv009";
				break;
			case 'lv100':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0043";
				break;
		}
		return $vsql;
	}
	public  function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0100 where lv001='$vSelectID'";	
				break;
			case 'lv006':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0002 where lv001='$vSelectID'";	
				break;
			default:
				$vsql ="";
				break;
