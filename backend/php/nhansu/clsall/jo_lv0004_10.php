...
class   jo_lv0004 extends lv_controler
{
	...
	public $ArrJobs=null;
///////////
	public $DefaultFieldList="lv001,lv015,lv022,lv016,lv017,lv098,lv008,lv013,lv018,lv019,lv021,lv009,lv010,lv025";	
////////////////////GetDate
...
////////////
	...
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0004 WHERE lv021=0  and lv022 in (1,2,3,13) ".$this->GetCondition();
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0004 WHERE 1=1 and lv022 in (1,2,3,13) ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
';
		...
		';
		$sqlS = "SELECT *,lv015 lv829,lv015 lv099,TIMEDIFF(lv017,lv016) lv098 FROM jo_lv0004 WHERE 1=1 and lv022 in (1,2,3,13)  ".$this->GetCondition()."";
		$vorder=$curRow;
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,lv015 lv099,TIMEDIFF(lv017,lv016) lv098,ADDTIME(lv025,'00:05:00') CheckNgay FROM jo_lv0004 WHERE 1=1 and lv022 in (1,2,3,13)  ".$this->GetCondition()." order by lv016 desc LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
	...
	function LV_BuilList7Day($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,lv015 lv099,TIMEDIFF(lv017,lv016) lv098 FROM jo_lv0004 WHERE 1=1 and lv022 in (1,2,3,13) and lv016>=concat(curdate(),' 00:00:00') order by lv016 asc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT *,lv015 lv829,lv015 lv099,TIMEDIFF(lv017,lv016) lv098 FROM jo_lv0004 WHERE 1=1 and lv022 in (1,2,3,13)  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv021':
				...
				break;
			case 'lv022':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  jo_lv0100 where lv001 in (1,2,3,13)  order by lv009";
				break;
		}
		return $vsql;
	}
