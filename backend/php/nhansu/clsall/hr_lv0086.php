<?php
/////////////coding hr_lv0086///////////////
class   hr_lv0086 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"4","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"10","lv010"=>"0","lv011"=>"10","lv012"=>"0","lv013"=>"10","lv014"=>"0","lv015"=>"0","lv015"=>"0");	
	...
	{
		$vsql="insert into hr_lv0106 select *,$month,$year from  hr_lv0086";
		$vresult=db_query($vsql);
		return $vresult;
	}
	function LV_LoadMonthYear($month,$year)
	{
		$vsql="select A.*,B.lv002 percent1,B.lv003 percent2,B.lv004 percent3,B.lv005 percent4,B.lv006 percent5 from  hr_lv0106 A left join sl_lv0042 B on A.lv006=B.lv001 where A.lv015='$month' and A.lv016='$year'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
			...
			$this->lv013=$vrow['lv013'];
			$this->lv014=$vrow['lv014'];
			$this->doanhthu5=$vrow['lv005'];
			$this->doanhthu1=$vrow['lv008'];
			$this->hoahong1=$vrow['lv009'];
			$this->doanhthu2=$vrow['lv010'];
			$this->hoahong2=$vrow['lv011'];
			$this->percent1=$vrow['percent1'];
			$this->percent2=$vrow['percent2'];
			$this->percent3=$vrow['percent3'];
			$this->percent4=$vrow['percent4'];
			$this->percent5=$vrow['percent5'];
		}
	}
	function LV_LoadID($vlv001)
	{
		$lvsql="select * from  hr_lv0086 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$this->lv002 = GetServerDate()." ".GetServerTime() ;
		 $lvsql="insert into hr_lv0086 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014')";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'hr_lv0086.insert',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	...
	function LV_Update()
	{
		...
		$this->lv002 = GetServerDate()." ".GetServerTime() ;
		$lvsql="Update hr_lv0086 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014' ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0086.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM hr_lv0106  WHERE hr_lv0106.lv007<=0 AND hr_lv0106.lv001 IN ($lvarr) ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0106.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0106 set lv007=1  WHERE concat(lv015,lv016) IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0106.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Exist()
	{
		$lvsql="select count(*) num from  hr_lv0086 ";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		if($this->isApr==0) return false;
		echo $lvsql = "Update hr_lv0106 set lv007=0  WHERE concat(lv015,lv016) IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0106.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0086 WHERE 1=1 ".$this->GetCondition();
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004 from wh_lv0009 A inner join hr_lv0086 B on A.lv002=B.lv001  where 1=1 and B.lv001='$vContractID' )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."hr_lv0086/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class='@#04' align='@#05'>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0086 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvDateSort)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class='@#04' align='@#05'>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0086 WHERE 1=1 and lv009='$lvDateSort' ".$this->GetConditionMini()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class='@#04' align='@#05'>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0086 WHERE 1=1  ".$this->GetConditionRpt()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv005':
				...
				break;
			case 'lv006':
				$vsql="select lv001,lv001 lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0042";
				break;
			case 'lv014':
