<?php
/////////////coding hr_lv0240///////////////
class   hr_lv0240 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"22");
	var $ArrViewEnter=array("lv003"=>"33","lv005"=>"-1","lv006"=>"-1");
	...
	{
		$lvsql="select * from  hr_lv0240 Where lv001='$vlv001'";
	...
	function LV_LoadUserID($vUserID)
	{
		$lvsql="select * from  hr_lv0240 Where lv003='$vUserID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0240 (lv001,lv002,lv003,lv004,lv005,lv006) values('$this->lv001','".sof_escape_string($this->lv002)."','$this->lv003','".sof_escape_string($this->lv004)."','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0240.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		$lvsql="Update hr_lv0240 set lv002='".sof_escape_string($this->lv002)."',lv003='$this->lv003',lv004='".sof_escape_string($this->lv004)."',lv005='$this->LV_UserID',lv006=now() where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0240.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0240  WHERE hr_lv0240.lv001 IN ($lvarr)";// and (select count(*) from hr_lv0240 B where  B.lv002= hr_lv0240.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0240.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0240 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0240 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				$vField=$lstArr[$i];
				$vStringNumber="";
				$this->ArrViewEnter[$vField] = $this->ArrViewEnter[$vField] ?? 0;
				...
				{			
					...
					case 33:
						...
						$checked = (isset($this->Values[$vField]) && $this->Values[$vField] == 1) ? 'checked="true"' : '';
						$vstr = '<input autocomplete="off" class="txtenterquick" name="qxt'.$vField.'" type="checkbox" id="qxt'.$vField.'" value="1" '.$checked.' tabindex="2" style="width:100%;min-width:30px;text-align:center;" onKeyPress="return CheckKey(event,7)">';
						$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, (int)($this->ArrView[$lstArr[$i]] ?? 0)));
						break;
					case 0:
						...
						$value = isset($this->Values[$vField]) ? $this->Values[$vField] : '';
						$vstr = '<input '.$vStringNumber.' autocomplete="off" class="txtenterquick" name="qxt'.$vField.'" type="text" id="qxt'.$vField.'" value="'.htmlspecialchars($value, ENT_QUOTES).'" tabindex="2" style="width:100%;min-width:30px;text-align:center;" onKeyPress="return CheckKey(event,7)">';
						$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, (int)($this->ArrView[$lstArr[$i]] ?? 0)));
						break;
					default:
						$vTempEnter="<td>&nbsp;</td>";
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."hr_lv0240/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0240 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	{
		if (isset($this->ArrGetValueLink[$vFile][$vSelectID][0])) {
