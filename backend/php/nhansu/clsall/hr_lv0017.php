	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				$alignHtml = $this->Align($lvTd, $viewType);
				$formatted = $this->FormatView($value, $viewType);
				$linkHtml  = $this->getvaluelink($key, $formatted);

				$vTemp = str_replace("@02", $linkHtml, $alignHtml);
