...
///////////
	public $DefaultFieldList="lv299,lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv099,lv100,lv200,lv198,lv199,lv103,lv300";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv099"=>"100","lv100"=>"101","lv103"=>"104","lv200"=>"201","lv198"=>"199","lv199"=>"200","lv299"=>"300","lv300"=>"301");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv099"=>"0","lv100"=>"0","lv103"=>"0","lv200"=>"0");
	...
	function LV_Load()
	{
		...
		if($vrow)
		{
			...
			$this->lv198=$vrow['lv198'];
			$this->lv199=$vrow['lv199'];
			$this->lv200=$vrow['lv200'];
			$this->lv300=$vrow['lv300'];
		}
	}
	...
	{
		$vArrStaff=Array();
		$lvsql="select lv001 from  hr_tdltaxv3_0.hr_lv0002  Where lv001 in ($lvarr)";
		$vresult=db_query($lvsql);
	...
	function LV_CheckCreateTax($lvarr,$vHD=0)
	{
		...
		$lvsql="select * from  hr_lv0002  Where lv001 in ($lvarr)";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$this->lv001=$vrow['lv001'];
	...
	{
		$vArrShift=Array();
		$vsql="select * from  hr_lv0002 where lv001<>''";
		$vresult=db_query($vsql);
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
			$vArrShift[$vrow['lv001']][0]=$vrow['lv005'];
			$vArrShift[$vrow['lv001']][1]=(int)$vrow['lv006'];
			$vArrShift[$vrow['lv001']][3]=$vrow['lv003'];
			$vArrShift[$vrow['lv001']][9]=$vrow['lv009'];
			$vArrShift[$vrow['lv001']][10]=$vrow['lv010'];
			$vArrShift[$vrow['lv001']][11]=$vrow['lv011'];
			$vArrShift[$vrow['lv001']][100]=$vrow['lv100'];
			$vArrShift[$vrow['lv001']][103]=$vrow['lv103'];
			$vArrShift[$vrow['lv001']][200]=$vrow['lv200'];
			$vArrShift[$vrow['lv001']][300]=$vrow['lv300'];
			$i++;
		}
		return $vArrShift;
	}
	function LV_Insert($vDatabase='')
	{
		...
		if($vDatabase=='')
			$lvsql="insert into hr_lv0002 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv099,lv100,lv103,lv198,lv199,lv200,lv300) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv099','$this->lv100','$this->lv103','$this->lv198','$this->lv199','$this->lv200','$this->lv300')";
		else
		 $lvsql="insert into ".$vDatabase.".hr_lv0002 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv099,lv100,lv103,lv198,lv199,lv200,lv300) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv099','$this->lv100','$this->lv103','$this->lv198','$this->lv199','$this->lv200')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0002.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function GetChildDept($vParentID,&$i=0)
	{
		$strReturn="";
		...
		while($vrow1=db_fetch_array($vresult1))		
		{
			...
			$strReturn=$strReturn.$this->GetChildDept($vrow1['lv001'],$i);
			$i++;
		}
		$i--;
		return $strReturn;
	...
	function LV_Update($vDatabase='')
	{
		...
		if($this->lv001_=="") $this->lv001_=$this->lv001;
		 ...
		 if($vDatabase=='')
			$lvsql="Update hr_lv0002 set lv001='$this->lv001_',lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv099='$this->lv099',lv100='$this->lv100',lv103='$this->lv103',lv198='$this->lv198',lv199='$this->lv199',lv200='$this->lv200',lv300='$this->lv300' where  lv001='$this->lv001';";
		else
			$lvsql="Update ".$vDatabase.".hr_lv0002 set lv001='$this->lv001_',lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv099='$this->lv099',lv100='$this->lv100',lv103='$this->lv103',lv198='$this->lv198',lv199='$this->lv199',lv200='$this->lv200',lv300='$this->lv300' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				$keyName = $lstArr[$i];
				$index = isset($this->ArrGet[$keyName]) ? (int)$this->ArrGet[$keyName] : null;

				if ($index !== null && isset($this->ArrPush[$index])) {
					$vTemp = str_replace("@02", $this->ArrPush[$index], $vTemp);
				} else {
					// Nếu key không tồn tại, thay thế bằng chuỗi rỗng hoặc thông báo
					$vTemp = str_replace("@02", "", $vTemp);
				}
				$strH=$strH.$vTemp;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				if (
					isset($vrow[$key]) &&
					isset($this->ArrView[$key])
				) {
					$formattedValue = $this->FormatView($vrow[$key], (int)$this->ArrView[$key]);
					$alignedContent = $this->Align($lvTd, (int)$this->ArrView[$key]);

					$vTemp = str_replace(
						"@02",
						$this->getvaluelink($key, $formattedValue),
						$alignedContent
					);
				} else {
					// Xử lý nếu thiếu key, tránh lỗi
					$vTemp = str_replace("@02", "", $lvTd); // hoặc giá trị mặc định khác
				}
...
";
		...
		for ($i=0;$i<count($lstArr);$i++)
		{
			...
			$key = (int)$this->ArrGet[$lstArr[$i]] ?? null;
			if (isset($this->ArrPush[$key])) {
				$strTempChk = str_replace("@01", $i, $lvChk . $this->ArrPush[$key]);
			} else {
				// Gán giá trị mặc định nếu key không tồn tại
				$strTempChk = str_replace("@01", $i, $lvChk);
			}
...
//////////////////////Buil list////////////////////
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				$vsql="
				select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0001
				Union
				select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0002
				";
				break;
			case 'lv008':
				$vsql="select * from (
				select '0' lv001,'Không tính hơn công tiêu chuẩn và công vượt' lv002,IF('0'='$vSelectID',1,0) lv003
				Union
				select '1' lv001,'Tính hơn công tiêu chuẩn' lv002,IF('1'='$vSelectID',1,0) lv003
				Union
				select '2' lv001,'Tính công vượt' lv002,IF('2'='$vSelectID',1,0) lv003 ) MP
				";
				break;
			case 'lv099':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				$vsql="
				select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0001 where lv001='$vSelectID'
				Union
				select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0002 where lv001='$vSelectID'
				";
				break;
			case 'lv008':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from (
				select '0' lv001,'Không tính hơn công tiêu chuẩn và công vượt' lv002,1 lv003
				Union
				select '1' lv001,'Tính hơn công tiêu chuẩn' lv002,0 lv003
				Union
				select '2' lv001,'Tính công vượt' lv002,0 lv003 ) MP where lv001='$vSelectID'
				";
				break;
			case 'lv099':
