<?php
/////////////coding tc_lv0073///////////////
class   tc_lv0073 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function __construct($vCheckAdmin,$vUserID,$vright)
	{
		...
		$this->ActiveRating=Array(1=>1,2=>0,3=>0,4=>0);
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0073.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0073.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0073.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0073.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0073/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."&dayfrom=".$this->dayfrom."&dayto=".$this->dayto."&isTime=".$this->isTime."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0073/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
