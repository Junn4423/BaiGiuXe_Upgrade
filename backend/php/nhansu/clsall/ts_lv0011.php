	<?php
/////////////coding ts_lv0011///////////////
class   ts_lv0011 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv088,lv004,lv005,lv006,lv007,lv008,lv009,lv014,lv015,lv016,lv017";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv088"=>'89',"lv099"=>"100","lv100"=>"101");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"20","lv005"=>"21","lv006"=>"2","lv007"=>"0","lv008"=>"10","lv009"=>"21","lv010"=>"10","lv011"=>"10","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"22","lv017"=>"10","lv088"=>"0","lv099"=>"0","lv100"=>"10");	
	...
	{
		$lvsql="select lv007 from ts_lv0010 B where  B.lv001='$this->lv002'";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{
			$vrow=db_fetch_array($vReturn);
			if (is_array($vrow) && isset($vrow['lv007']) && $vrow['lv007'] >= 1)
			{
				$this->isAdd=0;	
				$this->isEdit=0;	
				$this->isDel=0;	
	...
	{
		$lvsql="select * from  ts_lv0011 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		if($vReturn){
			...
			$tsql="Update ts_lv0012 set lv004=lv004-$this->lv004,lv006=lv006-$this->lv006 where lv002='$this->lv003' and lv003 in (select C.lv002 from ts_lv0010 C where C.lv001='$this->lv002')";
			if(db_query($tsql)) $this->InsertLogOperation($this->DateCurrent,'ts_lv0012.Update',sof_escape_string($tsql));
		 	$this->InsertLogOperation($this->DateCurrent,'ts_lv0011.insert',sof_escape_string($lvsql));
	...
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020 from ts_lv0031 where lv002='$vlv002' and lv004<>0";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			...
			$this->lv004=$vrow['lv004'];
			$this->lv005=$vrow['lv005'];
			$this->lv006=$this->FormatView($vrow['lv006'],2);
			$this->lv007=$vrow['lv007'];
			$this->lv008=$vrow['lv008'];
			$this->lv009=$vrow['lv009'];
			$this->lv010=$vrow['lv010'];
	...
	function LV_Insert_FI_LI_FO($state,$vwhid,$vItem)
	{
		if($state=="LIFO")
			return $this->LV_Insert_LotConfirm($vItem,$vwhid,'desc');
		else
			return $this->LV_Insert_LotConfirm($vItem,$vwhid,'asc');
		

	}	
	function LV_Get_Inputing_sl($userid,$lotid,$lvitem,$lvwhid)
	{
		$vsql="select IF(ISNULL(sum(lv004)),0,sum(lv004)) slreal from ts_lv0031 where lv002='$userid' and lv014='$lotid' and lv003='$lvitem'";
	...
	function LV_Insert_LotConfirm($lvitem,$lvwhid,$lvsort)
	{
		...
		$vsql="select A.lv001,A.lv004 from ts_lv0020 A where A.lv002='$lvitem' and A.lv003='$lvwhid' order by A.lv004 $lvsort";
	...
	{
		$lvsql="select '$vInvoiceID' lv002,A.lv003,A.lv004,B.lv004 lv005,'' lv006,'' lv007,A.lv006 lv008,A.lv007 lv009,A.lv008 lv010,A.lv011 lv011,concat(CURDATE(),' ',CURTIME()) lv016,B.lv016 WHID,B.lv012 TypeWH from sl_lv0014 A inner join sl_lv0007 B on A.lv003=B.lv001  where A.lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			$this->lv002=$vrow['lv002'];
			...
			if($this->lv004>0)
			{
				if($vrow['TypeWH']=='FIFO' || $vrow['TypeWH']=='LIFO')
					$this->LV_Insert_FI_LI_FO($vrow['TypeWH'],$vWarehourID,$this->lv003);
				else
					$this->LV_Insert();
	...
	{
		$lvsql = "DELETE FROM ts_lv0031  WHERE ts_lv0031.lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}	
	function LV_Update()
	...
	function LV_UpdateEdit($vOldlv004,$vOldlv006)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn){
			$vsql1="update ts_lv0012 set lv004=lv004-($this->lv004-$vOldlv004),lv006=lv006-($this->lv006-$vOldlv006) where lv002='$this->lv003' and lv003='".$vlv003."'";
			if(	db_query($vsql1)) $this->InsertLogOperation($this->DateCurrent,'ts_lv0011.update',sof_escape_string($vsql1));
		 	$this->InsertLogOperation($this->DateCurrent,'ts_lv0011.update',sof_escape_string($lvsql));
			if($this->obj_wb_lv0006!=null)
		 	{
	...
	{
		$lvsql="select lv007 from  ts_lv0010 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ts_lv0011  WHERE ts_lv0011.lv001 IN ($lvarr) and (select lv007 from ts_lv0010 B where  B.lv001= ts_lv0011.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			
			$this->InsertLogOperation($this->DateCurrent,'ts_lv0011.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_DeleteDetail($lvarr)
	{
		$sqlD = "SELECT A.lv001 lv001,A.lv002 lv002,A.lv003 lv003,A.lv004 lv004,A.lv006 lv006,B.lv002 lvb002 FROM ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001 WHERE A.lv001 IN ($lvarr) and (select lv007 from ts_lv0010 B where  B.lv001= A.lv002)<=0;";
		$vresult=db_query($sqlD);
		$strReturn=false;
		while($vrow=db_fetch_array($vresult))
		{
			
			 $vsql="update ts_lv0012 set lv004=lv004+".$vrow['lv004'].",lv006=lv006+".$vrow['lv006']." where lv002='".$vrow['lv003']."' and lv003='".$vrow['lvb002']."'";
			if(db_query($vsql))
			{
				$tsql="delete from ts_lv0011 where lv001='".$vrow['lv001']."'";
				if(db_query($tsql))
				{
					if($this->obj_wb_lv0006!=null)
				 	{
	...
	{
		$sqlD = "SELECT A.*,B.lv002 lvb002 FROM ts_lv0011 A inner join ts_lv0010 B on A.lv002=B.lv001 WHERE B.lv007<=0;";
		$vresult=db_query($sqlD);
		$strReturn=false;
		while($vrow=db_fetch_array($vresult))
		{
	...
	{
		$sqlD = "SELECT lv002 FROM ts_lv0010 WHERE lv001='$vlv001' ";
		$vresult=db_query($sqlD);
		if($vresult){
			$vrow=db_fetch_array($vresult);
			return $vrow['lv002'];
	...
	protected function GetCondition()
	{
		...
		$strCondi=$strCondi." and A.lv002 in (select B.lv001 from ts_lv0010 B where B.lv002 in ($strwh))";
		return $strCondi;
	}
	////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0011 A WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,A.lv003 lv088,IF(ISNULL(CC.lv016),BB.lv006,CC.lv016) lv099 FROM ts_lv0011 A inner join ts_lv0010 BB on A.lv002=BB.lv001 left join sl_lv0013 CC on BB.lv006=CC.lv001 and BB.lv005 in ('BANHANG','TRAHANG')  WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";		
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ts_lv0011/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	...
	function LV_BuilListReportView($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		<table  align=\"left\" class=\"lvtable\" border=1>
		@#01
		@#02
		</table>
		";
...
//////////////////////Buil list////////////////////
	...
	{
		$lvsql="select lv004 from  ts_lv0020 Where lv001='$vLotID' and lv002='$vItem'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function Get_String_BuilHeader()
	{
		$this->lvHeader="";
		$strTD="";
		$sqlS1 = "SELECT lv014  FROM ts_lv0011 A WHERE 1=1  ".$this->GetCondition()." ";
	...
	function LV_BuilListReportOtherSoon($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vTax)
	{
		...
		$strF="<tr><td colspan=\"2\">&nbsp;</td>";
		$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
		$sqlS = "select * from (SELECT lv003,lv014,sum(lv004) lv004,lv005 FROM ts_lv0011 A WHERE 1=1  ".$this->GetCondition()." group by lv003,lv014) MP order by MP.lv003";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlconditionext($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv103':
				...
				break;
			case 'lv102':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ts_lv0001";
				break;
			case 'lv005':
				echo $vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0005";
				break;
			case 'lv007':
			...
			case 'lv013':
				...
				break;
			case 'lv017':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ts_lv0036";
				break;
			case 'lv099':
		...
		switch($vFile)
		{
			...
			case 'lv014':
				if($this->objlot->lv003=="")
					$vsql="select A.lv001,A.lv001 lv002,0 lv003,concat(A.lv006,' - ',A.lv005)  lv004,(select sum(B.lv004) from ts_lv0031 B where B.lv002='".getInfor($_SESSION['LEVINHUserID'],2)."' and B.lv003=A.lv002 and B.lv014=A.lv001 ) QuantityUsed from  ts_lv0020 A where A.lv002='$vSelectID' order by lv004";
				else
					$vsql="select A.lv001,A.lv001 lv002,0 lv003,concat(A.lv006,' - ',A.lv005)  lv004,(select sum(B.lv004) from ts_lv0031 B where B.lv002='".getInfor($_SESSION['LEVINHUserID'],2)."' and B.lv003=A.lv002 and B.lv014=A.lv001 ) QuantityUsed from  ts_lv0020 A where A.lv002='$vSelectID' and A.lv003='".$this->objlot->lv003."' order by lv004";
