...
////////////
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0005 (lv001,lv002,lv003,lv004) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004')";
	...
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update hr_lv0005 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				$formatType = isset($this->ArrView[$lstArr[$i]]) ? (int)$this->ArrView[$lstArr[$i]] : 0;
				...
				$vTemp = str_replace(
					"@02",
					$this->getvaluelink($lstArr[$i], $this->FormatView($vrow[$lstArr[$i]], $formatType)),
					$this->Align($lvTd, $formatType)
				);
				$strL=$strL.$vTemp;
			}
