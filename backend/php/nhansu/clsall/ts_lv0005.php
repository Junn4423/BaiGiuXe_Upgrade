<?php
/////////////coding ts_lv0005///////////////
class   ts_lv0005 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"20","lv005"=>"0","lv006"=>"20","lv007"=>"0","lv008"=>"20","lv009"=>"0","lv010"=>"20","lv011"=>"0","lv012"=>"0","lv013"=>"20");	
	...
	{
		$lvsql="select lv007 from ts_lv0004 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  ts_lv0005 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		 $lvsql="insert into ts_lv0005 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012')";
		$vReturn= db_query($lvsql);
	...
	function LV_Update()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		  $lvsql="Update ts_lv0005 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	function LV_UpdateOther()
	{
		if($this->isEdit==0) return false;
		if(!$this->LV_CheckLocked($this->lv002)) return false;
	    $lvsql="Update ts_lv0005 set lv008='$this->lv008',lv010='$this->lv010',lv012='$this->lv012' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql="select lv007 from  ts_lv0004 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ts_lv0005  WHERE ts_lv0005.lv001 IN ($lvarr) and (select lv007 from ts_lv0004 B where  B.lv001= ts_lv0005.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_DeleteOrther($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ts_lv0005  WHERE ts_lv0005.lv002 IN ($lvarr) and (select lv007 from ts_lv0004 B where  B.lv001= ts_lv0005.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql="select lv001 from  ts_lv0005 Where lv002='$vContracID' and lv003='$vItemID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0005 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0005 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ts_lv0005/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
