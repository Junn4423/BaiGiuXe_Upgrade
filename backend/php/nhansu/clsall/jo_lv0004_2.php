<?php
/////////////coding jo_lv0004///////////////
class   jo_lv0004_2 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $ArrJobs=null;
///////////
	public $DefaultFieldList="lv001,lv099,lv015,lv002,lv003,lv022,lv016,lv017,lv018,lv019,lv021,lv025";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv025"=>"26","lv099"=>"100","lv829"=>"830");
	...
	function GetCount()
	{
		$sqlC = "SELECT MP.nums from (SELECT COUNT(*) AS nums,(select count(*) from jo_lv0004 EE where EE.lv022=A.lv022 and EE.lv015=A.lv015 and EE.lv016=A.lv017 and EE.lv017=A.lv017 ) SOTRUNG  FROM jo_lv0004 A WHERE 1=1 ".$this->GetCondition().") MP where SOTRUNG>1";
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "select * from (SELECT *,lv015 lv829,lv015 lv099,(select count(*) from jo_lv0004 EE where EE.lv022=A.lv022 and EE.lv015=A.lv015 and EE.lv016=A.lv017 and EE.lv017=A.lv017 ) SOTRUNG FROM jo_lv0004 A WHERE 1=1  ".$this->GetCondition().") MP where SOTRUNG>1 $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				...
				{				
					...
					case 'lv002':
						...
						break;
					case 'lv003':
						$vstr='<select class="selenterquick"   name="qxtlv003" id="qxtlv003" tabindex="2" style="width:100%;min-width:50px" onKeyPress="return CheckKey(event,7)">'.$this->LV_LinkFieldExt('lv003',$this->tv003).'</select>';
						$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, (int)($this->ArrView[$lstArr[$i]] ?? 0)));
						break;
						break;
					...
					case 'lv015':
						...
						break;	
					case 'lv022':
						$vstr='<select class="selenterquick" name="qxtlv022" id="qxtlv022" tabindex="2" style="width:100%;min-width:60px" onKeyPress="return CheckKey(event,7)">'.$this->LV_LinkField('lv022',$this->tv02).'</select>';
						$vTempEnter = str_replace("@02", $vstr, $this->Align($lvTd, (int)($this->ArrView[$lstArr[$i]] ?? 0)));
						break;	
					default:
						$vTempEnter="<td>&nbsp;</td>";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT *,lv015 lv829,lv015 lv099 FROM jo_lv0004 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
