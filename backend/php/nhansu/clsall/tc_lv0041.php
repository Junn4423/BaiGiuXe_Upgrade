...
////////////
	...
	function LV_UpdateTimeWork2($vlv001,$vlv028)
	{
		$this->LV_LoadID($vlv001);
		if(!$this->LV_CheckLocked($this->lv004)) return false;
		$lvsql="Update tc_lv0011 set lv028='$vlv028' where  lv001='$vlv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0041.update',sof_escape_string($lvsql));
		}
...
//////////////////////Buil list////////////////////
	...
	function Get_String_DateFromTo()
	{
		...
		for($i=1;$i<=count($vArrDay);$i++)
		{
			...
			else
			{
				...
				$this->lvHeader1=$this->lvHeader1.$vTdProj.'<td align="center">Công</td><td  align="center">Ca</td><td  align="center">Giờ làm</td>  <td  align="center">Giờ TC</td><td  align="center">TC trưa</td><td  align="center">TC CN</td><td  align="center">Tăng ca lễ</td><td  align="center">Trễ</td><td  align="center">Sớm</td><td  align="center">Chỉnh giờ vào ra</td><td  align="center">Giờ từ máy</td> <td  align="center">Giờ làm đêm từ 22h -> 6h</td>';
			}
			$strTD=$strTD.$vTdProjData.'<td align="center">'."<!--".str_replace("/","-",$datecur)."-->".'</td>
			<td align="center"><!--1_'.str_replace("/","-",$datecur).'--></td>
			<td align="center"><!--2_'.str_replace("/","-",$datecur).'--></td>
			<td align="center"><!--3_'.str_replace("/","-",$datecur).'--></td>
			<td align="center"><!--4_'.str_replace("/","-",$datecur).'--></td>
			<td align="center"><!--5_'.str_replace("/","-",$datecur).'--></td>
			<td align="left"><!--6_'.str_replace("/","-",$datecur).'--></td>
			<td align="left"><!--7_'.str_replace("/","-",$datecur).'--></td>
			<td align="left"><!--8_'.str_replace("/","-",$datecur).'--></td>
			<td align="left"><!--9_'.str_replace("/","-",$datecur).'--></td>
			<td align="left"><!--10_'.str_replace("/","-",$datecur).'--></td> 
			<td align="left"><!--11_'.str_replace("/","-",$datecur).'--></td>';
    ...
    <Cell ss:StyleID="s84"><Data ss:Type="String">@33</Data></Cell>
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.lv002,A.lv003,A.lv004,A.lv005,A.lv014,A.lv016,A.lv017,A.lv018,A.lv019,A.lv020,A.lv021,A.lv015,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010, A.lv028,B.lv002 NVID,C.lv029 lv001,A.lv001 TimeCardId,A.lv089 FixDay  FROM tc_lv0011 A inner join tc_lv0010 B on A.lv002=B.lv001 inner join hr_lv0020 C on C.lv001=B.lv002 WHERE A.lv100<>1 $vcondition order by C.lv029 ASC,A.lv004 ASC";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
					...
					case 0:
						if($this->isEdit==1)
						{
							...
							$lvstrgt8=(trim($vrow['lv020'])=="")?"&nbsp;":substr($vrow['lv020'],0,5);
								...
								$lvstrgt28='<input type="textbox" lang="runchangetimework2(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',\'@!99\')" class="linetimework_'.$vDayCss.'" style="width:39px;text-align:center;background-color:#@@99;color:inherit;" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" id="work_'.$vrow['TimeCardId'].'" onblur="runchangetimework2(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value)" value="'.substr($vrow['lv028'],0,5).'"/>';
			...
			$this->ArrEmp[$lvvt][6]=str_replace("<!--11_".$vrow['lv004']."-->",$lvstrgt28,$this->ArrEmp[$lvvt][6]);
		...
		for($i=0;$i<count($this->ArrEmp);$i++)
		{
		 ...
		 $vp_col1_1="<td nowrap='nowrap'>".$this->GetValueArr($vArrPetrol,$this->ArrEmp[$i][9])."</td><td nowrap='nowrap'>".$this->GetValueArr($vArr,$this->ArrEmp[$i][4])."</td>";
			...
			";
				...
				$vp_col3_2="
			<td nowrap='nowrap'><input type=\"button\" class=\"linecontrol_100\"  style='width:60px;background-color:".$color."' type='text' onclick=\"UpdateMonthlyShift('".$this->ArrEmp[$i][8]."','".$this->ArrEmp[$i][0]."',this,100)\" lang=\"UpdateMonthly('".$this->ArrEmp[$i][8]."','".$this->ArrEmp[$i][0]."',@!99,100)\" value=\"".(($this->ArrEmp[$i][100]==1)?'Fixed':'Auto')."\"/></td>
			...
			$vp_col4=str_replace("#@@99",$color,$vp_col4); //thay doi mau thoi
			if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
			...
			
			{
				if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
					...
					$vp_col1="<td nowrap='nowrap'>".($i+1)."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td>"; // stt, ma pb, ..
