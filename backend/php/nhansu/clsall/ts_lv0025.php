<?php
/////////////coding ts_lv0025///////////////
class   ts_lv0025 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  ts_lv0025 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		$lvsql="insert into ts_lv0025 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0025.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update ts_lv0025 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0025.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ts_lv0025  WHERE ts_lv0025.lv001 IN ($lvarr)";// and (select count(*) from ts_lv0025 B where  B.lv002= ts_lv0025.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0025.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0025 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0025 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'ts_lv0025/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ts_lv0025 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	{
		$sqlC="select lv003,lv004,lv005,lv006,lv007 from ts_lv0049 where lv002='$vProductPriceID'  order by lv007 desc limit 0,10";
		$bResultC = db_query($sqlC);
		$lvTrH="
		<tr class=\"lvhtable\">
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvItemID)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0025 WHERE 1=1 and lv007 = '$lvItemID'   order by lv009,lv013,lv012 asc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		$strTrH=str_replace("@#01",$strH,$lvTrH);
		 ...
		 $vselectbox="
		 <br><strong>".(($this->lang=='VN')?'Chọn nhà cung cấp khác':'Choose other supplier')."</strong>
		 ...
		 $vselectbox=$vselectbox."<td width=\"50%\"><ul id=\"pop-nav3\" lang=\"pop-nav3\" onkeyup=\"ChangeName(this,3)\" onMouseOver=\"ChangeName(this,3)\" xml:lang=\"pop-nav3\">
                                                            <li class=\"menupopT\">
                                                              <input type=\"text\" autocomplete=\"off\" class=\"search_img_btn\" name=\"txtlv914_search\" id=\"txtlv914_search\" style=\"width:100%\" onKeyUp=\"LoadPopupParent(this,'txtlv914','ts_lv0003','concat(lv002,@! - @!,lv001)')\" onFocus=\"LoadPopupParent(this,'txtlv914','ts_lv0003','concat(lv002,@! - @!,lv001)')\" tabindex=\"200\">
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv010':
				...
				break;
			case 'lv014':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ts_lv0050";
				break;
			case 'lv914':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ts_lv0003";
				break;
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv010':
				...
				break;
			case 'lv014':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ts_lv0050 where lv001='$vSelectID'";
				$lvopt=4;
				break;
			default:
				$vsql ="";
