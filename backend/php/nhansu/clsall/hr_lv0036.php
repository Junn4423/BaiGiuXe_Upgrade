<?php
/////////////coding hr_lv0036///////////////
class   hr_lv0036 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv099,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010";	
////////////////////GetDate
	public $DateCurrent="1900-01-01";
	public $Count=null;
	public $paging=null;
	public $lang=null;
	protected $objhelp='hr_lv0036';
	public $Dir="";
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv099"=>"100","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"2","lv007"=>"10","lv008"=>"0","lv009"=>"0","lv010"=>"22","lv099"=>"0");	
	...
	{
		$lvsql="select * from  hr_lv0036 Where lv001='$vlv001'";
	...
	function LV_GetAlarm($vStaffID)
	{
		$vStrRetunr='';
		$lvsql="select * from  hr_lv0036 Where lv002='$vStaffID' and lv004>0";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			if($vStrRetunr=='')
				$vStrRetunr=$vrow['lv005'];
			else
				$vStrRetunr=$vStrRetunr.";".$vrow['lv005'];
		}
		return $vStrRetunr;
	}
	///////Xu ly danh gia nhan vien
	function LV_GetDNCT($vStaffID,$vPDNC)
	{
		$vStrRetunr='';
		$lvsql="select count(*) SoDNCT from  hr_lv0036 Where lv002='$vStaffID' and lv005='$vPDNC'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		return $vrow['SoDNCT'];
	}
	function LV_GetLoaiHU($vSoNgay,&$vSoTien=0)
	{
		$lvsql="select lv001,lv003 from  hr_lv0035 Where lv004<='$vSoNgay' and lv005>='$vSoNgay' and lv006=1";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vSoTien=$vrow['lv003'];
	...
	function LV_GetHCNS($vMaNV)
	{
		$lvsql="select lv001,lv101 from  hr_lv0020 Where lv001='$vMaNV'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			if(trim($vrow['lv101'])!='' && $vrow['lv101']!=null)
			{
			return $vrow['lv101'];
			}
		}
	...
	function LV_XuLyDGNV($vMaNVID,$vSoNgayTre,$vPDNC,$vDate)
	{
		//echo "$vMaNVID,$vSoNgayTre,$vPDNC,$vDate=";
		$vMaNVIDNew=$this->LV_GetHCNS($vMaNVID);
		$vSoDNCT=$this->LV_GetDNCT($vMaNVIDNew,$vPDNC);
		if($vSoDNCT==0)
		{
			$vSoTien=0;
			$vLoaiHU=$this->LV_GetLoaiHU($vSoNgayTre,$vSoTien);
			if($vLoaiHU!='')
			{
				$this->lv002=$vMaNVIDNew;
				$this->lv003='CYP001';
				$this->lv004=$vLoaiHU;
				$this->lv005=$vPDNC;
				$this->lv006=$vDate;	
	...
	function LV_XuLyDGNV_GOOD($vMaNVID,$vSoNgayTre,$vPDNC,$vDate)
	{
		//echo "$vMaNVID,$vSoNgayTre,$vPDNC,$vDate=";
		$vMaNVIDNew=$this->LV_GetHCNS($vMaNVID);
		$vSoDNCT=$this->LV_GetDNCT($vMaNVIDNew,$vPDNC);
		...
		{
			$vSoTien=0;
			$vLoaiHU='HU0';//$this->LV_GetLoaiHU($vSoNgayTre,$vSoTien);
			if($vLoaiHU!='')
			{
				$this->lv002=$vMaNVIDNew;
				$this->lv003='CYP001';
	...
	function LV_InsertHU()
	{
		if($this->isAdd==0) return false;
		$lvsql="insert into hr_lv0036 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->InsertLogOperation($this->DateCurrent,'hr_lv0036.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0036 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0036.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $this->lv006 = ($this->lv006!="")?recoverdate(($this->lv006), $this->lang):$this->DateDefault;
		 $lvsql="Update hr_lv0036 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001' and lv008=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0036.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0036  WHERE hr_lv0036.lv001 IN ($lvarr) and lv008=0";// and (select count(*) from hr_lv0036 B where  B.lv002= hr_lv0036.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0036.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0036 set lv008=1  WHERE hr_lv0036.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0036.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update hr_lv0036 set lv008=0  WHERE hr_lv0036.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0036.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	/////lv admin deletet
	...
	function GetCondition()
	{
		...
		if($this->IsHU0T!="") $strCondi=$strCondi." and (A.lv004 <> 'HU0')";
		...
		{	
			...
			foreach($vArrName as $vName)
			{
				...
				{
				if($strCondi1=="")	
					$strCondi1= " AND (B.lv002  like '%$vName%'";
				else
					$strCondi1=$strCondi1." OR B.lv002  like '%$vName%'";		
				}
			}
			if($strCondi1!='')
			{
		...
		if($this->lv801!="")  
		{
			if(strpos($this->lv001,',')===false) 
				$strCondi=$strCondi." and B.lv001  like '%$this->lv801%'";
			else
				$strCondi=$strCondi." and B.lv001 in ('".str_replace(",","','",$this->lv801)."')";
		}
		if($this->lv899!="")  $strCondi=$strCondi." and B.lv099 in ('".str_replace(",","','",$this->lv899)."')";
		if($this->DateFrom!="")
		{
			$strCondi=$strCondi." and A.lv006>= '".recoverdate($this->DateFrom,$this->lang)."'";
		}
		if($this->DateTo!="") 
		{
			$strCondi=$strCondi." and A.lv006<= '".recoverdate($this->DateTo,$this->lang)."'";
		}
		return $strCondi;
	}
		////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0036 A inner join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,A.lv002 lv099 FROM hr_lv0036 A inner join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$vorder++;
			$vSumTongTien=$vSumTongTien+$vrow['lv007'];
			for($i=0;$i<count($lstArr);$i++)
			{
		...
		$vSumTongTien = $vSumTongTien ?? 0;
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv007-->",$this->FormatView($vSumTongTien,10),$strF);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0036/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.*,A.lv002 lv099 FROM hr_lv0036 A inner join hr_lv0020 B on A.lv002=B.lv001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,concat('[',lv002,']->[',lv003,']') lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0003 order by lv002 DESC";
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0035";
				break;
		}
		return $vsql;
	}
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,concat('[',lv002,']->[',lv003,']') lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0003 where lv001='$vSelectID'";
				break;
			case 'lv099':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0020 where lv001='$vSelectID'";
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0035 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
