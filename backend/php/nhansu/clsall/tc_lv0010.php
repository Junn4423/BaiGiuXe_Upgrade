<?php
/////////////coding tc_lv0010///////////////
class   tc_lv0010 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='tc_lv0010';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"2","lv006"=>"10","lv007"=>"10","lv008"=>"0","lv009"=>"2","lv010"=>"0");	
	...
	{
		$lvsql="select * from  tc_lv0010 Where lv001='$vlv001'";
	...
	function LV_LoadContractLaborID($vlv010)
	{
		$lvsql="select * from  tc_lv0010 Where lv010='$vlv010'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into tc_lv0010 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009')";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'tc_lv0010.insert',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_InsertNoID($vdatabase='')
	{
		...
		if($vdatabase=='')
		{
			$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
			$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
			$lvsql="insert into tc_lv0010 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010')";
		}
		else
			$lvsql="insert into $vdatabase.tc_lv0010 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010')";
		$vReturn= db_query($lvsql);
		if($vReturn){
	...
	function LV_Update()
	{
		...
		$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		$lvsql="Update tc_lv0010 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv009='$this->lv009',lv010='$this->lv010' where  lv001='$this->lv001' AND lv007<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) {
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM tc_lv0010  WHERE tc_lv0010.lv007<=0 AND tc_lv0010.lv001 IN ($lvarr) ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0010.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_GetListCalendar($vListEmployeeID,$vField,$vNoID,$vContractID='',$vdatabase='')
	{
		if($vdatabase!="") $vdatabase=$vdatabase.".";
		if($vContractID!='')
			$vsql="select $vField from ".$vdatabase."tc_lv0010 where lv002 ='$vListEmployeeID' and lv001!='$vNoID' and lv010='$vContractID'";
		else
			$vsql="select $vField from ".$vdatabase."tc_lv0010 where lv002 ='$vListEmployeeID' and lv001!='$vNoID'";
		$vresult=db_query($vsql);
		$strReturn="";
		if($vresult)
		{
	...
	function LV_UpdateBalanceCalanda($vEmpID,$vCalendaID,$vStartDate,$vEndDate,$vContractID='',$vOldContractID='',$vdatabase='')
	{
		//Xác định mốc ngày hợp đồng
		//
		$vListCalenda=$this->LV_GetListCalendar($vEmpID,'lv001',$vCalendaID,$vOldContractID,$vdatabase);
		$this->LV_CopyOldDataToEmptyData($vEmpID,$vListCalenda,$vStartDate,$vEndDate,$vContractID,$vOldContractID,$vdatabase);
		if($vdatabase=='')
			$vsql="update tc_lv0011 set lv100=1 where lv002 in ($vListCalenda) and lv004>='$vStartDate' and lv004<=ADDDATE('$vEndDate',62) and lv100<>1";
		else
			$vsql="update $vdatabase.tc_lv0011 set lv100=1 where lv002 in ($vListCalenda) and lv004>='$vStartDate' and lv004<=ADDDATE('$vEndDate',62) and lv100<>1";
		$vReturn= db_query($vsql);
		//Giải quyết cập nhật
	}
	function LV_CopyOldDataToEmptyData($vEmpID,$vListCalenda,$vStartDate,$vEndDate,$vContractID,$vOldContractID)
	{
		//Cần dữ liệu calendar =0 để giải quyết nghiệp vụ.
		$vsql="select lv004,lv005,lv006,lv007,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020 from tc_lv0011 where lv002 in ($vListCalenda) and lv004>='$vStartDate' and lv004<=ADDDATE(CurDate(),31) and lv100=0";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
			$vsqlupdate="update tc_lv0011 set  lv005='".$vrow['lv005']."',lv006='".$vrow['lv006']."',lv007='".$vrow['lv007']."',lv010='".$vrow['lv010']."',lv011='".$vrow['lv011']."',lv012='".$vrow['lv012']."',lv013='".$vrow['lv013']."',lv014='".$vrow['lv014']."',lv015='".$vrow['lv015']."',lv016='".$vrow['lv016']."',lv017='".$vrow['lv017']."',lv018='".$vrow['lv018']."',lv019='".$vrow['lv019']."',lv020='".$vrow['lv020']."' where lv004='".$vrow['lv004']."' and lv099='$vContractID'";
			db_query($vsqlupdate);
		}
	}
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update tc_lv0010 set lv007=1  WHERE tc_lv0010.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0010.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update tc_lv0010 set lv007=0  WHERE tc_lv0010.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0010.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM tc_lv0010 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM tc_lv0010 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."tc_lv0010/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM tc_lv0010 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,concat(lv003,':',lv005,'->',lv006) lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0001";
				break;
			case 'lv0008':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  tc_lv0001 where lv001='$vSelectID'";
				break;
			case 'lv008':
