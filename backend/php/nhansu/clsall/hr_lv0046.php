<?php
/////////////coding hr_lv0046///////////////
class   hr_lv0046 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv008=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv008,lv006,lv007";	
////////////////////GetDate
	public $DateCurrent="1900-01-01";
	public $Count=null;
	public $paging=null;
	public $lang=null;
	protected $objhelp='hr_lv0046';
	public $Dir="";
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"2","lv007"=>"0","lv007"=>"0");	
	...
	{
		$lvsql="select * from  hr_lv0046 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into hr_lv0046 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$thiss->lv007','$thiss->lv008')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0046.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
			$this->lv006 = ($this->lv006!="")?recoverdate(($this->lv006), $this->lang):$this->DateDefault;
		 $lvsql="Update hr_lv0046 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0046.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM hr_lv0046  WHERE hr_lv0046.lv001 IN ($lvarr)";// and (select count(*) from hr_lv0046 B where  B.lv002= hr_lv0046.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'hr_lv0046.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM hr_lv0046 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0046 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
////////////////////Other//////////////////////////	
	function LV_BuilListOther($vlv001,$vlv005)
	{
		$lvList='stB,stC,stD,st1,st2,st3,st4,st5,stE';
		$lvOrderList='0,1,2,3,4,5,6,7,8';
		$ArrView=array("stB"=>"0","stC"=>"0","stD"=>"0","st1"=>"1","st2"=>"0","st3"=>"10","st4"=>"10","st5"=>"1","stE"=>"0");
		if($this->isView==0) return false;
		$lstArr=explode(",",$lvList);
		$lstOrdArr=explode(",",$lvOrderList);
		$lvTable="
		...
		$lvTr="<tr class=\"lvlinehtable@01\">
			<td width=1% align=right>@03</td>
			@#01
		</tr>
		";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		 $sqlS = "SELECT concat(B.lv004,' ',B.lv003,' ',B.lv002) stB,B.lv020 stC,A.lv008 stD,C.lv007 st1,concat(month(A.lv006),'/',year(A.lv006))  st2,A.lv004 st3,(select sum(CB.lv004) from hr_lv0046 CB where CB.lv003=A.lv003 and CB.lv005=A.lv005 and YEAR(CB.lv006)=YEAR(A.lv006)) st4,((A.lv004*C.lv007*(0.75))/24) st5,A.lv007 stE FROM hr_lv0046 A left join  hr_lv0020 B on A.lv003=B.lv001 left join hr_lv0045 C on C.lv003=A.lv003  WHERE A.lv002='$vlv001' and A.lv005='$vlv005' ";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'hr_lv0046/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM hr_lv0046 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
