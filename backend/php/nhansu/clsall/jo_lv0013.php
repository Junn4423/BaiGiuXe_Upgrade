...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv025"=>"26","lv099"=>"100","lv199"=>"200","lv829"=>"830");
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update jo_lv0004 set lv021='2'  WHERE jo_lv0004.lv001 IN ($lvarr)  ";
	...
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update jo_lv0004 set lv021='0'  WHERE jo_lv0004.lv001 IN ($lvarr) and lv021=1  ";
		$vReturn= db_query($lvsql);
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,DATEDIFF(lv016,lv025) SoNgayXinPhep,lv015 lv829,(DATEDIFF(lv017,lv016)+IF(LEFT(lv003,3)='1/2',0.5,1)) lv098 FROM jo_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
				...
				switch($lstArr[$i])
				{
					case 'lv199':
						...
						//if(($vrow['lv008']<1)) 
						{
							...
							if($this->isUnApr==1) $vChucNang=$vChucNang.'<td><input type="button" value="Trả lại" style="padding:3px;border-radius:3px;font-weight:bold;cursor:pointer;" onclick="UnAprQLy(\''.($vrow['lv001']).'\')"/></td>';
						}
					...
					case 'lv018':
							if($this->GetApr()==1)
							{
								$lvTdTextBox="<td align=@#05 rowspan=\"@#99\"><input type=\"textbox\" value=\"@02\" @03 onblur=\"UpdateText(this,'".$vrow['lv001']."',18)\" style=\"width:125px\" tabindex=\"2\"  maxlength=\"32\"  ondblclick=\"if(self.gfPop)gfPop.fPopCalendar(this);return false;\"/></td>";
								$vTemp=str_replace("@02",$this->FormatView((($vrow[$lstArr[$i]]=='')?$this->DateCurrent:$vrow[$lstArr[$i]]),(int)$this->ArrView[$lstArr[$i]]),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]]));	
							}
							else
								$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
							break;
						case 'lv019':
							if($this->GetApr()==1)
							{
								$lvTdTextBox="<td align=@#05 rowspan=\"@#99\"><input type=\"textbox\" value=\"@02\" @03 onblur=\"UpdateText(this,'".$vrow['lv001']."',19)\" style=\"width:125px\" tabindex=\"2\"  maxlength=\"32\"  ondblclick=\"if(self.gfPop)gfPop.fPopCalendar(this);return false;\"/></td>";
								$vTemp=str_replace("@02",$this->FormatView((($vrow[$lstArr[$i]]=='')?$this->DateCurrent:$vrow[$lstArr[$i]]),(int)$this->ArrView[$lstArr[$i]]),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]]));	
							}
							else
								$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
