<?php
/////////////coding ts_lv0022///////////////
class   ts_lv0022 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"10","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"22","lv014"=>"10","lv015"=>"10","lv016"=>"10","lv017"=>"0");	
	...
	{
		$lvsql="select lv011 from ts_lv0021 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  ts_lv0022 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into ts_lv0022 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertNoID()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="insert into ts_lv0022 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv098,lv099) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv098','$this->lv099')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$sqlC="select lv001,lv009,lv012,lv013 from ts_lv0025 where lv002='$vSupplierID' and lv007='$vItemID'";
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		if($arrRowC['lv001']!=NULL && $arrRowC['lv001']!="")
		{
			$lvsql="update ts_lv0025 set lv009='$vPrice',lv012='$vTax',lv013='$vDiscount' where lv002='$vSupplierID' and lv007='$vItemID'";
			$vReturn= db_query($lvsql);
			if($vReturn) 
			{
				if($arrRowC['lv009']!=$vPrice || $arrRowC['lv012']!=$vTax || $arrRowC['lv013']!=$vDiscount)
				{
					$lvsql="insert into ts_lv0049 (lv002,lv003,lv004,lv005,lv006,lv007) value('".$arrRowC['lv001']."','$vPrice','$vUnitPriceID','$vTax','$vDiscount',concat(CURRENT_DATE(),' ',CURRENT_TIME()))";
					$vReturn= db_query($lvsql);
				}
				$this->InsertLogOperation($this->DateCurrent,'ts_lv0025.update',sof_escape_string($lvsql));
			}
			return $vReturn;
		}
		else
		{
			$vlv001=InsertWithCheckExt('ts_lv0025', 'lv001', 'SP-NCC-',1);
			$lvsql="insert into ts_lv0025 (lv001,lv002,lv003,lv007,lv009,lv010,lv012,lv013) value('$vlv001','$vSupplierID','$vItemID','$vItemID','$vPrice','$vUnitPriceID','$vTax','$vDiscount')";
			$vReturn= db_query($lvsql);
			if($vReturn) 
				{
					$lvsql="insert into ts_lv0049 (lv002,lv003,lv004,lv005,lv006,lv007) value('$vlv001','$vPrice','$vUnitPriceID','$vTax','$vDiscount',concat(CURRENT_DATE(),' ',CURRENT_TIME()))";
					$vReturn= db_query($lvsql);
					$this->InsertLogOperation($this->DateCurrent,'ts_lv0025.insert',sof_escape_string($lvsql));
				}
			return $vReturn;
		}
	}
	...
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv098,lv099 from ts_lv0051 where lv002='$vlv002' and lv014='$vlv014'";
		$vReturn= db_query($lvsql);
	...
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013 from ts_lv0032 where lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			$this->lv002=$vrow['lv002'];
	...
	function LV_GetQuantityRecieption($vPOID,$vItemID,$vSize,$vColor)
	{
		...
		$lvsql="select sum(A.lv004) TotalQty,C.lv006 Quidoi from ts_lv0009 A inner join ts_lv0008 B on A.lv002=B.lv001 left join sl_lv0007 C on A.lv003=C.lv001 where  B.lv006='$vPOID' and B.lv005='MUAHANG' and A.lv003='$vItemID' And A.lv014 in ($strLotID)";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{
			$vrow=db_fetch_array($vReturn);
	...
	function LV_GetLot_TP($vItemId,$vColor,$vSize)
	{
		...
		$vsql="select distinct lv001 from ts_lv0020 where 1=1 and lv002='$vItemId'  $vcondtion";
		$vresult=db_query($vsql);
	...
	{
		$lvsql = "DELETE FROM ts_lv0032  WHERE  lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}
	...
	{
		$lvsql = "DELETE FROM ts_lv0051  WHERE lv002='$vlv002' and lv014='$vlv014'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="Update ts_lv0022 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_CheckLocked($vlv002)
	{
		$lvsql="select lv011 from  ts_lv0021 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ts_lv0022  WHERE ts_lv0022.lv001 IN ($lvarr) and (select lv011 from ts_lv0021 B where  B.lv001= ts_lv0022.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ts_lv0022 set lv014=1  WHERE ts_lv0022.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ts_lv0022 set lv014=0  WHERE ts_lv0022.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ts_lv0022.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ts_lv0022 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0022 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ts_lv0022/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM ts_lv0022 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvItemID)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,B.lv002 lv017,B.lv004 lv016 FROM ts_lv0022 A inner join ts_lv0021 B on A.lv002=B.lv001 WHERE 1=1 and A.lv003 = '$lvItemID'   order by B.lv004 desc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
