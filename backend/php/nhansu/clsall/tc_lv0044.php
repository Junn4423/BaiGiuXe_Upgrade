<?php
/////////////coding tc_lv0044///////////////
class  tc_lv0044 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $ArrNightShift=null;
	public $ArrNightShiftEmp=null;
...
////////////
	...
	function __construct($vCheckAdmin,$vUserID,$vright)
	{
		...
		$this->ArrNightShift=Array();
		$this->ArrNightShiftEmp=Array();
	...
	function LV_Update()
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) {			
		$this->InsertLogOperation($this->DateCurrent,'tc_lv0044.update',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_UpdateEatTC($vlv001,$vlv008)
	{
		$this->LV_LoadID($vlv001);
		if(!$this->LV_CheckLocked($this->lv004)) return false;
		$lvsql="Update tc_lv0011 set lv008='$vlv008' where  lv001='$vlv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) {			
	...
	function LV_OrtherInsert($lvNVID,$vlv004,$vlv007,$vlv008)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0044.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0044.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($vlv004)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0044.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			".$lvFrom.".action='".$this->Dir."tc_lv0044/?func=rpt&lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."&txtlv029=".$this->lv029."&txtlv001=".$this->lv001."'
			".$lvFrom.".target='_blank';
			".$lvFrom.".submit();
			".$lvFrom.".target='_self';
			//window.open('".$this->Dir."tc_lv0044/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."&NVID=".$this->lvNVID."&YearMonth=".$this->lv004."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function TinhCongTheoNgayVP_BV($vEmpID,$vyear,$vmonth,&$ArrEmpList)
	{
		...
		for($i=1;$i<=$vNumDay;$i++)
		{
			...
			if($vrow)
			{
				...
				if($ArrEmpList["$vEmpID"][$i]['MaCong']=='X' || $ArrEmpList["$vEmpID"][$i]['MaCong']=='HA' || $ArrEmpList["$vEmpID"][$i]['MaCong']=='HP' )
				{
					if($vrow['tiencom']!=1)
	...
	public function GetTimeCode($vEmployeeID,$vStartDate,$vEndDate,$vTimeCode)
	{
		...
		$this->ArrTimeCordPush[2]="Mã công";
		$this->ArrTimeCordPush[3]="Tên công";
		$this->ArrTimeCordPush[4]="Gi? công(h)";
...
//////////////////////Buil list////////////////////
		...
		if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
				{
					if($_GET['day']=='')
					{
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center"  COLSPAN="2"><b><font color="'.$color.'">'.getday($datecur).'</font></b></td>';
						$this->lvHeader1=$this->lvHeader1.'<td align="center">Công</td>'.'<td  align="center">Cơm</td>';
					}
					else
					{
						$this->lvHeader=$this->lvHeader.'<td class="lvhtable" align="center"  COLSPAN="2"><b><font color="'.$color.'">'.$this->FormatView($datecur,2).'</font></b></td>';						
						$this->lvHeader1=$this->lvHeader1.'<td  align="center">Công</td>'.'<td  align="center">Cơm</td>';
					}
				}
			else
			...
			$strTD=$strTD.'<td align="center">'."<!--".str_replace("/","-",$datecur)."-->".'</td><td align="center">'."<!--_".str_replace("/","-",$datecur)."-->".'</td>';
	...
	function Get_Arr_Employees()
	{
		...
		if($this->lv030!="")  $strCondi=$strCondi." AND DD.lv001 in ('".$this->lv030."')";
		if($this->lvSort==0)
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv009 Petrol,EE.lv001 MonthID from hr_lv0020 DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv029";
		else 
			$lvsql="select DD.lv001 CodeID,DD.lv002 Name,DD.lv029 Dep,EE.lv007 HeSoL,EE.lv008 DangGia,EE.lv005 Locks,EE.lv009 Petrol,EE.lv001 MonthID from hr_lv0020 DD DD left join tc_lv0009 EE on DD.lv001=EE.lv002 and EE.lv003='$this->month' and EE.lv004='$this->year' where 1=1 $strCondi  order by DD.lv001";
		$vresult=db_query($lvsql);	
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
	...
	function LV_GetArrNightShift($vCalID,$vDayOfWeek)
	{
		$vArr=Array();
		$vState=$this->ArrDayOfWeek[$vDayOfWeek];
		if($this->ArrNightShift[$vState][0]==true) return;
		$vsql="select A.*,B.lv002 NameCode from tc_lv0031 A left join tc_lv0002 B on A.lv003=B.lv001 where A.lv002='$vCalID' and (  concat(',',lv007,',') like concat('%,','$vState',',%'))";
		$bResult=db_query($vsql);
		$vArr[$vState][0]=true;
		$i=1;
		$vValue=0;
		while ($vrow = db_fetch_array ($bResult)){
			...
			$this->ArrNightShift[$vState][$i]['lv001']=$vrow['lv001'];
			$this->ArrNightShift[$vState][$i]['lv002']=$vrow['lv002'];
			$this->ArrNightShift[$vState][$i]['lv003']=$vrow['lv003'];
			$this->ArrNightShift[$vState][$i]['lv004']=$vrow['lv004'];
			$this->ArrNightShift[$vState][$i]['lv005']=$vrow['lv005'];
			$this->ArrNightShift[$vState][$i]['lv006']=$vrow['lv006'];
			$this->ArrNightShift[$vState][$i]['lv007']=$vrow['lv007'];
			$this->ArrNightShift[$vState][$i]['NameCode']=$vrow['NameCode'];			
			$i++;
		}
	}
	function LV_GetBuildNightShift($vEmployeeID,$vCalID,$vDate)
	{
		$this->LV_GetArrNightShift($vCalID,$vDate);
		$vState=$this->ArrDayOfWeek[$vDate];
		foreach($this->ArrNightShift[$vState] as $vNightShift)
		{
			if($this->ArrNightShiftEmp[$vEmployeeID][$vCalID][$vDate][$vNightShift['lv003']]!=NULL && $this->ArrNightShiftEmp[$vEmployeeID][$vCalID][$vDate][$vNightShift['lv003']]!='')
				$lvstrgt1=$lvstrgt1.'<tr style="border-bottom:1px #000 dashed"><td><span title="'.$vNightShift['NameCode'].'">'.$vNightShift['lv003'].'</span></td><td><input id="chk_'.$vEmployeeID.$vCalID.$vDate.$vNightShift['lv003'].'" title="'.$vNightShift['lv003'].'" type="checkbox" checked '.(($vFocus==1)?' tabindex="'.((int)substr ($vDate,6,10)).'"':'').'" onclick="runchangenighttime(this,\''.$vEmployeeID.'\',\''.$vCalID.'\',\''.$vDate.'\',\''.$vNightShift['lv003'].'\',\''.$vNightShift['lv006'].'\')" value="1"/></td><td><input onblur="runchangenighttime1(this,\''.$vEmployeeID.'\',\''.$vCalID.'\',\''.$vDate.'\',\''.$vNightShift['lv003'].'\',this.value)" id="qty_'.$vEmployeeID.$vCalID.$vDate.$vNightShift['lv003'].'" style="width:30px;height:16px;" type="textbox" value="'.$this->ArrNightShiftEmp[$vEmployeeID][$vCalID][$vDate][$vNightShift['lv003']].'" class="qty_enable"/></td></tr>';			
			else
				$lvstrgt1=$lvstrgt1.'<tr style="border-bottom:1px #000 dashed"><td><span title="'.$vNightShift['NameCode'].'">'.$vNightShift['lv003'].'</span></td><td><input id="chk_'.$vEmployeeID.$vCalID.$vDate.$vNightShift['lv003'].'" title="'.$vNightShift['lv003'].'" type="checkbox" '.(($vFocus==1)?' tabindex="'.((int)substr ($vDate,8,10)).'"':'').'" onclick="runchangenighttime(this,\''.$vEmployeeID.'\',\''.$vCalID.'\',\''.$vDate.'\',\''.$vNightShift['lv003'].'\',\''.$vNightShift['lv006'].'\')" value="1"/></td><td><input onblur="runchangenighttime1(this,\''.$vEmployeeID.'\',\''.$vCalID.'\',\''.$vDate.'\',\''.$vNightShift['lv003'].'\',this.value)" id="qty_'.$vEmployeeID.$vCalID.$vDate.$vNightShift['lv003'].'" style="width:30px;height:16px;" type="textbox" value="0" readonly="true" class="qty_disable"/></td></tr>';			
		}
		return '
		<!--<div onclick="openOT(\''.$vEmployeeID.$vCalID.$vDate.'\')" style="cursor:pointer">CĐ/TC</div>-->
		<div id="'.$vEmployeeID.$vCalID.$vDate.'" style="display:block"><table class="tbl_otns">'.$lvstrgt1.'</table></div>';
	}
	function LV_GetBuildNightShiftView($vEmployeeID,$vCalID,$vDate)
	{
		$this->LV_GetArrNightShift($vCalID,$vDate);
		$vState=$this->ArrDayOfWeek[$vDate];
		foreach($this->ArrNightShift[$vState] as $vNightShift)
		{
			if($this->ArrNightShiftEmp[$vEmployeeID][$vCalID][$vDate][$vNightShift['lv003']]>0)
				$lvstrgt1=$lvstrgt1.'<tr style="border-bottom:1px #000 dashed"><td><span title="'.$vNightShift['NameCode'].'">'.$vNightShift['lv003'].'</span></td><td>:</td><td>'.$this->ArrNightShiftEmp[$vEmployeeID][$vCalID][$vDate][$vNightShift['lv003']].'</td></tr>';			
		}
		return '
		<!--<div onclick="openOT(\''.$vEmployeeID.$vCalID.$vDate.'\')" style="cursor:pointer">CĐ/TC</div>-->
		<div id="'.$vEmployeeID.$vCalID.$vDate.'" style="display:block"><table class="tbl_otns">'.$lvstrgt1.'</table></div>';
	}
	function LV_GetFullNightShift($vCalID)
	{		
		$vsql="select * from tc_lv0046 A where A.lv002='$vCalID'";
		$bResult=db_query($vsql);
		$vState=$this->ArrDayOfWeek[$vDayOfWeek];
		while ($vrow = db_fetch_array ($bResult)){
			$this->ArrNightShiftEmp[$vrow['lv006']][$vCalID][$vrow['lv005']][$vrow['lv003']]=$vrow['lv004'];
		}		
	}
	...
	{		
		...
		$this->LV_GetFullNightShift($this->CalID);
		if($lvList=="") $lvList=$this->DefaultFieldList;
		$lvList=$lvList.",lv012";
		if($this->isView==0) return false;
		$lstArr=explode(",",$lvList);
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$lvstrgt="";
				switch ($lvgt)
				{
					case 0:
						if($this->isEdit==1)
						{
							$lvstrgt=$this->LV_GetBuildNightShift($vrow['NVID'],$this->CalID,$vrow['lv004']);//.'<select style="width:49px;font-size:10px"  id="timecard_'.$vrow['TimeCardId'].'" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value,0)">'.$this->CreateSelectArr($vArr,$vrow['lv007']).'</select>';
							$lvstrgt1='<input title="BT" type="checkbox" '.(($vrow['lv010']==1)?'checked':'').' '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" onclick="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',(this.checked)?\'1\':\'\',1)" value="1"/>';
							$lvstrgt2='<input title="TC"  id="comtc_'.$vrow['TimeCardId'].'" style="display:'.(($vrow['lv008']==1)?'bock':'none').';" type="checkbox" '.(($vrow['lv008']==1)?'checked':'').' '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'" onclick="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',(this.checked)?\'1\':\'\',2)"  value="1"/>';
						}
						else
						{
							$lvstrgt=$this->LV_GetBuildNightShiftView($vrow['NVID'],$this->CalID,$vrow['lv004']);
							$lvstrgt1=(trim($vrow['lv010'])=="")?"&nbsp;":$vrow['lv010'];
							$lvstrgt2=(trim($vrow['lv008'])=="")?"&nbsp;":'<font color="red">'.$vrow['lv008'].'</font>';
						}
					...
					case 1:
						if($this->isEdit==1)
						{
							$lvstrgt='<select style="width:49px;font-size:10px" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value,0)">'.$this->CreateSelectArr($vArr,$vrow['lv010']).'</select>';
							$lvstrgt1='<input type="checkbox" '.(($vFocus==1)?' tabindex="'.((int)substr ($vrow['lv004'],8,10)).'"':'').'"  onblur="runchangetime(\''.$vrow['TimeCardId'].'\',\''.$this->ArrEmp[$lvvt][0].'\',this.value,1)"/>';
						}
						else
						{
							$lvstrgt=(trim($vrow['lv010'])=="")?"&nbsp;":$vrow['lv010'];
							$lvstrgt1=(trim($vrow['lv010'])=="")?"&nbsp;":$vrow['lv010'];
							$lvstrgt2=(trim($vrow['lv008'])=="")?"&nbsp;":$vrow['lv008'];
						}
			...
			$this->ArrEmp[$lvvt][6]=str_replace("<!--_".$vrow['lv004']."-->",$lvstrgt1.$lvstrgt2,$this->ArrEmp[$lvvt][6]);
	...
	if($this->lvSort==1)		
		$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
			...
			@01
			<tr ><td colspan=\"".($lvNumDate+5)."\">".$this->TabFunction($lvFrom,$lvList,$maxRows)."</td><td colspan=\"4\" align=right>".$this->ListFieldExport($lvFrom,$lvList,$maxRows)."</td></tr>
		</table>
		";
		else
			$vTable="<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\" id=\"tabletc\">
		...
		for($i=0;$i<count($this->ArrEmp);$i++)
		{
			...
			$vp_col1_1="<td nowrap='nowrap'>".$this->FormatView($this->ArrEmp[$i][3],10)."</td><td nowrap='nowrap'>".$this->GetValueArr($vArr,$this->ArrEmp[$i][4])."</td><td nowrap='nowrap'>".$this->GetValueArr($vArrPetrol,$this->ArrEmp[$i][4])."</td>";
			...
			$vp_col3="<td nowrap='nowrap'><input style='width:60px' type='text' value='".$this->ArrEmp[$i][3]."' onblur=\"UpdateMonthly('".$this->ArrEmp[$i][8]."','".$this->ArrEmp[$i][0]."',this.value,1)\"/></td><td nowrap='nowrap'><select ".(($vFocus==1)?' tabindex="1"':'').'" style="width:60px" onblur="UpdateMonthly(\''.$this->ArrEmp[$i][8].'\',\''.$this->ArrEmp[$i][0].'\',this.value,2)">'.$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td>";
			...
			$vp_col3_2="<td nowrap='nowrap'><input style='width:60px' type='text' value='".$this->ArrEmp[$i][3]."' onblur=\"UpdateMonthly('".$this->ArrEmp[$i][8]."','".$this->ArrEmp[$i][0]."',this.value,1)\"/></td><td nowrap='nowrap'><select ".(($vFocus==1)?' tabindex="1"':'').'" style="width:60px" onblur="UpdateMonthly(\''.$this->ArrEmp[$i][8].'\',\''.$this->ArrEmp[$i][0].'\',this.value,2)">'.$this->CreateSelectArr($vArr,$this->ArrEmp[$i][4])."</select>"."</td><td nowrap='nowrap'><select ".(($vFocus==1)?' tabindex="1"':'').'" style="width:60px" onblur="UpdateMonthly(\''.$this->ArrEmp[$i][8].'\',\''.$this->ArrEmp[$i][0].'\',this.value,5)">'.$this->CreateSelectArr($vArrPetrol,$this->ArrEmp[$i][9])."</select>"."</td>";
			$vp_col3_3="<td nowrap='nowrap'>".$this->FormatView($this->ArrEmp[$i][3],10)."</td><td nowrap='nowrap'>".$this->GetValueArr($vArr,$this->ArrEmp[$i][4])."</td><td nowrap='nowrap'>".$this->GetValueArr($vArrPetrol,$this->ArrEmp[$i][4])."</td>";
			...
			{
				if($childfunc=="rpt"  || $childfunc=="excel" || $childfunc=="word" || $childfunc=="pdf")
					$vp_col1="<tr style='background-color:".$color."'><td nowrap='nowrap'>".($i+1)."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td>";
				else
					$vp_col1="<tr style='background-color:".$color."'><td nowrap='nowrap'>".($i+1)."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][0]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][1]."</td><td nowrap='nowrap'>".$this->ArrEmp[$i][2]."</td>";
				if($this->ArrEmp[$i][7]==1)
				{
					if($this->is_tc09_unapr==1)
						$lvListTrAll=$lvListTrAll."$vp_col1$vp_col2$vp_col3_1".$vp_col4.$vp_col5;	
					else
						$lvListTrAll=$lvListTrAll."$vp_col1<td nowrap='nowrap'>".$this->ArrEmp[$i][7]."</td>$vp_col3_1".$vp_col4.$vp_col5;	
				}
				else
					{
						if($this->is_tc09_add==1)
						{
							if($this->is_tc09_apr==1)
								$lvListTrAll=$lvListTrAll."$vp_col1$vp_col2_1$vp_col3".$vp_col4.$vp_col5;	
							else
								$lvListTrAll=$lvListTrAll."$vp_col1<td nowrap='nowrap'>".$this->ArrEmp[$i][7]."</td>$vp_col3_2".$vp_col4.$vp_col5;	
						}
						else
						{
							if($this->is_tc09_apr==1)
								$lvListTrAll=$lvListTrAll."$vp_col1$vp_col2_1$vp_col1_1".$vp_col4.$vp_col5;	
							else
								$lvListTrAll=$lvListTrAll."$vp_col1$vp_col2_2".$vp_col4.$vp_col5;	
						}
					}
			}
			else
			{
				...
				{
					if($this->is_tc09_unapr==1)
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col2$vp_col3_3".$vp_col4.$vp_col5;	
					else
						$lvListTrAll=$lvListTrAll."<tr style='background-color:".$color."'>$vp_col1$vp_col3_3".$vp_col4.$vp_col5;	
				}
				else
					if($this->is_tc09_add==1)
					{
