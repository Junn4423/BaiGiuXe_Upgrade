...
switch ($vtable) {
    ...
    case "pm_nc0011":
        ...
        switch ($vfun) {
            case "data":
                $obj = $pm_nc0011->LoadAll();
                $vOutput = [];
                while ($row = db_fetch_array($obj)) {
                    $vOutput[] = [
                        'maKhuVuc' => $row['lv001'],
                        'taiKhoanDN' => $row['lv002'],
                        'moTa' => $row['lv003'],
                    ];
                }
                break;
            ...
            case "add":
                $pm_nc0011->lv001 = $input['maKhuVuc'] ?? $_POST['lv001'] ?? null;
                $pm_nc0011->lv002 = $input['taiKhoanDN'] ?? $_POST['lv002'] ?? null;
                $pm_nc0011->lv003 = $input['moTa'] ?? $_POST['lv003'] ?? null;

                $vOutput = $pm_nc0011->KB_Insert()
                    ? ['success' => true, 'message' => 'Thêm thành công']
                    : ['success' => false, 'message' => 'Lỗi khi thêm bản ghi'];
                break;

            case "delete":
                $lv001 = $input['maKhuVuc'] ?? $_POST['lv001'] ?? null;
                $lv002 = $input['taiKhoanDN'] ?? $_POST['lv002'] ?? null;

                $vOutput = $pm_nc0011->KB_Delete($lv001, $lv002)
                    ? ['success' => true, 'message' => 'Xóa thành công']
                    : ['success' => false, 'message' => 'Lỗi khi xóa bản ghi'];
                break;
            
            case "layKhuVucLamViecCuaNhanVien":
                $lv002 = $input['taiKhoanDN'] ?? $_POST['lv002'] ?? null;
                $obj = $pm_nc0011->KB_LoadID($lv002);
                $vOutput = [];
                while ($row = db_fetch_array($obj)) {
                    $vOutput[] = [
                        'maKhuVuc' => $row['lv001'],
