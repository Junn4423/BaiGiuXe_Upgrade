...
function unicode_to_upper($fac_contetn)
{
 ...
 function is_valid_email($email)
	...
	/**
	 * Common functions for the Parking Management System API
	 */
	...
	// Function to check if a request parameter exists and is not empty
	function validateParam($param, $required = true) {
		if ($required && (!isset($param) || empty($param))) {
			return false;
		}
		return true;
	}
	...
	function logApiRequest($method, $endpoint, $params, $response) {
		$logDir = __DIR__ . '/logs';
		...
		$logEntry = "[{$timestamp}] {$method} {$endpoint} - Params: " . json_encode($params) . 
					" - Response: " . json_encode($response) . PHP_EOL;
		
		file_put_contents($logFile, $logEntry, FILE_APPEND);
	}

	// Function to check if a vehicle type exists
	function vehicleTypeExists($conn, $typeId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0001 WHERE lv001 = ?");
		$stmt->execute([$typeId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a vehicle exists
	function vehicleExists($conn, $licensePlate) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0002 WHERE lv001 = ?");
		$stmt->execute([$licensePlate]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if an RFID card exists
	function cardExists($conn, $cardUid) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0003 WHERE lv001 = ?");
		$stmt->execute([$cardUid]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a parking zone exists
	function zoneExists($conn, $zoneId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0004 WHERE lv001 = ?");
		$stmt->execute([$zoneId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a parking spot exists
	function spotExists($conn, $spotId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0005 WHERE lv001 = ?");
		$stmt->execute([$spotId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a camera exists
	function cameraExists($conn, $cameraId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0006 WHERE lv001 = ?");
		$stmt->execute([$cameraId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a gate exists
	function gateExists($conn, $gateId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0007 WHERE lv001 = ?");
		$stmt->execute([$gateId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a price policy exists
	function pricePolicyExists($conn, $policyId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0008 WHERE lv001 = ?");
		$stmt->execute([$policyId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to check if a parking session exists
	function sessionExists($conn, $sessionId) {
		$stmt = $conn->prepare("SELECT COUNT(*) FROM pm_nc0009 WHERE lv001 = ?");
		$stmt->execute([$sessionId]);
		return $stmt->fetchColumn() > 0;
	}

	// Function to calculate parking fee
	function calculateParkingFee($conn, $sessionId, $exitTime) {
		try {
			// Get session and price policy details
			$stmt = $conn->prepare("
				SELECT s.lv008 as entry_time, p.lv003 as free_minutes, 
					p.lv004 as package_price, p.lv005 as has_overtime_fee, 
					p.lv006 as overtime_price
				FROM pm_nc0009 s
				JOIN pm_nc0008 p ON s.lv005 = p.lv001
				WHERE s.lv001 = ?
			");
			$stmt->execute([$sessionId]);
			$data = $stmt->fetch();
			...
			$durationMinutes = ($exitDateTime->getTimestamp() - $entryTime->getTimestamp()) / 60;
			
			// Calculate fee
			$fee = $data['package_price'];
			if ($durationMinutes > $data['free_minutes'] && $data['has_overtime_fee']) {
				$overtimeMinutes = $durationMinutes - $data['free_minutes'];
				$overtimeHours = $overtimeMinutes / 60;
				$overtimeFee = $overtimeHours * $data['overtime_price'];
				$fee += $overtimeFee;
			}
			
			return round($fee);
		} catch (Exception $e) {
			error_log("Error calculating parking fee: " . $e->getMessage());
			return 0;
		}
	}

	// Function to generate a unique session ID
	function generateSessionId($cardUid, $licensePlate) {
		$timestamp = date('YmdHis');
		return $cardUid . '_' . $licensePlate . '_' . $timestamp;
	}
